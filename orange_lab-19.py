...
def generate_pdf_html_report(
    ...
    def _xlate_patho(text: str) -> str:
        ...
        _WORD_EN = {
            "المؤشرات": "Indicators", "تدعم": "support", "بقوة": "strongly",
            "وجود": "presence of", "عدوى": "infection", "حقيقية": "true",
            "يُنصح": "recommended", "بالعلاج": "treatment", "الموجَّه": "directed",
            "بنتيجة": "by result", "الحساسية": "sensitivity", "مع": "with",
            "مراعاة": "considering", "السياق": "context", "الكلينيكي": "clinical",
            "ابدأ": "Start", "العلاج": "therapy", "بناءً": "based",
            "على": "on", "نتيجة": "result", "الـ": "", "راجع": "review",
            "الجرعة": "dose", "حسب": "per", "الوظيفة": "function",
            "الكلوية": "renal", "راعِ": "consider", "شدة": "severity",
            "الأعراض": "symptoms", "وعوامل": "and factors", "الخطر": "risk",
            "النتيجة": "Result", "حدودية": "borderline", "تقييم": "assessment",
            "كامل": "full", "قبل": "before", "البدء": "starting",
        }
  ...
  <div class="hdr-right">
    ...
    if ranked:
        ...
        if is_preg and preg_warn_items:
            H.append(f'<div style="font-size:8pt;color:#6c3483;margin-top:0.5mm">'
                     f'{_T["preg_extra"]}</div>')
    else:
        H.append(f'<div class="sec-ttl">{_T["recommended"]}</div>')
        if is_preg and preg_warn_items:
    ...
    if banned:
        ...
        H.append(
            f'<div class="sec-ttl" style="margin-top:0.6mm;color:#922b21;border-bottom-color:#922b21">'
            f'{_T["avoid"]}</div>'
    ...
    if warned:
        H.append(f'<div class="sec-ttl" style="margin-top:0.6mm;color:#b7770d;border-bottom-color:#b7770d">'
                 f'{_T["dose_adj"]}</div>')
        if is_renal:
    ...
    if is_preg and preg_warn_items:
        ...
        H.append(
            f'<div class="sec-ttl" style="color:#7d3c98;border-bottom-color:#7d3c98">'
            f'{_T["pregnancy"]} &nbsp;'
            f'<span style="font-size:7pt;font-weight:normal;color:#888">'
