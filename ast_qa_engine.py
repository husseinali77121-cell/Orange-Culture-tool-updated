...
# ══════════════════════════════════════════════════════════════════════════════
# LEVEL 1 — Intrinsic Resistance
# ══════════════════════════════════════════════════════════════════════════════
# Drug reported as S when the organism is KNOWN to be intrinsically resistant
_INTRINSIC_RESISTANCE: Dict[str, List[str]] = {
    # Organism → list of drugs to which it is intrinsically RESISTANT
    "Enterococcus faecalis": [
        "Cephalexin","Cefadroxil","Cefaclor","Cefuroxime","Cefuroxime sodium",
        "Ceftriaxone","Cefotaxime","Ceftazidime","Cefepime","Cefazolin","Cefoxitin",
        "Trimethoprim/Sulfamethoxazole","Clindamycin","Aztreonam",
    ],
    "VRE": [
        "Vancomycin","Cephalexin","Cefadroxil","Cefuroxime","Ceftriaxone",
        "Trimethoprim/Sulfamethoxazole","Clindamycin",
    ],
    "Proteus mirabilis": ["Nitrofurantoin","Colistin","Tetracycline"],
    "Pseudomonas aeruginosa": [
        "Cefuroxime","Cefuroxime sodium","Cefaclor","Cephalexin","Cefadroxil",
        "Ertapenem","Nitrofurantoin","Trimethoprim/Sulfamethoxazole",
        "Amoxicillin + Clavulanic acid","Ampicillin",
    ],
    "Acinetobacter baumannii": ["Aztreonam","Trimethoprim","Fosfomycin"],
    "Klebsiella spp.":   ["Ampicillin","Amoxicillin"],
    "E. coli":           [],  # few true intrinsic
    "Stenotrophomonas maltophilia": [
        "Imipenem/Cilastatin","Meropenem","Ertapenem",
        "Ceftazidime","Cefepime","Ampicillin","Amoxicillin + Clavulanic acid",
    ],
    "Mycoplasma spp.": [
        "Amoxicillin + Clavulanic acid","Ampicillin","Ampicillin/Sulbactam",
        "Cephalexin","Ceftriaxone","Cefotaxime","Vancomycin","Penicillin",
    ],
    "MRSA": [
        "Oxacillin","Cefazolin","Cephalexin","Cefadroxil","Cefuroxime","Cefuroxime sodium",
        "Ceftriaxone","Cefotaxime","Cefepime","Amoxicillin + Clavulanic acid",
        "Ampicillin/Sulbactam","Piperacillin + Tazobactam","Ertapenem","Penicillin",
    ],
}

def _check_intrinsic_resistance(organism: str, sir: Dict[str, str]) -> List[QAIssue]:
    issues = []
    resistant_drugs = _INTRINSIC_RESISTANCE.get(organism, [])
...
def _check_phenotype_consistency(
    ...
    # ── ESBL phenotype checks (Gram-negatives) ──
        ...
        for drug in ("Ceftriaxone", "Cefotaxime", "Ceftazidime", "Cefepime"):
            if _is_S(drug, sir):
                issues.append(QAIssue(
                    level=2, severity="HIGH",
                    category="Phenotype Consistency",
                    message=f"ESBL/AmpC predicted but {drug}=S",
                    detail=(
                        f"ESBL/AmpC-producing organisms should exhibit resistance or "
                        f"reduced susceptibility to {drug} due to inoculum effect and enzyme activity. "
                        f"{drug}=S may represent false susceptibility (inoculum effect). "
                        f"EUCAST/CLSI recommend reporting as R regardless of in-vitro result for ESBL."
                    ),
                    drug=drug,
                    reference="EUCAST 2026 ESBL Expert Rules / IDSA AMR 2025",
                ))
