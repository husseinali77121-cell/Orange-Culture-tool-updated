...
def load_commercial_names(filepath: str = "commercial_names.txt") -> Dict[str, str]:
    """Loads commercial names — searches multiple paths for Streamlit Cloud compat."""
    result: Dict[str, str] = {}
    import os as _os
...
# ═══════════════════════════════════════════════════════════════════════
# CLINICAL ENGINES — v4.0
# Treatment Duration | IV→PO Switch | Hepatic Dosing
# Combination Therapy | De-escalation Advisor
# Reference: IDSA AMR 2025 | Sanford 2025 | WHO AWaRe 2025
# ═══════════════════════════════════════════════════════════════════════
# ═══════════════════════════════════════════════════════════════════════
...
# Professional A4 Layout | ReportLab Platypus
# Orange Lab Clinical Report Design System
# ═══════════════════════════════════════════════════════════════════════
# ═══════════════════════════════════════════════════════════════════════
...
def _drug_section(drugs: list, aware: str, st_: dict,
    ...
    for d in drugs:
        ...
        if brands:
            cell_left.append(Paragraph(f"Brands: {brands}", st_["small"]))
...
def generate_pdf_report_en(
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Orange Lab Report — {patient_name} — {organism}",
    ...
    pat_data = [
        ...
        [Paragraph("<b>Pregnant</b>", st_["body_bold"]),
         Paragraph("YES — Review pregnancy warnings below" if is_preg else "No", 
                   st_["warn"] if is_preg else st_["body"]),
    ...
    story += [
        ...
        Paragraph(
            ...
            st_["disclaimer"]),
        Paragraph(
            "This report is generated by Orange Culture Tool (CDSS) — "
...
if uploaded:
    ...
    with col2:
        ...
        if final_drugs:
            ...
            # ════════════════════════════════════════════════════════════
            # SECTION: Clinical Decision Engines UI
            # ════════════════════════════════════════════════════════════
            st.divider()

            # ── A. Treatment Duration ──────────────────────────────────
            with st.expander("⏱️ Treatment Duration", expanded=False):
                st.caption("Evidence-based treatment duration — IDSA AMR 2025 | Sanford 2025")
                sev_opt = st.selectbox(
                    "Severity",
                ...
                st.session_state.severity_level = sev_opt
                syn_label = syn["syndrome"] if syn else ""
                _dur = get_treatment_duration(
                    specimen=culture_type, organism=organism_type,
                    syndrome=syn_label, age=age, sex=sex,
                    is_renal=is_renal, phenotypes=phenotypes, severity=sev_opt,
                )
                dc1, dc2, dc3 = st.columns(3)
                dc1.metric("Standard", f"{_dur.get('standard_days',_dur.get('standard','?'))} days")
                dc2.metric("Range", f"{_dur.get('min_days','?')}–{_dur.get('max_days','?')} days")
                dc3.metric("IV / PO", f"IV:{_dur.get('iv_days',0)}d PO:{_dur.get('po_days',0)}d")
                st.info(f"📋 {_dur.get('notes','')}")
                if _dur.get("follow_up_culture"):
                    st.warning("🔄 Follow-up culture recommended after treatment completion")
                st.caption(f"📚 Ref: {_dur.get('ref','')}")

            # ── B. Combination Therapy (auto-shows if MDR detected) ────
            _combos = get_combination_therapy(phenotypes)
            if _combos:
                with st.expander(f"🔬 Combination Therapy — {len(_combos)} phenotype(s)", expanded=True):
                    st.caption("MDR/XDR combination therapy — IDSA AMR 2025 | ESCAPE organisms")
                    for _cs in _combos:
                        _pd = _cs["data"]
                        urg_color = "error" if _pd["urgency"] == "CRITICAL" else "warning"
                        if urg_color == "error":
                            st.error(f"**{_pd['urgency']}** — {_pd['title']}")
                        else:
                            st.warning(f"**{_pd['urgency']}** — {_pd['title']}")
                        for _opt in _pd["options"]:
                            _is_avoid = "AVOID" in _opt["evidence"] or "AVOID" in _opt["combo"].upper()
                            if _is_avoid:
                                st.error(f"🚫 {_opt['combo']} | {_opt['caution']}")
                            else:
                                with st.container(border=True):
                                    _c1, _c2 = st.columns([3,1])
                                    with _c1:
                                        st.markdown(f"**{_opt['combo']}** — {_opt['evidence']}")
                                        st.caption(_opt["indication"])
                                        if _opt.get("caution"):
                                            st.warning(_opt["caution"])
                                    with _c2:
                                        st.caption(_opt["ref"])

            # ── C. IV → PO Switch ──────────────────────────────────────
            with st.expander("💊 IV → PO Switch", expanded=False):
                st.caption("OPAT Switch Evaluation — IDSA 2019 | BNF 2025")
                sw1, sw2 = st.columns(2)
                with sw1:
                    _sw_drug = st.selectbox(
                        "Current IV drug",
                        [""] + [d["name"] for d in allowed],
                        key="sw_drug_sel"
                    )
                    _sw_days = st.number_input(
                        "Days on IV", min_value=0, max_value=30,
                        value=st.session_state.get("days_on_iv",3),
                        key="sw_days_num"
                    )
                    st.session_state.days_on_iv = _sw_days
                with sw2:
                    _sw_improving = st.checkbox("Clinical improvement documented",
                        value=st.session_state.get("clinical_improving_48h",True),
                        key="sw_impr_chk")
                    _sw_oral = st.checkbox("Tolerating oral medications",
                        value=st.session_state.get("tolerating_oral",True),
                        key="sw_oral_chk")
                    _sw_bact = st.checkbox("No active bacteremia",
                        value=st.session_state.get("bacteremia_resolved",True),
                        key="sw_bact_chk")
                    st.session_state.clinical_improving_48h = _sw_improving
                    st.session_state.tolerating_oral        = _sw_oral
                    st.session_state.bacteremia_resolved    = _sw_bact
                if _sw_drug:
                    _sw_res = evaluate_iv_po_switch(
                        drug_name=_sw_drug,
                        syndrome=syn["syndrome"] if syn else "",
                        clinical_improving=_sw_improving,
                        tolerating_oral=_sw_oral,
                        bacteremia_resolved=_sw_bact,
                        days_on_iv=_sw_days,
                    )
                    if _sw_res["can_switch"]:
                        st.success(_sw_res["verdict"])
                    else:
                        st.warning(_sw_res["verdict"])
                    _c1, _c2 = st.columns(2)
                    with _c1:
                        st.markdown("**✅ Supporting:**")
                        for _s in _sw_res["supporters"]: st.write(f"• {_s}")
                    with _c2:
                        st.markdown("**⚠️ Blocking:**")
                        for _b in _sw_res["blockers"]: st.write(f"• {_b}")
                    st.caption(f"📚 {_sw_res['ref']}")
                else:
                    st.info("Select current IV drug to evaluate switch criteria")
            ...
            if is_hepatic:
                with st.expander("🟡 Hepatic Dosing (Child-Pugh)", expanded=True):
                    ...
                    _cp = st.selectbox(
                        ...
                        format_func=lambda x: {"A":"A — Mild (5-6 pts)","B":"B — Moderate (7-9 pts)","C":"C — Severe (10-15 pts)"}[x],
                        key="cp_class_sel"
                    )
                    st.session_state.child_pugh_class = _cp
                    _hep_recs = get_hepatic_recommendations(allowed, _cp)
                    _action_recs = [r for r in _hep_recs if r["requires_action"]]
                    _normal_recs = [r for r in _hep_recs if not r["requires_action"]]
                    if _action_recs:
                        st.markdown("**⚠️ Adjustments required:**")
                        for _r in _action_recs:
                            if "Avoid" in _r["level"]:
                                st.error(f"❌ **{_r['name']}**: {_r['recommendation']} | _{_r['note']}_")
                            else:
                                st.warning(f"⚠️ **{_r['name']}**: {_r['recommendation']} | _{_r['note']}_")
                    if _normal_recs:
                        with st.expander(f"✅ {len(_normal_recs)} drugs need no hepatic adjustment"):
                            for _r in _normal_recs:
                                st.caption(f"✅ {_r['name']}: {_r['recommendation']}")
                    if not _hep_recs:
                        st.success("No hepatic adjustments needed for current recommendations")

            # ── E. De-escalation Advisor ───────────────────────────────
            with st.expander("📉 De-escalation Advisor", expanded=False):
                ...
                with _de1:
                    _de_hrs = st.number_input(
                        "Hours on current therapy", min_value=0, max_value=336, step=12,
                        value=st.session_state.get("hours_on_treatment",72),
                        key="de_hrs_num"
                    )
                    st.session_state.hours_on_treatment = _de_hrs
                with _de2:
                    _de_impr = st.checkbox(
                        "Clinical improvement documented",
                        value=st.session_state.get("de_clinical_improving",True),
                        key="de_impr_chk"
                    )
                    st.session_state.de_clinical_improving = _de_impr
                _de_res = evaluate_deescalation(
                    allowed=allowed, phenotypes=phenotypes,
                    hours_on_treatment=_de_hrs, clinical_improving=_de_impr,
                )
                if _de_res["can_deescalate"]:
                    st.success("✅ De-escalation recommended")
                elif _de_hrs < 48:
                    st.info("ℹ️ Complete 48h course before reassessment")
                else:
                    st.warning("⚠️ Review required before de-escalating")
                for _rec in _de_res["recommendations"]:
                    st.write(_rec)
                st.caption(f"📚 {_de_res['ref']}")
            ...
            st.markdown("#### 📄 PDF Report (English Only)")
            st.caption("Professional A4 format — Arabic not supported in PDF, English only")
            _pdf_cols = st.columns(3)
            with _pdf_cols[0]:
                _pdf_combo = st.checkbox("Include Combination Therapy", 
                    value=st.session_state.get("pdf_include_combo", True), key="pdf_combo_chk")
            with _pdf_cols[1]:
                _pdf_dur   = st.checkbox("Include Treatment Duration",
                    value=st.session_state.get("pdf_include_duration", True), key="pdf_dur_chk")
            with _pdf_cols[2]:
                _pdf_patho = st.checkbox("Include Pathogenicity Score",
                    value=st.session_state.get("pdf_include_patho", True), key="pdf_patho_chk")
            ...
            if REPORTLAB_AVAILABLE:
                _syn_label = syn["syndrome"] if syn else ""
                _dur_data  = get_treatment_duration(
                    specimen=culture_type, organism=organism_type,
                    syndrome=_syn_label, age=age, sex=sex,
                    is_renal=is_renal, phenotypes=phenotypes,
                    severity=st.session_state.get("severity_level","moderate"),
                ) if _pdf_dur else None
                _combos_pdf = get_combination_therapy(phenotypes) if _pdf_combo else None
                _cp_class   = st.session_state.get("child_pugh_class","A")
                _hep_recs_pdf = get_hepatic_recommendations(allowed, _cp_class) if is_hepatic else None

                _pdf_bytes = generate_pdf_report_en(
                    patient_name        = _pt,
                    age=age, sex=sex, weight=weight,
                    cl_cr=cl_cr, is_renal=is_renal,
                    is_preg=is_preg, is_hepatic=is_hepatic,
                    ...
                    duration_data=_dur_data,
                    combo_suggestions=_combos_pdf,
                    show_commercial_names=show_commercial,
                    child_pugh=_cp_class,
                    hepatic_recs=_hep_recs_pdf,
                )
                if _pdf_bytes:
                    st.download_button(
                        "📄 Download PDF Report (EN)",
                        data=_pdf_bytes,
                        file_name=(f"Orange_Report_{organism_type.replace(' ','_')}_"
                                   f"{_pt.replace(' ','_')[:15]}_"
                    ...
                    st.error("Failed to generate PDF — check reportlab installation")
            else:
                st.info("Install `reportlab` in requirements.txt to enable PDF export")
