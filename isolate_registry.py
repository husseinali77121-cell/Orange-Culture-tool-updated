...
class IsolateRegistry:
    ...
    def sync_pull(self, token: str, repo: str, branch: str = "main",
        ...
        if content is None:
            logger.info("Registry pull: no remote DB yet, or the fetch failed — "
                        "keeping the local database.")
            return False
        # Belt and braces alongside the _gh_get fix. Zero bytes is never a
        # legitimate registry: SQLite treats an empty file as a valid empty
        # database, so without this the guard below would compare 0 local
        # against 0 remote, adopt nothing, and let the next push erase the
        # real DB on GitHub. An empty body means the FETCH failed, not that
        # the registry is empty.
        if not content:
            logger.error("Registry pull: remote returned EMPTY content — refusing "
...
def _gh_get(token: str, repo: str, path: str, branch: str):
    """Return (content_bytes, sha), or (None, sha/None) when the content is not
    retrievable.

    ⚠️ THE 1 MB CLIFF. The Contents API only inlines base64 content for files up
    to 1 MB. From 1-100 MB the JSON still returns 200 OK with the metadata and
    the correct sha, but `content` is an EMPTY STRING and `encoding` is "none";
    the bytes are only available through the raw media type.

    The old code did `content = base64.b64decode(b64) if b64 else b""` and handed
    back b"" — empty bytes, not None. sync_pull()'s `if content is None` guard
    therefore passed, the empty buffer was written to a temp file, SQLite happily
    opened a zero-byte file as a valid EMPTY database, and remote_count came back
    0 with no exception raised (so the "remote unreadable → keep local" branch
    never fired either). On a Streamlit Cloud cold start the local DB is the
    empty one committed to the repo, so local_count was also 0, `local_count >
    remote_count` was 0 > 0 = False, and the empty DB was adopted — after which
    the next save pushed it over the real registry on GitHub. The whole
    antibiogram history, gone, and sync_pull() returned True and logged success.

    So: fetch metadata for the sha, take the inline content when GitHub gives it,
    and otherwise re-fetch through the raw media type. Never return b"".
    """
    url = _GH_API.format(repo=repo, path=path) + f"?ref={branch}"

    # ── 1. Metadata (always returned, whatever the file size) ──────────────
    ...
    if b64 and data.get("encoding") == "base64":
        try:
            return base64.b64decode(b64), sha                 # ≤ 1 MB — the normal path
        except Exception as e:
            logger.error("GitHub GET %s: bad base64 (%s)", path, e)
            return None, sha

    # ── 2. > 1 MB → content is "" / encoding "none": use the raw media type ──
    logger.info("GitHub GET %s: no inline content (size=%s) — retrying raw.",
                path, data.get("size"))
    ...
    try:
        with urllib.request.urlopen(req, timeout=60) as resp:
            body = resp.read()
        if not body:
            logger.error("GitHub raw GET %s returned no bytes — treating as failure.", path)
            return None, sha
        return body, sha
    except Exception as e:
        # Returning None (not b"") is what stops sync_pull from adopting nothing.
        logger.error("GitHub raw GET %s failed: %s — pull will be skipped.", path, e)
        return None, sha
