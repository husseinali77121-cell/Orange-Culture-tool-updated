...
def _ar2en(text, en=True):
    """Render Arabic clinical wording in English — or say plainly that it can't.

    No-op for the Arabic report (`en=False`) and for text that has no Arabic.

    WHY THIS IS A FALLBACK AND NOT THE MECHANISM. This function used to end with
    `re.sub(r'[\\u0600-\\u06FF]+', '', s)` — delete every Arabic run the tables
    did not know. Because `_AR2EN_WORDS` also maps bare function words
    ("لا"→"no", "من"→"from"), half a sentence was substituted and the other half
    deleted, so a real clinical statement reached the English PDF as a fragment
    that still looked like English:

        "المزرعة أثبتت أن Ceftriaxone لا يثبط نمو الجرثومة. MIC أعلى من الحد
         العلاجي → خطر فشل علاجي."
      → "Ceftriaxone no . MIC from → risk of ."

    and, worse, "...حتى لو أظهرت المزرعة حساسية" ("even if the culture reports
    susceptible") came out as "until susceptibility" — the opposite instruction.
    A prescriber cannot see that words are missing; a mangled sentence is more
    dangerous than an obviously absent one.

    So: clinical text is now AUTHORED in English at source (`reason_short_en` /
    `reason_detail_en` on banned items, `esbl_note_en` on warnings). This
    function only handles strings that have no authored English yet, and when it
    cannot translate one fully it returns a marker pointing at the Arabic report
    instead of inventing a sentence. Every such case is logged with the original
    text so the missing `_en` string can be added at source.
    """
    ...
    if _AR_RE.search(s):
        # Untranslatable. Do NOT delete the leftovers and ship the remains.
        logger.warning("EN PDF: no authored English for %r — emitting marker. "
                       "Add reason_*_en / *_en at source.", text[:120])
        return _EN_UNAVAILABLE
