...
def analyze_antibiotics(
    ...
    for drug in final_drugs:
        ...
        # CRITICAL pregnancy safety: tetracyclines & fluoroquinolones are
        # CONTRAINDICATED in pregnancy regardless of their "Warn" tag.
        # Tetracyclines → fetal bone/teeth damage. FQs → cartilage/arthropathy.
        if is_preg and info.get("preg_status") == "Warn":
            if "tetracycline" in cls:
                banned.append(build_banned_item(
                    drug, "pregnancy",
                    "ممنوع في الحمل — التتراسيكلينات تُتلف عظام وأسنان الجنين.",
                    "Tetracyclines (Doxycycline/Tetracycline/Minocycline) محظورة في الحمل "
                    "(خاصة بعد الأسبوع 15): ترسّب في العظام والأسنان، تصبغ دائم للأسنان، "
                    "وتثبيط نمو العظام. بديل: Amoxicillin-Clav أو Cephalosporin حسب الحساسية.",
                ))
                continue
            if "fluoroquinolone" in cls:
                banned.append(build_banned_item(
                    drug, "pregnancy",
                    "ممنوع في الحمل — الفلوروكينولونات تؤثر على غضاريف الجنين.",
                    "Fluoroquinolones (Cipro/Levo/Ofloxacin) تُتجنب في الحمل: "
                    "سُمّية غضروفية (arthropathy) في الدراسات الحيوانية. "
                    "استخدم بديلاً آمناً حسب الحساسية.",
                ))
                continue
            # Other genuinely trimester-dependent drugs → caution ONLY (no duplication)
