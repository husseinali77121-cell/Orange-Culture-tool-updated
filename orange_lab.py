...
def detect_pus_cells(text: str) -> str:
    """
    استخرج قيمة Pus cells / WBCs من نص OCR.
    يدعم صيغ: 6-8, 10-15, >10, 5, 8-12/HPF
    """
    import re
    patterns = [
        # Pus Cells: 6-8 /HPF  or  Pus cells 6 - 8
        r"pus\s*cells?\s*[:\-]?\s*(\d+\s*[-–]\s*\d+|\d+)",
        r"wbc\s*[:\-]?\s*(\d+\s*[-–]\s*\d+|\d+)",
        r"wbcs\s*[:\-]?\s*(\d+\s*[-–]\s*\d+|\d+)",
        r"خلايا\s*صديد\s*[:\-]?\s*(\d+\s*[-–]\s*\d+|\d+)",
        # standalone before /HPF
        r"(\d+\s*[-–]\s*\d+)\s*/\s*hpf",
        r"(\d+)\s*/\s*hpf",
    ]
    text_l = text.lower()
...
def detect_rbcs(text: str) -> str:
    ...
    # Second line with /HPF (after pus cells line)
    lines = text_l.splitlines()
    hpf_lines = [l for l in lines if "/hpf" in l or "hpf" in l]
    if len(hpf_lines) >= 2:
        # First HPF line = pus, second = RBCs
        m2 = re.search(r"(\d+\s*[-–]\s*\d+|\d+)", hpf_lines[1])
        if m2:
...
def generate_decision_tree_image(
    ...
    def gf(size: int, bold: bool = False):
        """
        Font loader: tries Arabic-capable fonts first (Amiri supports Arabic+Latin),
        then Latin-only fallbacks. This fixes Arabic rendering on Debian Trixie.
        """
        import os as _os
        _b = "Bold" if bold else "Regular"
        paths = [
            # Amiri — full Arabic + Latin (installed via fonts-hosny-amiri in packages.txt)
            ...
            # Noto Naskh Arabic (installed via fonts-noto-core in packages.txt)
            "/usr/share/fonts/truetype/noto/NotoNaskhArabic-Regular.ttf",
            "/usr/share/fonts/truetype/noto/NotoSansArabic-Regular.ttf",
            "/usr/share/fonts/noto/NotoNaskhArabic-Regular.ttf",
            "/usr/share/fonts/noto/NotoSansArabic-Regular.ttf",
            # Liberation / DejaVu — Latin fallback
        ...
        for p in paths:
            try:
                ...
                continue
        # Last resort: search for any font in system
        for _fdir in ["/usr/share/fonts/opentype", "/usr/share/fonts/truetype", "/usr/share/fonts"]:
            if _os.path.isdir(_fdir):
                for _root, _, _files in _os.walk(_fdir):
                    for _f in _files:
                        if _f.lower().endswith((".ttf",".otf")) and any(
                            k in _f.lower() for k in ("amiri","noto","liberation","sans","arabic")):
                            try:
                                return ImageFont.truetype(_os.path.join(_root, _f), size * S)
                            except Exception:
                                continue
        return ImageFont.load_default()
...
if uploaded:
    ...
    with col1:
        ...
        # ── Auto-populate Pus/RBCs/Condition from OCR if available ─────────
        if payload:
