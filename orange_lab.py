...
body { font-family: 'Amiri','Noto Naskh Arabic','DejaVu Sans',Arial,sans-serif;
       font-size: 10pt; color: #1a1a2e; direction: ltr; background: #fff; }
...
.hdr-lab  { font-size:16pt; font-weight:bold; }
.hdr-sub  { font-size:9pt; opacity:0.85; margin-top:0.5mm; }
.hdr-pills { margin-top:1mm; }
.hdr-right { font-size:9pt; opacity:0.9; text-align:right; direction:ltr; }
.accent   { height:2mm; background:#ff8c00; }
.content  { padding: 3mm 0; }
/* Micro info grid: 4 columns */
.info4 { display: table; width: 100%; border-collapse: collapse; font-size:9pt; margin:2mm 0; }
.info4 tr td { padding: 1.5mm 3mm; border: 0.3pt solid #d5d8dc; }
.lbl4 { background:#f4f6f8; font-weight:bold; color:#0d3b66; width:14%; }
.val4 { width:22%; }
/* Section */
.sec-ttl { font-size:9.5pt; font-weight:bold; color:#0d3b66; text-transform:uppercase;
            border-bottom:1.2pt solid #0d3b66; padding-bottom:0.5mm; margin:3mm 0 1.5mm;
            direction:ltr; text-align:left; }
/* AST table */
.ast { width:100%; border-collapse:collapse; font-size:9pt; direction:ltr; }
.ast th { background:#0d3b66; color:#fff; padding:1.5mm 3mm; text-align:left; font-size:9pt; }
.ast td { padding:1.5mm 3mm; border:0.3pt solid #d5d8dc; text-align:left; }
...
.tier-sep { font-size:8.5pt; font-weight:bold; text-transform:uppercase; letter-spacing:0.4pt;
            direction:ltr; text-align:left;
            padding:0.5mm 0; margin-top:2mm; border-top:0.8pt solid; }
/* Alerts */
.alert { padding:1.5mm 3mm; border-radius:1.5mm; margin:0.8mm 0; font-size:9pt; direction:ltr; text-align:left; }
...
.compact-tbl { width:100%; border-collapse:collapse; font-size:9pt; direction:ltr; }
...
"""
  ...
  <div class="hdr-right">
    ...
    if allowed:
        ...
        prev_tier = ""
        for i, _rd in enumerate(ranked[:9], 1):
            _raw  = _rd.get("aware","")
    ...
    if banned:
        ...
        H.append(
            '<div class="sec-ttl" style="margin-top:3mm;color:#922b21;border-bottom-color:#922b21">'
    ...
    if duration_data:
        ...
        H.append(f'<table class="compact-tbl">'
                 ...
                 f'</table>')
        if d.get("notes"):
            H.append(f'<div class="alert al-info" style="font-size:8.5pt;margin-top:1mm">📋 {_esc(d["notes"][:160])}</div>')
    ...
    if patho_assessment:
        ...
        H.append(f'<div style="font-size:10.5pt;margin:1mm 0;font-weight:bold;color:{clr2}">{sc}% — {verd}</div>')
        # Interpretation
        if interp:
            H.append(f'<div style="font-size:9.5pt;color:#444;margin-bottom:1mm">{interp[:160]}</div>')
        # Flags
        flag_msgs = {
        ...
        # Recommendations
        if recs:
            H.append('<div style="font-size:8.5pt;font-weight:bold;margin-top:1mm">Recommendations:</div>')
            for r in recs[:3]:
