...
def generate_pdf_html_report(
    ...
    # ── Helpers ─────────────────────────────────────────────────────────
    AWARE_CSS = {"Access": ("access-card","access-pill","access-txt"),
                 "Watch":  ("watch-card","watch-pill","watch-txt"),
                 "Reserve":("reserve-card","reserve-pill","reserve-txt")}
    AWARE_CLR  = {"Access":"#1e8449","Watch":"#b7770d","Reserve":"#922b21"}
    TIER_MAP   = {"Access":"First-line","Watch":"Alternative","Reserve":"Reserve / MDR"}
...
.grid2 { display: table; width: 100%; border-collapse: separate; border-spacing: 2mm; }
.g2-l { display: table-cell; width: 49%; vertical-align: top; }
.g2-r { display: table-cell; width: 49%; vertical-align: top; }
.ranked-row { padding: 2mm 3mm; margin: 1mm 0; border-radius: 2mm;
              display: flex; justify-content: space-between; align-items: center;
              page-break-inside: avoid; }
.access-card  { background: #eafaf1; border: 0.8pt solid #1e8449; }
.watch-card   { background: #fef9e7; border: 0.8pt solid #b7770d; }
.reserve-card { background: #fdf2f2; border: 0.8pt solid #922b21; }
.access-pill  { background:#1e8449;color:#fff; }
.watch-pill   { background:#b7770d;color:#fff; }
.reserve-pill { background:#922b21;color:#fff; }
.access-txt   { color:#1e8449;font-weight:bold; }
.watch-txt    { color:#b7770d;font-weight:bold; }
.reserve-txt  { color:#922b21;font-weight:bold; }
.pill { padding:0.5mm 2.5mm;border-radius:2mm;font-size:7pt;font-weight:bold; }
.sir-pill { background:#fff;font-size:7pt;border:0.5pt solid #888;
            color:#333;padding:0.3mm 2mm;border-radius:1.5mm; }
.ranked-name { font-size:9.5pt; }
.tier-sep { font-size:7.5pt;font-weight:bold;text-transform:uppercase;
            letter-spacing:0.5pt;padding:1mm 0 0.5mm;margin-top:2mm;
            border-top:1pt solid; }
.compact-note { font-size:7pt; color:#666; }
"""

    def hdr(page_label: str, pg: int) -> str:
        mdr_pills = ""
        if mdr_class: mdr_pills += f'<span class="pill reserve-pill">{_esc(mdr_class)}</span> '
        for ph in ph_labels[:3]: mdr_pills += f'<span class="pill" style="background:#6e2fa0;color:#fff">{_esc(ph)}</span> '
        if esbl_prob=="high": mdr_pills += '<span class="pill watch-pill">ESBL+</span>'
        return f"""
<div class="header">
  <div>
    <div class="header-title">🔬 {_esc(lab_name)}</div>
    <div class="header-sub">{_esc(lab_city)} | Microbiology CDSS | {_esc(date_in or now_str[:10])}</div>
    {"<div style='margin-top:1mm'>"+mdr_pills+"</div>" if mdr_pills and pg==1 else ""}
  </div>
  <div class="header-right ltr" style="font-size:8pt">
    <b>{page_label}</b><br>
    {_esc(patient_name or "—")} | {age}y {_esc(sex)}<br>
    <i>{_esc(organism)}</i>
  </div>
...
<div class="content">"""
    ...
    # PAGE 1
    # ════════════════════════════════════════════════════════════════════
    H.append(f"""<!DOCTYPE html><html lang="ar" dir="rtl">
<head><meta charset="UTF-8"><style>{CSS}</style></head><body>""")
    H.append(hdr("PAGE 1 / 2 — CULTURE REPORT", 1))

    # ── Patient + Culture ──────────────────────────────────────────────
    renal_txt = f'<span class="warn-val">Impaired — CrCl:{cl_cr:.0f}</span>' if is_renal else "Normal"
    hep_txt   = f'<span class="warn-val">Impaired — CP-{child_pugh}</span>' if is_hepatic else "Normal"
    preg_row  = f'<tr><td class="lbl">Pregnancy</td><td class="val warn-val">YES — see alerts</td></tr>' if (is_preg and sex=="Female" and age>=12) else ""

    H.append(f"""<div class="grid2">
<div class="g2-l">
<div class="section-title">Patient</div>
<table class="info-table">
<tr><td class="lbl">Name</td><td class="val"><b>{_esc(patient_name or "—")}</b></td></tr>
<tr><td class="lbl">Age/Sex/Wt</td><td class="val">{age}y | {_esc(sex)} | {weight}kg</td></tr>
<tr><td class="lbl">Renal</td><td class="val">{renal_txt}</td></tr>
<tr><td class="lbl">Hepatic</td><td class="val">{hep_txt}</td></tr>
{preg_row}
</table></div>
<div class="g2-r">
<div class="section-title">Culture Result</div>
<table class="info-table">
...
<tr><td class="lbl">Pus Cells / RBCs</td><td class="val ltr">{_esc(pus_cells or "—")} / {_esc(rbcs or "—")} /HPF</td></tr>
</table></div></div>""")
    ...
    if sir_map:
        ...
        for drug, sir in sorted(sir_map.items()):
            aw   = _aware_map.get(drug,"")
            a_cls, a_pill, _ = AWARE_CSS.get(aw, ("","",""))
            _pill_td = (f'<span class="pill {a_pill}">{_esc(aw)}</span>' if aw else "")
            rows += (f'<tr><td>{_esc(drug)}</td>'
                     f'<td class="{_sir_class(sir)} ltr" style="font-size:10.5pt"><b>{sir}</b></td>'
                     f'<td>{_sir_label(sir)}</td>'
                     f'<td>{_pill_td}</td></tr>')

        H.append(f"""<div class="section">
<div class="section-title">Antibiotic Sensitivity Test (AST)</div>
<table class="ast-table">
<thead><tr><th>Antibiotic</th><th>Result</th><th>Interpretation</th><th>WHO AWaRe</th></tr></thead>
<tbody>{rows}</tbody></table></div>""")
    ...
    H.append(hdr("PAGE 2 / 2 — CLINICAL DECISION SUPPORT", 2))

    # ── Ranked Recommendations ─────────────────────────────────────────
    H.append('<div class="section"><div class="section-title">Recommended Therapy — Ranked</div>')
    prev_tier = ""
    for i, d in enumerate(ranked[:9], 1):
        aw        = d.get("aware","")
        tier_lbl  = TIER_MAP.get(aw, aw)
        clr       = AWARE_CLR.get(aw,"#444")
        card_cls, pill_cls, txt_cls = AWARE_CSS.get(aw, ("","",""))
        ...
        if tier_lbl != prev_tier:
            H.append(f'<div class="tier-sep {txt_cls}" style="border-color:{clr}">{tier_lbl}</div>')
            prev_tier = tier_lbl

        H.append(f"""<div class="ranked-row {card_cls}">
  <div style="flex:1">
    <span class="{txt_cls}" style="font-size:9.5pt"><b>{i}. {_esc(d.get("name",""))}</b></span>
    &ensp;<span class="sir-pill ltr">{sir_v}</span>
    &ensp;<span style="font-size:7.5pt;color:#555">{route}</span>
    {"<span class='compact-note'>&ensp;⚠ "+renal_n+"</span>" if renal_n else ""}
    {"<span class='compact-note'>&ensp;Brands: "+brands+"</span>" if brands else ""}
  </div>
  <div>{pill(aw, pill_cls)}</div>
</div>""")
    H.append("</div>")
    ...
    if banned:
        ...
        ban_grp = defaultdict(list)
        for d in banned: ban_grp[d.get("class","Other")].append(d.get("name",""))
        rows_ban = " | ".join(f'<b>{_esc(k)}:</b> {_esc(", ".join(vs))}' for k,vs in ban_grp.items())
        H.append(f'<div class="section no-break">'
                 f'<div class="section-title">🚫 Avoid (Culture-Resistant)</div>'
                 f'<div class="alert alert-danger" style="font-size:8pt">{rows_ban}</div>'
                 f'<div class="compact-note" style="margin-top:1mm">See AST table (Page 1) for full profile</div>'
                 f'</div>')

    # ── Caution / Renal / Pregnancy (compact, only if relevant) ────────
    alerts_html = ""
    if warned:
        for d in warned[:5]:
            r = _esc(d.get("reason_short","") or (d.get("renal_note","") if is_renal else ""))
            if r: alerts_html += f'<div style="font-size:8pt;margin:0.8mm 0">⚠ <b>{_esc(d.get("name",""))}</b>: {r}</div>'
    if is_preg and preg_warn_items:
        names_p = ", ".join(d.get("name","") for d in preg_warn_items[:4])
        alerts_html += (f'<div style="font-size:8pt;margin:0.8mm 0">🤰 <b>Pregnancy:</b> '
                        f'Caution with {_esc(names_p)}. '
                        f'Tetracyclines/TMP-SMX: depend on trimester. Physician decision.</div>')
    if interactions:
        for ia in interactions[:3]:
            alerts_html += f'<div style="font-size:8pt;margin:0.5mm 0">💊 {_esc(ia)}</div>'
    if alerts_html:
        H.append(f'<div class="section no-break">'
                 f'<div class="section-title">⚠ Alerts</div>'
                 f'<div class="alert alert-warn">{alerts_html}</div>'
                 f'</div>')

    # ── 2-column: Pathogenicity + Duration ─────────────────────────────
    H.append('<div class="grid2">')
    H.append('<div class="g2-l">')
    if patho_assessment:
        ...
        H.append(f'<div class="score-bar-outer"><div class="score-bar-inner" style="width:{sc}%;background:{clr2}"></div></div>')
        H.append(f'<div style="font-size:9pt;margin:1mm 0"><b style="color:{clr2}">{sc}%</b> — {verd}</div>')
        for fl, msg in [("ABU_NO_TREAT","ABU — Do NOT Treat"),
                        ("MW_REJECT","Specimen REJECTED — Repeat"),
                        ("SIRS_HIGH","SIRS ≥3 — Sepsis Probable"),
                        ("PEDIATRIC_UTI","Pediatric threshold applied")]:
            if fl in flags:
                H.append(f'<div class="alert alert-warn" style="font-size:7.5pt;margin:0.5mm 0">{msg}</div>')
    H.append('</div><div class="g2-r">')
    if duration_data:
        d = duration_data
        ...
        H.append(f'<table class="info-table" style="font-size:8pt">'
                 ...
                 f'</table>')
        if d.get("notes"):
            H.append(f'<div class="compact-note" style="margin-top:1mm">📋 {_esc(d["notes"][:120])}</div>')
        if d.get("follow_up_culture"):
            H.append('<div class="alert alert-warn" style="font-size:7.5pt;margin-top:1mm">🔄 Follow-up culture recommended</div>')
    H.append('</div></div>')

    # ── Combo (MDR) — compact ──────────────────────────────────────────
    if combo_suggestions:
        H.append('<div class="section no-break"><div class="section-title">Combination Therapy — MDR</div>')
        for cs in combo_suggestions[:2]:
            data = cs["data"]
            H.append(f'<div class="alert alert-danger" style="font-size:8pt;margin-bottom:1mm">'
            ...
            for opt in data["options"][:3]:
                ...
                icon  = "🚫" if avoid else "•"
                clr3  = "#922b21" if avoid else "#1a1a2e"
                H.append(f'<div style="font-size:8pt;margin:0.5mm 0;color:{clr3}">'
                         f'{icon} <b>{_esc(opt["combo"])}</b>'
                         f' <span style="color:#888">({_esc(opt["evidence"])})</span></div>')
        H.append('</div>')

    # ── Hepatic (only if impaired) ─────────────────────────────────────
    if is_hepatic and hepatic_recs:
        action_recs = [r for r in hepatic_recs if r.get("requires_action")][:4]
        if action_recs:
            H.append(f'<div class="section no-break"><div class="section-title">Hepatic Dosing — CP-{_esc(child_pugh)}</div>')
            for r in action_recs:
                cls4 = "danger-val" if "Avoid" in r["level"] else "warn-val"
                H.append(f'<div style="font-size:8pt;margin:0.5mm 0">'
                         f'<b class="{cls4}">{_esc(r["name"])}</b>: {_esc(r["recommendation"])}</div>')
            H.append('</div>')

    # ── Footer ─────────────────────────────────────────────────────────
    H.append(f"""<hr class="divider">
<div class="grid2">
  <div class="g2-l" style="font-size:7pt;color:#666">
    <b>References:</b> CLSI 2026 | EUCAST 2026 | IDSA AMR 2025 | WHO AWaRe 2025 | Sanford 2025 | BNF 2025 | Egypt Nat. Guidelines
  </div>
  <div class="g2-r" style="font-size:7pt;color:#666">
    <b>Disclaimer:</b> Clinical decision support only. Treatment decisions remain the responsibility of the treating physician.
  </div>
</div>
<div style="text-align:center;font-size:7pt;color:#aaa;margin-top:1mm">
