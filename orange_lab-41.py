...
if _page == "📇 سجل العزلات":
    try:
        ...
        st.caption("ابعت نص الخطأ ده عشان نظبّطه بدقة.")
    st.stop()
...
if uploaded:
    ...
    # ── In-page clickable step navigation (always visible, works on mobile) ──
    # Three buttons across the top. Click any to jump to that step. The current
    # step is highlighted (primary + disabled so you can't "navigate to self").
    # Results is locked until an AST value exists. Being in-page (not sidebar)
    # guarantees the controls are always reachable, including on mobile.
