...
_up_tab, _cam_tab = st.tabs(["📁 رفع صورة / Upload", "📷 كاميرا / Camera"])
with _up_tab:
    uploaded = st.file_uploader(
        "📷 Upload Culture Report Image",
        type=["jpg", "jpeg", "png", "heic", "heif", "webp", "bmp"],
        help="من الموبايل: صور الأيفون/الأندرويد (HEIC) مدعومة. لو الصورة مترفضتش، جرّب تبويب الكاميرا.",
    )
with _cam_tab:
    _cam_shot = st.camera_input("صوّر تقرير المزرعة مباشرة")
    if _cam_shot is not None:
        uploaded = _cam_shot   # camera returns a PNG that always decodes
...
if uploaded and not HEIF_AVAILABLE:
    ...
    if _up_name.endswith((".heic", ".heif")):
        st.warning(
            ...
            "أضِف سطر `pillow-heif` في ملف requirements.txt، أو استخدم تبويب الكاميرا، "
            "أو غيّر إعداد الكاميرا في موبايلك لـ 'Most Compatible / JPEG'."
        )
