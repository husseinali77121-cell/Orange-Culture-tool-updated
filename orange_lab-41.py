...
# Default CLSI: this lab reads its zones against CLSI tables, so a "I" on its
# reports is CLSI Intermediate. The standard the APP interprets with must match
# the standard the BENCH generated the result with — reading a CLSI "I" with
# EUCAST semantics ("susceptible at increased exposure") would promote a genuinely
# uncertain result into a recommendation.
INTERP_STD = str(_secret("interpretation_standard", "CLSI")).strip().upper()
if INTERP_STD not in ("EUCAST", "CLSI"):
    # Fall back to CLSI, matching the default above and the reasoning in this
    # comment block. The old fallback was EUCAST, which meant a typo in the
    # secret ("clsi ", "CLSI M100", …) silently flipped the app into reading this
    # lab's CLSI "I" as EUCAST "susceptible, increased exposure" — the exact
    # misread this block exists to prevent. An invalid value must degrade to the
    # standard the bench actually uses, never to the other one.
    logger.warning("interpretation_standard=%r is not EUCAST/CLSI — falling back "
                   "to CLSI (the standard this lab reads its zones against).",
                   _secret("interpretation_standard"))
    INTERP_STD = "CLSI"
...
uploaded = st.file_uploader(
    ...
    # WHY THIS LIST IS LONGER THAN jpg/jpeg/png. The old filter was exactly
    # ["jpg","jpeg","png"], and st.file_uploader enforces `type` IN THE BROWSER:
    # a file whose extension is not listed is refused before any upload starts,
    # so on a phone the picker either greys the photo out or the tap does
    # nothing — no request reaches the server, no spinner, no error. That is
    # precisely the "picks a file and nothing happens" report, and it is why the
    # diagnostic page's unfiltered uploader received the same photo the main one
    # rejected. Modern phones do not hand back jpg by default:
    #   • iPhone + recent Samsung/Xiaomi save photos as HEIC/HEIF,
    #   • sharing through Google Photos / WhatsApp yields WEBP,
    #   • some Android browsers present JPEGs as .jfif,
    #   • picking from Files/Drive can arrive extension-less as image/*.
    # Listing these lets the file through; whether we can DECODE it is a separate
    # question handled below (extract_all_data → preprocess_image), where an
    # undecodable file gets a clear message instead of a silent no-op.
    type=["jpg", "jpeg", "jpe", "jfif", "png", "webp", "bmp", "tif", "tiff",
          "heic", "heif"],
)

# Say so up front. ensure_ocr_dependencies() now raises OCRUnavailable from
# inside extract_all_data() instead of calling st.error()+st.stop() itself —
# correct, since a pure module has no business deciding how the UI reports a
# failure, but it means the app has to do the telling. Without this the user
# picks a file, waits through the spinner, and gets a failure at the end for a
# reason that was knowable before they started.
if not OCR_AVAILABLE:
    st.error(
        "⚠️ مكتبات الـ OCR غير متاحة على هذا السيرفر — التحليل التلقائي للصور "
        f"متوقف.\n\n`{OCR_IMPORT_ERROR}`\n\n"
        "تأكد من وجود `opencv-python-headless` · `pytesseract` · `Pillow` في "
        "`requirements.txt`، و `tesseract-ocr` في `packages.txt`."
    )
    ...
    if is_new:
        ...
        with st.spinner("🔍 جاري تحليل صورة التقرير..."):
            try:
                ...
                # A missing dependency is a deployment problem, not a bad photo.
                # Telling the user "تعذر تحليل الصورة" sends them off to retake a
                # perfectly good picture.
                st.error(f"⚠️ محرك الـ OCR غير متاح على السيرفر: {e}")
                st.stop()
        ...
        if allowed:
            ...
            with st.expander("🏆 Smart Antibiotic Ranking", expanded=False):
                ...
                for i, item in enumerate(ranked[:8], 1):
                    ...
                    elif item.get("sir_note") == "use_with_caution":
                        st.caption("　　⚠️ **Intermediate — استخدم بحذر** (CLSI)")
        ...
        if banned:
            with st.expander("🚫 Contraindicated / Ineffective", expanded=True):
                cat_labels = {
                    ...
                    "intrinsic": "مقاومة جوهرية",
                    "renal":     "قصور كلوي",
                    "pregnancy": "ممنوع في الحمل",
                    "child":     "غير مناسب للعمر",
                    "organism":  "غير فعال للجرثومة",
                    "specimen":  "غير مناسب لهذه العينة",
                    "unknown":   "⚠️ غير معروف — راجع الاسم",
                    "other":     "موانع أخرى",
                }
                # Intrinsic resistance is split out. Printed in the same list and
                # the same style as acquired resistance, it says something false
                # by implication: Tetracycline-R is a fact about THIS isolate
                # (it picked up a tet gene), while Clindamycin-R is true of every
                # E. coli that has ever existed. Together they read as "this
                # organism is resistant to a lot" — half of which was never a
                # treatment option to lose. Only the first list belongs to the
                # patient; the second belongs to the panel design.
                _intrinsic = [i for i in banned if i.get("category") == "intrinsic"]
                _acquired  = [i for i in banned if i.get("category") != "intrinsic"]

                for item in _acquired:
                    st.error(
                ...
                if _intrinsic:
                    st.markdown("---")
                    st.caption(
                        "**🧬 مقاومة جوهرية — ليست نتائج مزرعة**  \n"
                        "المضادات دي الجرثومة مقاومة ليها بطبيعتها، مش باكتساب آلية "
                        "مقاومة. **مكانتش خياراً علاجياً في أي وقت**، ووجودها هنا "
                        "لا يعني أن العزلة أكثر مقاومة — يعني أن اللوحة تحتاج مراجعة."
                    )
                    for item in _intrinsic:
                        st.warning(
                            f"🧬 **{item['name']}** — مقاومة جوهرية  \n"
            ...
            with st.expander("🟡 Warnings / Dose Adjustment Required", expanded=True):
                ...
                if _interm:
                    ...
                    if INTERP_STD == "EUCAST":
                        ...
                        st.warning(
                            f"⚠️ **USE WITH CAUTION — Intermediate ({INTERP_LABEL})**  \n"
                            f"**{_names}**"
                        )
                        st.markdown(
                            """<div style="border-right:3px solid #e67e22;
                            background:#fff8f0;padding:0.6rem 0.8rem;border-radius:6px;
                            font-size:0.9rem;line-height:1.9">
                            <b>التنبيه للطبيب:</b><br>
                            • معدّل الاستجابة <b>قد يكون أقل</b> من المضاد الحسّاس.<br>
                            • قد يكون فعّالاً في المواقع التي <b>يتركّز فيها الدواء</b>
                              (مثل البول للكينولونات والبيتا-لاكتام)، أو
                              <b>بجرعة أعلى</b> من المعتادة.<br>
                            • الفئة تتضمّن أيضاً <b>منطقة عازلة</b> ضد التباين التقني
                              البسيط، فقد تتغيّر النتيجة عند الإعادة.<br>
                            • <b>يُفضَّل بديل حسّاس (S) إن وُجد.</b>
                            </div>""",
                            unsafe_allow_html=True,
                        )
                        st.caption(f"المرجع: CLSI {CLSI_VER} — تعريف فئة Intermediate.")
                for item in _others:
        ...
        if final_drugs:
            ...
            if WEASYPRINT_AVAILABLE:
                ...
                _pdf_sig = hashlib.md5((
                    ...
                    # INTERP_STD changes what "I" prints as, and the QC errors
                    # drive the banner — both must invalidate a cached PDF, or a
                    # corrected panel would still download with the old warning
                    # (or without a new one).
                    f"|{INTERP_STD}"
                    f"|{','.join(sorted(q.get('id','') for q in qc_issues if q.get('severity')=='error'))}"
                ).encode()).hexdigest()[:16]
                ...
                # The QC banner was removed from the physician's PDF (see
                # reports.py): a lab does not publish a panel it does not trust
                # and annotate its way out. But the signal cannot just vanish —
                # it moves HERE, to the person who can still do something about
                # it, before the report exists. Not a blocker: the microbiologist
                # decides, and there are legitimate reasons to release anyway.
                _qc_err_now = [q for q in qc_issues if q.get("severity") == "error"]
                if _qc_err_now:
                    _names_err = ", ".join(
                        ", ".join(q.get("drugs") or []) for q in _qc_err_now[:3]
                    ) or "—"
                    st.error(
                        f"🔴 **{len(_qc_err_now)} خطأ في اللوحة لم يُحَل** — {_names_err}  \n"
                        "التقرير ده هيروح للطبيب **من غير أي تحذير عليه**. "
                        "الأفضل تعيد الاختبار الأول، أو تصحّح النتيجة في الخطوة الثانية."
                    )
                    with st.expander("🔍 تفاصيل الأخطاء"):
                        for _q in _qc_err_now:
                            st.markdown(f"- {_q.get('message','')}")
                            if _q.get("fix"):
                                st.caption(_q["fix"])
                    ...
                    with st.spinner("Generating PDF report..."):
                        try:
                            _new_pdf = generate_pdf_html_report(
                                ...
                                # The standard the bench used, so the PDF prints
                                # the right meaning of "I" and says which table
                                # it was interpreted against (ISO 15189 needs
                                # that traceable from the report itself).
                                interp_std=INTERP_STD,
