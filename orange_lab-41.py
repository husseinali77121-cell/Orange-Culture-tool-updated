...
# HEIC/HEIF support (iPhone & modern Android default photo format). Optional:
# if pillow-heif isn't installed, JPEG/PNG/WEBP/BMP still work and HEIC uploads
# get a clear message. Add `pillow-heif` to requirements.txt to enable HEIC.
try:
...
# ── Modernist / Orange Lab UI skin ────────────────────────────────────────
# Supersedes the old inline <style> block. Must run AFTER st.set_page_config().
# Base palette + font live in .streamlit/config.toml; the full skin (borders,
# dark sidebar, S/I/R chips, tabs, metrics, chrome-hiding) is applied here.
# Wrapped defensively: the theme is purely cosmetic and must never be able to
# break the clinical app if orange_theme.py is missing.
try:
    from orange_theme import inject_theme
    inject_theme()
except Exception as _theme_err:
    logger.warning("UI theme not applied (orange_theme.py issue): %s", _theme_err)
...
def normalize_upload_bytes(raw: bytes) -> bytes:
    """Convert ANY supported upload to standard, upright JPEG bytes.

    Phone photos are usually HEIC/HEIF (iPhone & modern Android) or WEBP, which
    OpenCV can't decode and browsers can't preview. Pillow decodes them (HEIC
    via pillow-heif), fixes EXIF rotation, and re-encodes to JPEG so both the
    preview (st.image) and the OCR (OpenCV) work. If Pillow can't read it, the
    raw bytes are returned unchanged so a normal JPEG/PNG still flows through.
    """
    if Image is None:
        return raw
    try:
        ...
        try:                       # honor phone orientation (photos come rotated)
        ...
        logger.warning("Image normalize failed (%s) — passing raw bytes through.", exc)
        return raw
...
def detect_rbcs(text: str) -> str:
    ...
    for pat in patterns:
        ...
        if m:
            return m.group(1).strip()
    # Handle text qualifiers for RBCs — only when TNTC sits on the RBC/red-cell
    # line. The common layout is "Pus: n / RBCs: TNTC", so the old
    # "no 'pus' before 'tntc'" test wrongly suppressed a real RBC TNTC.
    for _line in text_l.splitlines():
        if (re.search(r"rbc|red\s*blood|erythrocyt|كريات\s*حمراء", _line)
                and re.search(r"tntc|too\s+numerous|innumerable|uncountable", _line)):
            return "TNTC"
...
# ── Sidebar navigation ─────────────────────────────────────────────────────
# Primary workflow = a 3-step flow (Patient → AST → Results). Registry and
# Antibiogram are separate database tools. All live in the sidebar so they're
# always reachable (incl. mobile). Buttons (not a radio) are used so the active
# highlight always reflects the true state, even when steps advance from inside
# the flow (the "Next" buttons).
st.session_state.setdefault("nav_page", "analysis")   # analysis | registry | antibiogram

# Current workflow step (persisted in session_state → safe to read this early).
_side_step = 3 if st.session_state.get("show_results", False) \
    else st.session_state.get("wizard_step", 1)
_n_ast_side = len(st.session_state.get("sir_map_edited", {}) or {})
_on_analysis = st.session_state.nav_page == "analysis"

with st.sidebar:
    st.markdown("<div class='ol-nav-head'>WORKFLOW</div>", unsafe_allow_html=True)
    _wf_steps = [
        (1, "①  Patient & Specimen", True),
        (2, "②  AST / Sensitivity",  True),
        (3, "③  Results & Report",   _n_ast_side > 0),
    ]
    for _si, _slabel, _sen in _wf_steps:
        _here = _on_analysis and (_side_step == _si)
        if st.button(_slabel, key=f"nav_step_{_si}", use_container_width=True,
                     type=("primary" if _here else "secondary"),
                     disabled=(_here or not _sen),
                     help=(None if _sen else "Enter at least one AST result first")):
            st.session_state.nav_page = "analysis"
            if _si == 3:
                st.session_state.show_results = True
            else:
                st.session_state.show_results = False
                st.session_state.wizard_step = _si
            st.rerun()

    st.markdown("<div class='ol-nav-head'>DATABASE</div>", unsafe_allow_html=True)
    if st.button("📇  Isolate Registry", key="nav_registry", use_container_width=True,
                 type=("primary" if st.session_state.nav_page == "registry" else "secondary"),
                 disabled=(st.session_state.nav_page == "registry")):
        st.session_state.nav_page = "registry"
        st.rerun()
    if st.button("📊  Antibiogram", key="nav_antibiogram", use_container_width=True,
                 type=("primary" if st.session_state.nav_page == "antibiogram" else "secondary"),
                 disabled=(st.session_state.nav_page == "antibiogram")):
        st.session_state.nav_page = "antibiogram"
        st.rerun()

    st.divider()
    st.caption(f"📇 Saved isolates: {REGISTRY.count()}")
    if not _gh_registry_config():
        st.caption("⚠️ GitHub sync off — local save only.")

# ── Page routing (tools render then st.stop(); analysis flow is the default) ─
if st.session_state.nav_page == "registry":
    try:
        render_registry_page(REGISTRY)
        if st.button("🔄 Sync registry with GitHub", key="reg_sync_btn"):
            if _registry_push():
                st.success("✅ Synced with GitHub.")
            else:
                st.warning("GitHub sync is off or failed — check your secrets.")
        ...
        st.error(f"⚠️ Registry page error — {type(_reg_err).__name__}: {_reg_err}")
        st.caption("Send me this exact text so I can pinpoint the fix.")
    st.stop()

if st.session_state.nav_page == "antibiogram":
    try:
        render_antibiogram_page(REGISTRY)
    except Exception as _abx_err:
        logger.exception("Antibiogram page failed: %s", _abx_err)
        st.error(f"⚠️ Antibiogram page error — {type(_abx_err).__name__}: {_abx_err}")
        st.caption("Send me this exact text so I can pinpoint the fix.")
    st.stop()
...
uploaded = st.file_uploader(
    ...
    type=["jpg", "jpeg", "png", "heic", "heif", "webp", "bmp"],
    help="من الموبايل: صور JPG/PNG وصور الأيفون (HEIC) مدعومة.",
)

# Camera is fully OPT-IN — it is NOT rendered unless the user ticks this box, so
# it can never interfere with the normal file uploader (which is the default).
_use_cam = st.checkbox("📷 أو صوّر التقرير بالكاميرا مباشرة", value=False)
if _use_cam:
    _cam_shot = st.camera_input("صوّر تقرير المزرعة")
    if _cam_shot is not None:
        uploaded = _cam_shot

if uploaded and not HEIF_AVAILABLE:
    _up_name = (getattr(uploaded, "name", "") or "").lower()
    if _up_name.endswith((".heic", ".heif")):
        st.warning(
            "الصورة بصيغة HEIC ومكتبة التحويل (pillow-heif) مش متثبّتة على السيرفر. "
            "أضِف سطر `pillow-heif` في requirements.txt، أو استخدم الكاميرا، "
            "أو غيّر إعداد كاميرا موبايلك لـ 'Most Compatible / JPEG'."
        )

if uploaded:
    file_bytes = normalize_upload_bytes(uploaded.getvalue())
    ...
    with _prev_l:
        st.image(file_bytes, caption="Preview (مصغّر)", width=260)
    with _prev_r:
    ...
    # ── Step navigation now lives in the sidebar (WORKFLOW). Here we show a
    # lightweight, non-clickable progress breadcrumb so the current step is
    # always obvious inside the main area. ──────────────────────────────────
    _crumbs = ["Patient & Specimen", "AST / Sensitivity", "Results & Report"]
    _marks = "①②③"
    _crumb_html = "  ›  ".join(
        (f"<b class='ol-crumb-active'>{_marks[i]} {c}</b>" if (i + 1) == _cur_step
         else (f"<span class='ol-crumb-done'>✓ {c}</span>" if (i + 1) < _cur_step
               else f"<span class='ol-crumb-todo'>{_marks[i]} {c}</span>"))
        for i, c in enumerate(_crumbs)
    )
    st.markdown(f"<div class='ol-crumbs'>{_crumb_html}</div>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        with _basic_host:
            ...
            # Pus/RBC microscopy is meaningful for urine (pyuria/haematuria) and
            # for wound/pus specimens; hidden elsewhere.
            _is_wound = _spec_cat == "wound"
            ...
            if payload and not st.session_state.get(_ocr_done_key, False):
                ...
                _filled  = []
                # Pus/RBC microscopy only makes sense for urine and wound/pus —
                # never seed it into a blood/CSF/sputum report.
                _allow_micro = _is_urine or _is_wound
            ...
            with _c_wt:
                ...
                weight = st.number_input("Weight (kg)", min_value=1.0, max_value=300.0,
                                         value=float(_wt_default), step=0.5,
            ...
            if is_renal:
                ...
                if age < 18:
                    ...
                    _default_h = float(min(180, max(45, 6 * age + 77))) if age >= 1 else 55.0
        ...
        with _adv_host:
            ...
            try:
                ...
                # Don't break the main flow — but never swallow silently. This
                # file's logging setup exists precisely to replace silent except.
  ...
  ">🖨️ فتح للطباعة (Ctrl+P)</a>"""
                        st.markdown(print_html.format(b64=b64), unsafe_allow_html=True)
