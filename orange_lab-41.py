...
def init_session_state() -> None:
    defaults = {
        ...
        "patho_wound_type":       "",
        "patho_result":           None,
        # ─── Wizard: two-step flow (Input → Results) ───────────────────────
        "show_results":            False,  # False = editing inputs, True = viewing results
        # ─── Clinical Engines v4 ──────────────────────────────────────────
...
if uploaded:
    ...
    if is_new:
        ...
        with st.spinner("🔍 جاري تحليل صورة التقرير..."):
            try:
                ...
                # صورة جديدة → ابدأ من خطوة الإدخال (المعالج)
    ...
    # ── Wizard stepper: two clear phases (Input → Results) ───────────────────
    # A visual progress bar so the user always knows which phase they are in, and
    # a toggle to move between editing inputs and viewing the final report.
    _step2 = st.session_state.get("show_results", False)
    _s_active = "background:#1a6b3a;color:#fff"
    _s_done   = "background:#cfe8d8;color:#1a6b3a"
    _s_idle   = "background:#eef2f5;color:#7a8a99"
    _st1 = _s_done if _step2 else _s_active
    _st2 = _s_active if _step2 else _s_idle
    st.markdown(
        "<div style='display:flex;gap:8px;align-items:center;margin:2px 0 10px'>"
        f"<span style='{_st1};padding:5px 14px;border-radius:16px;font-weight:600;"
        f"font-size:0.9em'>① الإدخال</span>"
        "<span style='flex:0 0 24px;height:2px;background:#cfd8df'></span>"
        f"<span style='{_st2};padding:5px 14px;border-radius:16px;font-weight:600;"
        f"font-size:0.9em'>② النتائج والتقرير</span>"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    with col1:
        ...
        # Colony count is a urine-specific quantitative threshold (CFU/mL). For
        # other specimens it is not interpreted, so only surface it for urine.
        if _is_urine:
    ...
    with col2:
        ...
        # ══════════════════════════════════════════════════════════════
        # WIZARD GATE — separates Step ① (input) from Step ② (results)
        # ══════════════════════════════════════════════════════════════
        # In the input phase we show a prominent "view results" button and STOP
        # before any result is computed/rendered, so the user focuses on entering
        # data. In the results phase we show a "back to editing" button and let
        # the analysis below run. All inputs persist in session_state, so moving
        # between phases never loses anything.
        if not st.session_state.get("show_results", False):
            st.divider()
            if not final_drugs:
                st.info("➕ أدخل نتائج الحساسية (AST) بالأعلى، ثم اضغط لعرض التحليل.")
            _go = st.button(
                "▶️ عرض التحليل والتوصيات والتقرير",
            ...
            _cba, _cbb = st.columns([1, 1])
            if _cba.button("◀️ العودة لتعديل المدخلات", use_container_width=True,
                           key="wizard_back_edit"):
                st.session_state.show_results = False
                st.rerun()
            _cbb.caption("📄 النتائج والتقرير بالأسفل")
            st.divider()
