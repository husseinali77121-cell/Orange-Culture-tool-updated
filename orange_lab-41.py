...
if uploaded:
    ...
    def _chip(idx, label):
        if idx < _cur_step:
            style = "background:#cfe8d8;color:#1a6b3a"      # done
        elif idx == _cur_step:
            style = "background:#1a6b3a;color:#fff"          # active
        else:
            style = "background:#eef2f5;color:#7a8a99"       # idle
        return (f"<span style='{style};padding:5px 12px;border-radius:16px;"
                f"font-weight:600;font-size:0.85em'>{label}</span>")

    _sep = "<span style='flex:0 0 18px;height:2px;background:#cfd8df'></span>"
    st.markdown(
        "<div style='display:flex;gap:6px;align-items:center;margin:2px 0 12px;"
        "flex-wrap:wrap'>"
        f"{_chip(1, '① المريض والمزرعة')}{_sep}"
        f"{_chip(2, '② الحساسية (AST)')}{_sep}"
        f"{_chip(3, '③ النتائج والتقرير')}"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    if _cur_step == 1:
        st.divider()
        _n1a, _n1b = st.columns([1, 1])
        if _n1a.button("التالي: إدخال الحساسية ▶️", type="primary",
                       use_container_width=True, key="wiz_to_ast"):
            st.session_state.wizard_step = 2
            st.rerun()
        _n1b.caption("① أكمل بيانات المريض والمزرعة، ثم انتقل للحساسية.")
        st.stop()
    ...
    with col2:
        ...
        if not st.session_state.get("show_results", False):
            ...
            _g2a, _g2b = st.columns([1, 1.3])
            if _g2a.button("◀️ رجوع: المريض والمزرعة", use_container_width=True,
                           key="wizard_back_patient"):
                st.session_state.wizard_step = 1
                st.rerun()
            _go = _g2b.button(
            ...
            if _cba.button("◀️ العودة لتعديل الإدخال", use_container_width=True,
                           key="wizard_back_edit"):
                # go back to the AST step (step 2) — inputs are all preserved
                st.session_state.show_results = False
                st.session_state.wizard_step = 2
                st.rerun()
