...
# اختياري: يخلّي الرفع يقبل HEIC/HEIF مباشرة من كاميرا الموبايل (iPhone وبعض
# أجهزة Android الحديثة). لو المكتبة مش متثبتة، الملف بيفشل برسالة واضحة بدل
# الكراش — ضِف `pillow-heif` في requirements.txt لتفعيله.
try:
...
# أقصى حجم مقبول للملف المرفوع (Streamlit نفسه بيقف عند 200MB افتراضياً).
_MAX_UPLOAD_BYTES = 40 * 1024 * 1024


@st.cache_data(show_spinner=False, max_entries=3)
def _normalize_upload_bytes(raw: bytes, filename: str = "") -> bytes:
    """حوّل أي ملف جاي من مُنتقي ملفات الموبايل إلى JPEG (RGB) معتدل الاتجاه.

    ده سبب مشكلة «الصورة مش بترفع من الموبايل»: مُنتقي Android/iOS بيرجّع
    HEIC/HEIF أو WEBP أو PNG بقناة شفافية أو JPEG بنظام CMYK، وكمان بيرجّع صور
    بورتريه متدوّرة عن طريق وسم EXIF بس. و cv2.imdecode():
      • بيرجّع None لأغلب الصيغ دي  → "تعذر قراءة الصورة"
      • وبيتجاهل EXIF في الباقي     → الصورة بتتقري مقلوبة → OCR بيطلع فاضي
    بنعيد الترميز مرة واحدة هنا، فالمعاينة والـ hash والـ OCR كلهم بيشوفوا نفس
    الصورة السليمة المعتدلة.
    """
    if not raw:
        raise ValueError("الملف فارغ — أعد اختيار الصورة.")
    if len(raw) > _MAX_UPLOAD_BYTES:
        raise ValueError(
            f"حجم الملف كبير جداً ({len(raw)/1048576:.1f} MB). "
            f"الحد الأقصى {_MAX_UPLOAD_BYTES//1048576} MB — صوّر بدقة أقل."
        )
    if not PIL_AVAILABLE:
        # من غير Pillow منقدرش نعيد الترميز — سلّم الملف كما هو لـ cv2.
        return raw
    ...
    try:
        ...
        _hint = ""
        if _ext in ("heic", "heif") and not HEIF_SUPPORT:
            _hint = ("\n\nالملف بصيغة HEIC من كاميرا الموبايل. غيّر إعداد الكاميرا "
                     "إلى «الأكثر توافقاً / JPEG»، أو ضِف `pillow-heif` في "
                     "requirements.txt.")
        elif _ext in ("pdf", "doc", "docx", "mp4", "mov"):
            _hint = f"\n\nده مش ملف صورة ({_ext.upper()}). ارفع JPG أو PNG."
        raise ValueError(f"تعذر فتح الملف كصورة: {exc}{_hint}")

    # EXIF: صور الموبايل البورتريه بتتخزّن أفقي + وسم دوران. لازم نطبّق الدوران
    # فعلياً قبل ما نشيل الـ EXIF بإعادة الترميز، وإلا هتتقري مقلوبة.
    try:
        im = ImageOps.exif_transpose(im) or im
    except Exception as exc:
        logger.debug("exif_transpose failed: %s", exc)

    if im.mode != "RGB":
        im = im.convert("RGB")

    buf = io.BytesIO()
    im.save(buf, format="JPEG", quality=92, optimize=False)
    return buf.getvalue()
...
def extract_detected_drugs(full_text: str) -> List[str]:
    ...
    for abx_name, _info in ABX_GUIDELINES.items():
        if len(abx_name) >= 5:
            _p = text_lower.find(abx_name.lower())
            if _p >= 0:
                _note(abx_name, _p)
        # Also check aliases from the drug's own aliases list
        for alias in _info.get("aliases", []):
            if len(alias) >= 4:
                _p = text_lower.find(alias.lower())
                if _p >= 0:
                    _note(abx_name, _p)
...
# بدون type=[...] عن قصد. الـ type بيتحوّل لـ accept="..." على وسم <input> في
# المتصفح، ومنتقي الملفات في Android (Google Photos / Drive / Files) بيسلّم
# الملف عن طريق content:// من غير امتداد أو بـ MIME مش مطابق، فالمتصفح بيرفضه
# أو بيظهر الصور رمادية مش قابلة للاختيار — وده سبب إن الرفع من الموبايل مش
# شغال. بنقبل أي ملف، وبنتحقق منه بنفسنا في _normalize_upload_bytes().
uploaded = st.file_uploader(
    "📷 Upload Culture Report Image",
    type=None,
    accept_multiple_files=False,
    key="culture_img_uploader",
    help="اختر صورة تقرير المزرعة (JPG / PNG / HEIC) — أو صوّرها بالكاميرا مباشرة",
)

if uploaded:
    try:
        file_bytes = _normalize_upload_bytes(uploaded.getvalue(),
                                             getattr(uploaded, "name", ""))
    except ValueError as _img_err:
        st.error(f"⚠️ {_img_err}")
        st.stop()
    except Exception as _img_err:
        logger.exception("Upload normalisation failed: %s", _img_err)
        st.error(f"⚠️ تعذر قراءة الملف المرفوع: {_img_err}")
        st.stop()

    # الـ hash محسوب على البايتات بعد التطبيع، فنفس الصورة = نفس الـ hash دايماً.
    ...
    if not st.session_state.sir_map_edited:
        if st.session_state.get(_ast_backup_key):
            st.session_state.sir_map_edited = dict(st.session_state[_ast_backup_key])
    ...
    with col1:
        ...
        with _basic_host:
            ...
            # and wound/pus (pus cells). It is not a standard input for stool, blood,
            # CSF or sputum — the field is hidden there.
            if _is_urine or _is_wound:
                ...
                # نفس منطق الـ Colony Count فوق: القيمة القديمة بتفضل محفوظة في
                # session_state (فلو رجع لعينة بول تاني هيلاقيها)، لكن منبعتش
                # حاجة للتحليل ولا للتقرير. من غير التصفير ده كانت قيمة Pus/RBCs
                # المتخزّنة من عينة بول سابقة بتتسرّب وتطبع في تقرير Stool C/S —
                # والـ Pus cells / RBCs مش من مكوّنات تقرير مزرعة البراز أصلاً
                # (بتتقاس في Stool Analysis منفصلة)، وكمان كانت بتدخل حساب
                # الـ Pathogenicity Score غلط.
                pus_cells_text = ""
                rbcs_text      = ""
            ...
            with _c_age:
                ...
                is_neonate = _infant and _age_months < 1   # ≈ < 28 يوم
