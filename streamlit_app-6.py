...
body { font-family: 'Amiri','Noto Naskh Arabic','DejaVu Sans',Arial,sans-serif;
       font-size: 8.5pt; color: #1a1a2e; direction: rtl; background: #fff; }
...
.hdr-right { font-size:7.5pt; opacity:0.9; text-align:left; direction:ltr; }
...
/* AST table */
.ast { width:100%; border-collapse:collapse; font-size:7.5pt; }
.ast th { background:#0d3b66; color:#fff; padding:1.5mm 3mm; text-align:right; font-size:7.5pt; }
.ast td { padding:1.2mm 3mm; border:0.3pt solid #d5d8dc; }
...
.grid2 { display:table; width:100%; border-spacing:2mm; border-collapse:separate; }
.g2l { display:table-cell; width:49%; vertical-align:top; }
.g2r { display:table-cell; width:49%; vertical-align:top; }
/* Ranked */
.ranked-row { padding:1.5mm 3mm; margin:0.8mm 0; border-radius:1.5mm;
...
/* Alerts */
.alert { padding:1.5mm 3mm; border-radius:1.5mm; margin:0.8mm 0; font-size:7.5pt; }
...
.compact-tbl { width:100%; border-collapse:collapse; font-size:7.5pt; }
.compact-tbl td { padding:1mm 3mm; border:0.3pt solid #d5d8dc; }
...
"""
    ...
    def hdr_html(page_lbl: str) -> str:
        ...
        if esbl_prob in ("high","moderate"): mdr_pills += pill("ESBL+","background:#b7770d;color:#fff")
  ...
  <div class="hdr-right">
    ...
    H = []
    H.append(f"<!DOCTYPE html><html lang='ar' dir='rtl'><head><meta charset='UTF-8'>"
             f"<style>{CSS}</style></head><body>")

    # ════════════════════════════════════════════════════════════════
    # PAGE 1: Patient + Culture + AST (compact, fits A4)
    # ════════════════════════════════════════════════════════════════
    H.append(hdr_html("PAGE 1 / 2"))

    # Patient + Culture: 4-column info table (8 cells, 2 rows)
    renal_v = f'<span class="warn-val">Impaired — CrCl:{cl_cr:.0f}</span>' if is_renal else "Normal"
    hep_v   = f'<span class="warn-val">Impaired CP-{child_pugh}</span>' if is_hepatic else "Normal"
    preg_v  = ('<span class="warn-val">YES</span>' if is_preg else "") if (sex=="Female" and age>=12) else "N/A"

    H.append(f"""<div class="sec-ttl">Patient &amp; Culture</div>
<table class="info4">
<tr>
  <td class="lbl4">Patient</td><td class="val4"><b>{_esc(patient_name or "—")}</b></td>
  <td class="lbl4">Specimen</td><td class="val4">{_esc(specimen)}</td>
  <td class="lbl4">Organism</td><td><b><i>{_esc(organism)}</i></b></td>
</tr>
<tr>
  <td class="lbl4">Age/Sex/Wt</td><td class="val4">{age}y | {_esc(sex)} | {weight}kg</td>
  <td class="lbl4">Colony</td><td class="val4 ltr">{_esc(colony_count or "—")}</td>
  <td class="lbl4">Pus/RBCs</td><td class="ltr">{_esc(pus_cells or "—")}/{_esc(rbcs or "—")}/HPF</td>
</tr>
<tr>
  <td class="lbl4">Renal</td><td class="val4">{renal_v}</td>
  <td class="lbl4">Hepatic</td><td class="val4">{"<span class='warn-val'>Impaired</span>" if is_hepatic else "Normal"}</td>
  <td class="lbl4">Pregnancy</td><td>{preg_v}</td>
</tr>
</table>""")
    ...
    if sir_map:
        ...
        for drug, sir in sorted(sir_map.items()):
            ...
            aw_html = pill(aw, AWARE_PILL.get(aw,"")) if aw else ""
            rows += (f'<tr><td style="font-size:7.5pt">{_esc(drug)}</td>'
                     f'<td class="{_sir_class(sir)} ltr" style="font-size:9pt;font-weight:bold">{sir}</td>'
                     f'<td style="font-size:7pt">{_sir_label(sir)}</td>'
                     f'<td>{aw_html}</td></tr>')
        H.append(f"""<div class="sec-ttl" style="margin-top:3mm">Antibiotic Sensitivity Test (AST)</div>
<table class="ast">
<thead><tr><th>Antibiotic</th><th>Result</th><th>Interpretation</th><th>WHO AWaRe</th></tr></thead>
<tbody>{rows}</tbody>
</table>""")

    H.append("</div>")  # close content

    # ════════════════════════════════════════════════════════════════
    # PAGE 2: Clinical Decision Support
    # ════════════════════════════════════════════════════════════════
    H.append('<div class="pb"></div>')
    H.append(hdr_html("PAGE 2 / 2 — CLINICAL DECISION SUPPORT"))

    # Ranked Therapy
    H.append('<div class="sec-ttl">Recommended Therapy — Ranked</div>')
    prev_tier = ""
    for i, d in enumerate(ranked[:9], 1):
        aw       = d.get("aware","")
        tier_lbl = TIER_LBL.get(aw, aw)
        clr      = AWARE_CLR.get(aw,"#444")
        card_css = AWARE_CARD.get(aw,"")
        ...
        if tier_lbl != prev_tier:
            H.append(f'<div class="tier-sep" style="color:{clr};border-color:{clr}">{tier_lbl}</div>')
            prev_tier = tier_lbl

        H.append(f"""<div class="ranked-row" style="{card_css};border-radius:1.5mm;padding:1.5mm 3mm;margin:0.5mm 0">
  <div style="flex:1">
    <b style="font-size:9pt;color:{clr}">{i}. {_esc(d.get("name",""))}</b>
    &ensp;<span class="ltr" style="background:#fff;border:0.4pt solid {clr};color:{clr};
      font-size:7pt;padding:0.2mm 2mm;border-radius:1mm">{sir_v}</span>
    &ensp;<span style="font-size:7pt;color:#555">{route}</span>
    {"&ensp;<small style='color:#b7770d'>⚠ "+renal_n+"</small>" if renal_n else ""}
    {"&ensp;<small style='color:#888'>Brands: "+brands+"</small>" if brands else ""}
  </div>
  <div>{pill(aw, AWARE_PILL.get(aw,""))}</div>
</div>""")
    ...
    if banned:
        ...
        rows_txt = " &nbsp;|&nbsp; ".join(
            f"<b>{_esc(k)}:</b> {_esc(', '.join(vs))}" for k,vs in ban_grp.items())
        H.append(f'<div class="sec-ttl" style="margin-top:2mm">🚫 Avoid</div>'
                 f'<div class="alert al-danger" style="font-size:7pt">{rows_txt}</div>'
                 f'<div style="font-size:6.5pt;color:#888">See AST table (Page 1) for full profile</div>')

    # Alerts: Renal + Pregnancy + Interactions (compact)
    alert_items = []
    if warned:
        for d in warned[:4]:
            r = d.get("reason_short","") or (d.get("renal_note","") if is_renal else "")
            if r: alert_items.append(f'⚠ <b>{_esc(d.get("name",""))}</b>: {_esc(r)}')
    if is_preg and preg_warn_items:
        names_p = ", ".join(d.get("name","") for d in preg_warn_items[:4])
        alert_items.append(f'🤰 <b>Pregnancy:</b> Caution with {_esc(names_p)}. '
                          f'Tetracyclines/TMP-SMX: depend on trimester. Physician decision.')
    if interactions:
        for ia in interactions[:2]:
            alert_items.append(f'💊 {_esc(ia)}')
    if alert_items:
        H.append('<div class="sec-ttl" style="margin-top:2mm">⚠ Alerts</div>'
                 '<div class="alert al-warn">'
                 + '<br>'.join(alert_items) + '</div>')
    ...
    if patho_assessment:
        ...
        H.append('<div class="sec-ttl">Pathogenicity</div>')
        ...
        H.append(f'<div style="font-size:8pt;margin:0.5mm 0">'
                 f'<b style="color:{clr2}">{sc}%</b> — {verd}</div>')
        flag_msgs = {"ABU_NO_TREAT":"ABU — Do NOT Treat","MW_REJECT":"Specimen REJECTED",
                     "SIRS_HIGH":"SIRS ≥3 — Sepsis","PEDIATRIC_UTI":"Pediatric threshold"}
        for fl, msg in flag_msgs.items():
            if fl in flags:
                H.append(f'<div class="alert al-warn" style="font-size:6.5pt">{msg}</div>')
    H.append('</div><div class="g2r">')
    if duration_data:
        d = duration_data
        ...
        H.append(f'<table class="compact-tbl">'
                 ...
                 f'<tr><td class="lbl">Standard</td><td><b>{std} days</b></td></tr>'
                 f'<tr><td class="lbl">Range</td><td>{d.get("min_days","?")}–{d.get("max_days","?")} d</td></tr>'
                 f'<tr><td class="lbl">IV/PO</td><td class="ltr">IV:{d.get("iv_days",0)}d · PO:{d.get("po_days",0)}d</td></tr>'
                 f'</table>')
        if d.get("notes"):
            H.append(f'<div style="font-size:6.5pt;color:#555;margin-top:0.5mm">'
                     f'📋 {_esc(d["notes"][:130])}</div>')
        if d.get("follow_up_culture"):
            H.append('<div class="alert al-warn" style="font-size:6.5pt">🔄 Follow-up culture recommended</div>')
    H.append('</div></div>')
