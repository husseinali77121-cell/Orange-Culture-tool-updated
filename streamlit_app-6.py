...
def _inject_cephradine():
    if "Cephradine" not in ABX_GUIDELINES:
        # ── Dynamic validation: only use specimens/organisms known to the app ──
        _valid_specs = set(list(SPECIMEN_ORDER or DEFAULT_SPECIMENS))
        _valid_orgs  = set(ORGANISM_PROFILE.keys())

        # Build specimen_notes using ONLY keys that exist in SPECIMEN_ORDER
        _wound_note = "Good Gram+ coverage for mild-moderate wound infections."
        _spec_notes: Dict[str, str] = {}
        _spec_notes["Urine"]  = "Adequate for lower UTI (uncomplicated cystitis)."
        _spec_notes["Sputum"] = "Limited Gram- coverage; not first choice for pneumonia."
        for _ws in ["Wound Swab", "Wound & Pus", "Wound Swab/Pus", "Pus", "Abscess"]:
            if _ws in _valid_specs:
                _spec_notes[_ws] = _wound_note

        # Build organisms using ONLY organisms in ORGANISM_PROFILE
        _all_ceph_orgs = [
            # Only organisms that commonly appear in standard ORGANISM_PROFILE
        ...
        _valid_ceph_orgs = [o for o in _all_ceph_orgs if o in _valid_orgs]
        # Always keep at least S. aureus if nothing matched
        if not _valid_ceph_orgs and "Staphylococcus aureus" in _valid_orgs:
            _valid_ceph_orgs = ["Staphylococcus aureus"]
        ...
        ABX_GUIDELINES["Cephradine"] = {
            ...
            "organisms":       _valid_ceph_orgs,
            "specimen_notes":  _spec_notes,
        }
...
def get_treatment_duration(
    ...
    key = None
    if "urine" in spec:
        if any(k in synd for k in ["pyelonephritis", "upper", "kidney"]):
            key = "Pyelonephritis_inpatient" if severity == "severe" else "Pyelonephritis_outpatient"
        elif sex == "Female" and age < 65 and not is_renal:
    ...
    elif any(w in spec for w in ["abdomen", "periton", "stool"]):
...
HEPATIC_DOSING: Dict[str, Dict] = {
    ...
    "Clindamycin":     {"A": ("Normal","No adjustment"), "B": ("Caution","Use with caution; reduce 25-50%"), "C": ("Avoid","Avoid — accumulation risk"), "note": "Primary hepatic metabolism"},
    "Rifampicin":      {"A": ("Normal (no jaundice)","Normal if no jaundice"), "B": ("Max 8mg/kg/d","Max 8mg/kg/day; weekly LFTs"), "C": ("Avoid","Avoid — hepatotoxic + CYP inducer"), "note": "Hepatotoxic + strong CYP inducer"},
    "Erythromycin":    {"A": ("Normal","No adjustment"), "B": ("Reduce 25%","Reduce dose by 25%"), "C": ("Reduce 50%","Reduce 50% or avoid"), "note": "Cholestatic hepatitis risk"},
    ...
    "Moxifloxacin":    {"A": ("Normal","No adjustment"), "B": ("Caution","Monitor LFTs"), "C": ("Avoid","Avoid in severe hepatic failure"), "note": "Hepatic glucuronidation; QT risk increases"},
    ...
    "Amoxicillin-Clavulanate": {"A": ("Normal","No adjustment"), "B": ("Normal","No adjustment"), "C": ("Avoid","Avoid — Clavulanate-associated DILI risk"), "note": "Clavulanate linked to drug-induced liver injury"},
    ...
    "Tigecycline":     {"A": ("Normal","No adjustment"), "B": ("Normal","No adjustment"), "C": ("Reduce","100mg loading then 12.5mg q12h in Child-Pugh C"), "note": "Biliary excretion — adjust in severe impairment"},
    ...
    "Fluconazole":     {"A": ("Normal","No adjustment"), "B": ("Caution","Monitor LFTs closely"), "C": ("Caution/Reduce","Reduce dose; close monitoring"), "note": "Partial hepatic metabolism"},
    ...
    "Chloramphenicol": {"A": ("Caution","Use with caution"), "B": ("Avoid","Avoid"), "C": ("Avoid","Avoid — gray syndrome risk"), "note": "Hepatic glucuronidation — accumulates"},
    "Fusidic Acid":    {"A": ("Normal","No adjustment"), "B": ("Reduce","Reduce dose; monitor LFTs"), "C": ("Avoid","Avoid in severe failure"), "note": "Primarily hepatic elimination"},
}
...
# ═══════════════════════════════════════════════════════════════════════
# PDF REPORT ENGINE — English Only
# Design: Orange Lab A4 Clinical Report System
# Technology: ReportLab Platypus | Professional Medical Layout
# Arabic not supported in standard ReportLab — English only
# ═══════════════════════════════════════════════════════════════════════
# ═══════════════════════════════════════════════════════════════════════
# PDF REPORT ENGINE — English Only (Arabic not supported in standard ReportLab)
# Design System: Orange Lab Clinical Report
# ReportLab Platypus | A4 | Professional Medical Layout
# ═══════════════════════════════════════════════════════════════════════
...
def _find_arabic_font() -> Optional[str]:
    ...
    for p in candidates:
        if os.path.isfile(p):
            return p
    # Search recursively in common font dirs
    for font_dir in ["/usr/share/fonts", "/usr/local/share/fonts", os.path.expanduser("~/.fonts")]:
        if os.path.isdir(font_dir):
            for root, _, files in os.walk(font_dir):
                for f in files:
                    fl = f.lower()
                    if fl.endswith(".ttf") and any(k in fl for k in ["arabic","noto","amiri","cairo","naskh"]):
                        return os.path.join(root, f)
    return None

_ARABIC_FONT_PATH = _find_arabic_font()
...
def _register_arabic_font() -> bool:
    ...
    if _ARABIC_REGISTERED:
        return True
    if not REPORTLAB_AVAILABLE or not _ARABIC_FONT_PATH:
        return False
    try:
...
ARABIC_PDF_SUPPORTED = _ARABIC_LIB_OK and bool(_ARABIC_FONT_PATH)

def _ar(text: str, fallback: str = "") -> str:
    """
    Process Arabic text for proper PDF rendering.
    Returns reshaped+bidi text if Arabic PDF is supported,
    otherwise returns fallback (English) or empty string.
    """
    if not text:
        return fallback or ""
    # Check if text contains Arabic characters
    _has_arabic = any("\u0600" <= ch <= "\u06ff" for ch in text)
    if not _has_arabic:
        return text  # Pure English — return as-is
    if not _ARABIC_LIB_OK or not _ARABIC_REGISTERED:
        return fallback or ""
    try:
        reshaped  = arabic_reshaper.reshape(text)
        bidi_text = _bidi_display(reshaped)
        return bidi_text
    except Exception:
        return fallback or ""
...
_C = {
    # Brand
    "primary":    colors.HexColor("#0d3b66"),   # Dark navy
    "accent":     colors.HexColor("#ff8c00"),   # Orange Lab orange
    "accent_lt":  colors.HexColor("#fff3e0"),   # Light orange bg
    # Status
    "access":     colors.HexColor("#1e8449"),   # WHO Access — dark green
    "access_bg":  colors.HexColor("#eafaf1"),
    "watch":      colors.HexColor("#b7770d"),   # WHO Watch — amber
    "watch_bg":   colors.HexColor("#fef9e7"),
    "reserve":    colors.HexColor("#922b21"),   # WHO Reserve — dark red
    "reserve_bg": colors.HexColor("#fdedec"),
    "warn":       colors.HexColor("#6e2fa0"),   # Pregnancy warn — purple
    "warn_bg":    colors.HexColor("#f5eef8"),
    "banned_bg":  colors.HexColor("#fdf2f8"),
    # MDR
    "mdr_xdr":    colors.HexColor("#7b241c"),
    "mdr_bg":     colors.HexColor("#fef2f2"),
    # Neutral
...
def _styles():
    """Return custom paragraph style dict"""
    base = getSampleStyleSheet()
    def S(name, **kw):
        return ParagraphStyle(name, parent=base["Normal"], **kw)
    return {
        "title":      S("ot_title",   fontSize=18, textColor=_C["primary"],
                         fontName="Helvetica-Bold", spaceAfter=2, leading=22),
        "subtitle":   S("ot_sub",     fontSize=10, textColor=_C["muted"],
                         fontName="Helvetica", spaceAfter=0),
        "section":    S("ot_sec",     fontSize=9, textColor=_C["primary"],
                         fontName="Helvetica-Bold", spaceBefore=4, spaceAfter=2,
                         textTransform="uppercase", leading=12),
        "body":       S("ot_body",    fontSize=8, textColor=_C["text"],
                         fontName="Helvetica", leading=11, spaceAfter=1),
        "body_bold":  S("ot_bb",      fontSize=8, textColor=_C["text"],
                         fontName="Helvetica-Bold", leading=11),
        "small":      S("ot_sm",      fontSize=7, textColor=_C["muted"],
                         fontName="Helvetica", leading=9),
        "badge_acc":  S("ot_acc",     fontSize=8, textColor=_C["access"],
                         fontName="Helvetica-Bold", backColor=_C["access_bg"],
                         borderPadding=(2, 4, 2, 4)),
        "badge_wat":  S("ot_wat",     fontSize=8, textColor=_C["watch"],
                         fontName="Helvetica-Bold", backColor=_C["watch_bg"],
                         borderPadding=(2, 4, 2, 4)),
        "badge_res":  S("ot_res",     fontSize=8, textColor=_C["reserve"],
                         fontName="Helvetica-Bold", backColor=_C["reserve_bg"],
                         borderPadding=(2, 4, 2, 4)),
        "drug_name":  S("ot_dn",      fontSize=9, textColor=_C["primary"],
                         fontName="Helvetica-Bold", leading=12),
        "note_text":  S("ot_note",    fontSize=7, textColor=_C["muted"],
                         fontName="Helvetica-Oblique", leading=10),
        "warn":       S("ot_warn",    fontSize=8, textColor=_C["warn"],
                         fontName="Helvetica-Bold"),
        "danger":     S("ot_danger",  fontSize=8, textColor=_C["reserve"],
                         fontName="Helvetica-Bold"),
        "combo":      S("ot_combo",   fontSize=8, textColor=_C["primary"],
                         fontName="Helvetica-Bold", leading=11),
        "disclaimer": S("ot_disc",    fontSize=6.5, textColor=_C["muted"],
                         fontName="Helvetica-Oblique", leading=9),
        # Arabic styles (used when Arabic font is registered)
        "arabic_body": S("ot_ar_body",  fontSize=9, textColor=_C["text"],
                         fontName=_ARABIC_FONT_NAME if _ARABIC_REGISTERED else "Helvetica",
                         leading=14, alignment=TA_RIGHT),
        "arabic_small": S("ot_ar_sm",  fontSize=8, textColor=_C["muted"],
                          fontName=_ARABIC_FONT_NAME if _ARABIC_REGISTERED else "Helvetica",
...
def _header_footer(canvas, doc):
    ...
    canvas.drawString(MARGIN, h - 13*mm, doc._lab_name.upper())

    # Right: report type
    canvas.setFont("Helvetica", 9)
    canvas.drawRightString(w - MARGIN, h - 10*mm, "MICROBIOLOGY CLINICAL DECISION REPORT")
    canvas.drawRightString(w - MARGIN, h - 15*mm, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    # Footer
    canvas.setFillColor(_C["border"])
    canvas.rect(0, 10*mm, w, 0.3*mm, fill=True, stroke=False)
    canvas.setFont("Helvetica", 6.5)
    canvas.setFillColor(_C["muted"])
    canvas.drawString(MARGIN, 6*mm,
        "DISCLAIMER: This report is a clinical decision support tool and does NOT replace clinical judgment. "
        "Treatment decisions are the responsibility of the treating physician.")
    canvas.drawRightString(w - MARGIN, 6*mm,
        f"EUCAST 2026 | CLSI 2026 | IDSA AMR 2025 | WHO AWaRe 2025 | Page {doc.page}")

    canvas.setFillColor(_C["accent"])
    canvas.setFont("Helvetica-Bold", 7)
    canvas.drawCentredString(w / 2, 6*mm, f"Developed by Dr. Hussein Ali | Orange Lab")
...
def _section_title(text: str, st: dict) -> list:
    return [
        Spacer(1, 3*mm),
        HRFlowable(width="100%", thickness=0.5, color=_C["primary"], spaceAfter=2),
        Paragraph(text, st["section"]),
        Spacer(1, 1*mm),
    ]


def _info_table(data: list, col_widths: list = None) -> Table:
    """Compact key-value info table"""
    usable = PAGE_W - 2 * MARGIN
    cw = col_widths or [45*mm, (usable - 45*mm) / 1]
...
def _ast_table(sir_map: dict, st_: dict) -> Table:
    """AST results table with S/I/R coloring"""
    usable = PAGE_W - 2 * MARGIN
    SIR_COLORS = {"S": _C["access"], "I": _C["watch"], "R": _C["reserve"]}
    SIR_LABELS = {"S": "Sensitive", "I": "Intermediate", "R": "Resistant"}

    headers = [
        Paragraph("<b>Antibiotic</b>", st_["body_bold"]),
        Paragraph("<b>Result</b>",     st_["body_bold"]),
        Paragraph("<b>Interpretation</b>", st_["body_bold"]),
    ]
    rows = [headers]
    for drug, sir in sorted(sir_map.items()):
        color = SIR_COLORS.get(sir, _C["text"])
        rows.append([
            Paragraph(drug, st_["body"]),
            Paragraph(f'<font color="{color.hexval() if hasattr(color,"hexval") else "#000"}">'
                      f'<b>{sir}</b></font>', st_["body_bold"]),
            Paragraph(SIR_LABELS.get(sir, sir), st_["body"]),
        ])

    cw = [usable * 0.50, usable * 0.15, usable * 0.35]
...
def _drug_section(drugs: list, aware: str, st_: dict,
                  show_commercial: bool = False, renal: bool = False,
                  renal_severity: str = "") -> list:
    """Render a list of drugs with AWaRe badge + notes"""
    if not drugs:
        return []
    usable = PAGE_W - 2 * MARGIN
    style_map = {"Access": st_["badge_acc"], "Watch": st_["badge_wat"], "Reserve": st_["badge_res"]}
    badge_style = style_map.get(aware, st_["body"])
    rows = []
    for d in drugs:
        name  = d.get("name", "")
        note  = (d.get("specimen_notes") or {}).get("", "") or d.get("note", "")
        route = d.get("route", "")
        ren   = d.get("renal_note", "") if renal else ""
        brands = get_commercial_name(name) if show_commercial else ""

        cell_left = [Paragraph(f"<b>{name}</b>", st_["drug_name"])]
        if brands:
            # Show brands prominently right under drug name
            cell_left.append(Paragraph(
                f"<i>Brand names: {brands}</i>",
                ParagraphStyle("brand_inline",
                    parent=st_["small"],
                    textColor=_C["accent"],
                    fontName="Helvetica-Oblique",
                    fontSize=7.5)))
        if route:
            cell_left.append(Paragraph(f"Route: {route}", st_["small"]))
        if note:
            short_note = note[:120] + "..." if len(note) > 120 else note
            cell_left.append(Paragraph(short_note, st_["note_text"]))
        if ren:
            cell_left.append(Paragraph(f"Renal: {ren}", st_["warn"]))

        cell_right = Paragraph(aware, badge_style)
        rows.append([[cell_left], [cell_right]])

    # Build as flat table
    flat_rows = []
    for left_parts, right_part in rows:
        flat_rows.append([left_parts[0], right_part])
    cw = [usable * 0.78, usable * 0.22]
    t  = Table(flat_rows, colWidths=cw)
    t.setStyle(TableStyle([
        ("VALIGN",         (0, 0), (-1, -1), "TOP"),
        ("GRID",           (0, 0), (-1, -1), 0.3, _C["border"]),
...
def generate_pdf_report_en(
    ...
    combo_suggestions:    list = None,
    show_commercial_names: bool = False,
    child_pugh:           str = "",
    hepatic_recs:         list = None,
) -> Optional[bytes]:
    """Generate professional English-only PDF clinical report."""
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Microbiology CDSS Report — {patient_name} — {organism}",
        author=f"Dr. Hussein Ali | {lab_name}",
    )
    doc._lab_name = lab_name
    doc._arabic_ok = ARABIC_PDF_SUPPORTED and _ARABIC_REGISTERED
    st_ = _styles()
    story = []

    # ── 1. PATIENT HEADER ─────────────────────────────────────────────
    story += [
        Paragraph(f"Clinical Microbiology Report", st_["title"]),
        Paragraph(f"{lab_name}  |  {lab_city}  |  {date_in or datetime.now().strftime('%Y-%m-%d')}", st_["subtitle"]),
        Spacer(1, 3*mm),
    ]

    # Patient info 3-column
    renal_info = f"IMPAIRED (CrCl: {cl_cr:.1f} ml/min)" if is_renal else "Normal"
    usable = PAGE_W - 2 * MARGIN
    pat_data = [
        [Paragraph("<b>Patient Name</b>", st_["body_bold"]),
         Paragraph(patient_name or "Not specified", st_["body"]),
         Paragraph("<b>Date</b>", st_["body_bold"]),
         Paragraph(date_in or "—", st_["body"])],
        [Paragraph("<b>Age / Sex</b>", st_["body_bold"]),
         Paragraph(f"{age} years / {sex}", st_["body"]),
         Paragraph("<b>Weight</b>", st_["body_bold"]),
         Paragraph(f"{weight} kg", st_["body"])],
        [Paragraph("<b>Renal Function</b>", st_["body_bold"]),
         Paragraph(renal_info, st_["warn"] if is_renal else st_["body"]),
         Paragraph("<b>Hepatic</b>", st_["body_bold"]),
         Paragraph(f"IMPAIRED (Child-Pugh {child_pugh})" if is_hepatic else "Normal",
                   st_["warn"] if is_hepatic else st_["body"])],
        # Only show pregnancy row for females of childbearing age
        *([
            [Paragraph("<b>Pregnant</b>", st_["body_bold"]),
             Paragraph("YES — Review pregnancy warnings below",  st_["warn"]),
             Paragraph("", st_["body"]), Paragraph("", st_["body"])],
        ] if is_preg else (
            [[Paragraph("<b>Pregnant</b>", st_["body_bold"]),
              Paragraph("Not applicable (Male / Pediatric)", st_["body"]),
              Paragraph("", st_["body"]), Paragraph("", st_["body"])],
            ] if (sex == "Male" or age < 12) else []
        )),
    ]
    cw4 = [usable*0.22, usable*0.28, usable*0.22, usable*0.28]
    pat_t = Table(pat_data, colWidths=cw4)
    ...
    # ── 2. CULTURE RESULT ─────────────────────────────────────────────
    story += _section_title("Culture Result", st_)
    cult_data = [
        [Paragraph("<b>Specimen</b>", st_["body_bold"]),    Paragraph(specimen, st_["body"]),
         Paragraph("<b>Colony Count</b>", st_["body_bold"]), Paragraph(colony_count or "—", st_["body"])],
        [Paragraph("<b>Organism</b>", st_["body_bold"]),    Paragraph(f"<b>{organism}</b>", st_["drug_name"]),
         Paragraph("<b>Pus Cells</b>", st_["body_bold"]),   Paragraph(pus_cells or "—", st_["body"])],
        [Paragraph("<b>RBCs</b>", st_["body_bold"]),        Paragraph(rbcs or "—", st_["body"]),
         Paragraph("", st_["body"]),                        Paragraph("", st_["body"])],
    ]
    cult_t = Table(cult_data, colWidths=cw4)
    cult_t.setStyle(TableStyle([
        ("BACKGROUND",  (0, 0), (0, -1), _C["header_bg"]),
        ("BACKGROUND",  (2, 0), (2, -1), _C["header_bg"]),
        ("FONTSIZE",    (0, 0), (-1, -1), 8),
        ("GRID",        (0, 0), (-1, -1), 0.3, _C["border"]),
    ...
    # ── 3. MDR / PHENOTYPE CLASSIFICATION ─────────────────────────────
    mdr_class = mdr_result.get("classification", "")
    ph_labels = [p.get("phenotype", "") for p in phenotypes]
    esbl_prob = esbl_result.get("probability", "low")

    if mdr_class or ph_labels or esbl_prob != "low":
        story += _section_title("Resistance Classification", st_)
        badges = []
        if mdr_class:
            clr = _C["mdr_xdr"] if mdr_class in ("XDR", "PDR") else _C["reserve"]
            badges.append(Paragraph(f"<b>{mdr_class}</b>", ParagraphStyle(
                "badge_mdr", parent=_styles()["body_bold"],
                textColor=clr, backColor=_C["mdr_bg"],
                borderPadding=(3, 6, 3, 6))))
        for ph in ph_labels:
            badges.append(Paragraph(f"<b>{ph}</b>", ParagraphStyle(
                f"badge_{ph}", parent=_styles()["body_bold"],
                textColor=_C["reserve"], backColor=_C["reserve_bg"],
                borderPadding=(3, 6, 3, 6))))
        if esbl_prob == "high":
            badges.append(Paragraph("<b>ESBL High Probability</b>", st_["warn"]))
        elif esbl_prob == "moderate":
            badges.append(Paragraph("<b>ESBL Moderate Probability</b>", st_["warn"]))

        if mdr_result.get("detail"):
            badges.append(Paragraph(mdr_result["detail"], st_["small"]))
        if mdr_result.get("action"):
            badges.append(Paragraph(f"Action: {mdr_result['action']}", st_["body"]))

        story += badges

    # ── 4. AST RESULTS ────────────────────────────────────────────────
    if sir_map:
        story += _section_title("Antibiotic Sensitivity Test (AST)", st_)
        story.append(_ast_table(sir_map, st_))

    # ── 5. RECOMMENDED ANTIBIOTICS ────────────────────────────────────
    story += _section_title("Antibiotic Recommendations", st_)

    if allowed:
        story.append(Paragraph("RECOMMENDED OPTIONS", ParagraphStyle(
            "rec_hdr", parent=st_["section"],
            textColor=_C["access"], fontSize=8)))
        story += _drug_section(allowed, "Access", st_,
                               show_commercial=show_commercial_names,
                               renal=is_renal,
                               renal_severity=str(cl_cr))
    if warned:
        story.append(Paragraph("USE WITH CAUTION", ParagraphStyle(
            "warn_hdr", parent=st_["section"],
            textColor=_C["watch"], fontSize=8)))
        story += _drug_section(warned, "Watch", st_,
                               show_commercial=show_commercial_names,
                               renal=is_renal)
    if banned:
        story += [
            Paragraph("CONTRAINDICATED / AVOID", ParagraphStyle(
                "ban_hdr", parent=st_["section"],
                textColor=_C["reserve"], fontSize=8)),
        ]
        ban_rows = [[Paragraph(f"<b>{d.get('name','')}</b>", st_["danger"]),
                     Paragraph(d.get("reason_short", d.get("category", "")), st_["body"])]
                    for d in banned]
        if ban_rows:
            bw = [usable*0.40, usable*0.60]
            bt = Table(ban_rows, colWidths=bw)
            bt.setStyle(TableStyle([
                ("FONTSIZE",      (0, 0), (-1, -1), 8),
                ("GRID",          (0, 0), (-1, -1), 0.3, _C["border"]),
                ("BACKGROUND",    (0, 0), (-1, -1), _C["banned_bg"]),
                ("TOPPADDING",    (0, 0), (-1, -1), 2),
                ("BOTTOMPADDING", (0, 0), (-1, -1), 2),
                ("LEFTPADDING",   (0, 0), (-1, -1), 5),
            ]))
    ...
    if is_preg and preg_warn_items:
        story += [
            Paragraph("PREGNANCY — USE WITH CAUTION", ParagraphStyle(
                "preg_hdr", parent=st_["section"],
                textColor=_C["warn"], fontSize=8)),
        ]
        for item in preg_warn_items:
            story.append(Paragraph(
                f"<b>{item.get('name','')}</b>: {item.get('preg_note','')[:150]}",
                st_["note_text"]))
        story.append(Spacer(1, 2*mm))

    # ── 6. PATHOGENICITY ASSESSMENT ───────────────────────────────────
    if patho_assessment:
        story += _section_title("Pathogenicity Assessment", st_)
        ...
        # Score bar via table
        bar_w = usable * 0.7
        fill  = int(bar_w * sc / 100)
        score_color = (_C["access"] if sc < 30 else
                       _C["watch"] if sc < 50 else
                       _C["reserve"] if sc >= 75 else
                       colors.HexColor("#e67e22"))
        story.append(Paragraph(f"Pathogenicity Score: <b>{sc}%</b> — {verd}", st_["body"]))
        if interp:
            story.append(Paragraph(interp, st_["note_text"]))
        for f in flags:
            story.append(Paragraph(f"FLAG: {f}", st_["warn"]))
        for r in recs:
            story.append(Paragraph(f"  • {r}", st_["body"]))

    # ── 7. TREATMENT DURATION ─────────────────────────────────────────
    if duration_data:
        story += _section_title("Treatment Duration Recommendation", st_)
        d = duration_data
        dur_data = [
            ["Protocol", d.get("label", "—")],
            ["Standard Duration", f"{d.get('standard_days', d.get('standard', '?'))} days"],
            ["Acceptable Range", f"{d.get('min_days', '?')} – {d.get('max_days', '?')} days"],
            ["IV / PO Split", f"IV: {d.get('iv_days', 0)}d  |  PO: {d.get('po_days', 0)}d"],
            ["Notes", d.get("notes", "—")],
            ["Reference", d.get("ref", "—")],
            ["Follow-up Culture", "Recommended" if d.get("follow_up_culture") else "Not required"],
        ]
        story.append(_info_table(
            [[Paragraph(f"<b>{r[0]}</b>", st_["body_bold"]),
              Paragraph(str(r[1]), st_["body"])] for r in dur_data],
            col_widths=[45*mm, usable - 45*mm]
        ))

    # ── 8. COMBINATION THERAPY ────────────────────────────────────────
    if combo_suggestions:
        story += _section_title("Combination Therapy Recommendations", st_)
        for cs in combo_suggestions:
            ph     = cs["phenotype"]
            data   = cs["data"]
            story.append(Paragraph(
                f"{data['urgency']} Priority — {data['title']}",
                st_["warn"] if data["urgency"] == "CRITICAL" else st_["body_bold"]))
            combo_rows = [
                [Paragraph("<b>Combination</b>", st_["body_bold"]),
                 Paragraph("<b>Indication</b>",  st_["body_bold"]),
                 Paragraph("<b>Evidence</b>",     st_["body_bold"]),
                 Paragraph("<b>Reference</b>",    st_["body_bold"])],
            ]
            for opt in data["options"]:
                is_avoid = "AVOID" in opt["combo"] or "AVOID" in opt["evidence"]
                name_style = st_["danger"] if is_avoid else st_["combo"]
                combo_rows.append([
                    Paragraph(opt["combo"], name_style),
                    Paragraph(opt["indication"][:100], st_["note_text"]),
                    Paragraph(opt["evidence"], st_["body_bold"]),
                    Paragraph(opt["ref"][:60],  st_["small"]),
                ])
            cw_combo = [usable*0.30, usable*0.38, usable*0.12, usable*0.20]
            ct = Table(combo_rows, colWidths=cw_combo, repeatRows=1)
    ...
    if is_hepatic and hepatic_recs:
        action_recs = [r for r in hepatic_recs if r.get("requires_action")]
        if action_recs:
            story += _section_title(f"Hepatic Dose Adjustments (Child-Pugh {child_pugh})", st_)
            hep_rows = [
                [Paragraph("<b>Drug</b>",           st_["body_bold"]),
                 Paragraph("<b>Level</b>",           st_["body_bold"]),
                 Paragraph("<b>Recommendation</b>", st_["body_bold"])],
            ]
            for r in action_recs:
                lvl_style = st_["danger"] if "Avoid" in r["level"] else st_["warn"]
                hep_rows.append([
                    Paragraph(f"<b>{r['name']}</b>", st_["drug_name"]),
                    Paragraph(r["level"],             lvl_style),
                    Paragraph(r["recommendation"],    st_["body"]),
                ])
            cw_hep = [usable*0.30, usable*0.25, usable*0.45]
            ht = Table(hep_rows, colWidths=cw_hep, repeatRows=1)
            ...
            story += [ht, Spacer(1, 2*mm)]

    # ── 10. DRUG INTERACTIONS ─────────────────────────────────────────
    if interactions:
        story += _section_title("Drug Interactions", st_)
        for ia in interactions:
            story.append(Paragraph(f"⚠ {ia}", st_["warn"]))

    # ── 11. GUIDELINES FOOTER ─────────────────────────────────────────
    story += [
        Spacer(1, 4*mm),
        HRFlowable(width="100%", thickness=0.5, color=_C["border"]),
        Spacer(1, 1*mm),
        Paragraph(
            "Guidelines: CLSI M100 2026 | EUCAST 2026 | IDSA AMR 2025 | WHO AWaRe 2025 | "
            "BNF 2025 | Sanford Guide 2025 | Egypt National Guidelines",
            st_["disclaimer"]),
        Paragraph(
            "This report is generated by Microbiology CDSS (CDSS) — "
            f"Developed by Dr. Hussein Ali | {lab_name} | {lab_city}",
            st_["disclaimer"]),
    ]
...
# ═══
# ═══════════════════════════════════════════════════════════════════════
# HTML → PDF ENGINE — Full Arabic + RTL Support
# WeasyPrint | Bilingual Report (Arabic + English)
# Design: Microbiology CDSS — Orange Lab Professional Medical Report
# ═══════════════════════════════════════════════════════════════════════
...
def _sir_class(sir: str) -> str:
    return {"S": "sir-s", "I": "sir-i", "R": "sir-r"}.get(sir, "")


def _sir_label(sir: str) -> str:
    return {"S": "حساس — Sensitive", "I": "متوسط — Intermediate", "R": "مقاوم — Resistant"}.get(sir, sir)


def _aware_badge(aware: str) -> str:
    cls = {"Access": "badge-access", "Watch": "badge-watch", "Reserve": "badge-reserve"}.get(aware, "")
    label = {"Access": "🟢 Access", "Watch": "🟡 Watch", "Reserve": "🔴 Reserve"}.get(aware, aware)
...
def _drug_card_html(d: dict, aware: str,
                    is_renal: bool = False,
                    show_commercial: bool = False) -> str:
    name   = _esc(d.get("name", ""))
    route  = _esc(d.get("route", ""))
    note   = _esc((d.get("specimen_notes") or {}).get("", "") or d.get("note", ""))
    ren    = _esc(d.get("renal_note", "")) if is_renal else ""
    brands = _esc(get_commercial_name(d.get("name", ""))) if show_commercial else ""
    badge  = _aware_badge(aware)
    ...
    <div class="drug-card no-break">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        <div>
          ...
          {"&nbsp;&nbsp;&bull;&nbsp;&nbsp;<small style='color:#666'>"+route+"</small>" if route else ""}
          {"<br><span class='drug-brands'>الأسماء التجارية: "+brands+"</span>" if brands else ""}
          {"<br><span class='drug-note'>"+note[:140]+("..." if len(note)>140 else "")+"</span>" if note else ""}
          {"<br><span class='drug-renal'>⚠ "+ren+"</span>" if ren else ""}
        </div>
        <div style="margin-right:4mm;white-space:nowrap">{badge}</div>
      </div>
    </div>'''
    return html
...
@page {
    ...
    @bottom-center {
        content: "صفحة " counter(page) " من " counter(pages) " | Microbiology CDSS";
...
body {
    font-family: 'Amiri', 'Noto Naskh Arabic', 'DejaVu Sans', 'Arial Unicode MS', sans-serif;
...
.header-title { font-size: 18pt; font-weight: bold; margin-bottom: 1mm; }
.header-sub   { font-size: 9pt; opacity: 0.85; }
.header-right { text-align: left; font-size: 8.5pt; opacity: 0.9; direction: ltr; }
.accent-bar   { height: 3mm; background: #ff8c00; }
.content      { padding: 5mm 0; }
.section      { margin: 4mm 0; page-break-inside: avoid; }
.section-title {
    font-size: 10pt; font-weight: bold; color: #0d3b66;
    text-transform: uppercase; letter-spacing: 0.5pt;
    border-bottom: 1.5pt solid #0d3b66;
    padding-bottom: 1mm; margin-bottom: 3mm;
}
/* Info tables */
table.info-table { width: 100%; border-collapse: collapse; font-size: 9pt; }
table.info-table td { padding: 2mm 4mm; border: 0.3pt solid #d5d8dc; }
table.info-table .lbl { background: #f4f6f8; font-weight: bold; color: #0d3b66; width: 24%; }
table.info-table .val { color: #1a1a2e; }
table.info-table .warn-val { color: #b7770d; font-weight: bold; }
table.info-table .danger-val { color: #922b21; font-weight: bold; }
/* AST table */
table.ast-table { width: 100%; border-collapse: collapse; font-size: 9pt; margin: 2mm 0; }
table.ast-table th {
    background: #0d3b66; color: white;
    padding: 2.5mm 4mm; text-align: right; font-weight: bold;
}
table.ast-table td { padding: 2mm 4mm; border: 0.3pt solid #d5d8dc; }
table.ast-table tr:nth-child(even) td { background: #f8f9fa; }
.sir-s { color: #1e8449; font-weight: bold; }
.sir-i { color: #b7770d; font-weight: bold; }
.sir-r { color: #922b21; font-weight: bold; }
/* Drug cards */
.drug-card {
    border: 0.4pt solid #d5d8dc; margin: 1.5mm 0;
    padding: 2.5mm 4mm; border-radius: 2mm;
}
.drug-name   { font-weight: bold; font-size: 10pt; color: #0d3b66; }
.drug-brands { color: #ff8c00; font-style: italic; font-size: 8.5pt; }
.drug-note   { color: #666680; font-size: 8pt; margin-top: 1mm; }
.drug-renal  { color: #b7770d; font-size: 8pt; }
/* AWaRe badges */
.badge { padding: 1mm 3mm; border-radius: 1mm; font-size: 8pt; font-weight: bold; }
.badge-access  { background: #eafaf1; color: #1e8449; border: 0.5pt solid #1e8449; }
.badge-watch   { background: #fef9e7; color: #b7770d; border: 0.5pt solid #b7770d; }
.badge-reserve { background: #fdedec; color: #922b21; border: 0.5pt solid #922b21; }
/* Section stripes */
.stripe-access { border-right: 3pt solid #1e8449; padding-right: 3mm; }
.stripe-watch  { border-right: 3pt solid #b7770d; padding-right: 3mm; }
.stripe-banned { border-right: 3pt solid #922b21; padding-right: 3mm; background: #fdf8f8; padding: 3mm; border-radius: 2mm; }
/* MDR */
.mdr-badge {
    display: inline-block; background: #fef2f2;
    color: #7b241c; border: 1pt solid #f5b7b1;
    padding: 1.5mm 4mm; border-radius: 2mm;
    font-weight: bold; font-size: 9.5pt; margin: 1mm;
}
.pheno-badge {
    display: inline-block; background: #fdf2f8;
    color: #6e2fa0; border: 1pt solid #d7bde2;
    padding: 1mm 3mm; border-radius: 2mm;
    font-weight: bold; font-size: 9pt; margin: 1mm;
}
/* Patho score */
.score-wrap  { margin: 2mm 0; }
.score-label { font-size: 9pt; color: #1a1a2e; margin-bottom: 1mm; }
.score-bar-outer { background: #e5e7eb; border-radius: 2mm; height: 5mm; width: 100%; }
.score-bar-inner { height: 5mm; border-radius: 2mm; }
/* Duration */
.duration-grid {
    display: table; width: 100%; border-collapse: separate;
    border-spacing: 3mm; margin: 2mm 0;
}
.duration-cell {
    display: table-cell; text-align: center;
    background: #f4f6f8; border: 0.5pt solid #d5d8dc;
    border-radius: 2mm; padding: 3mm;
}
.duration-val   { font-size: 18pt; font-weight: bold; color: #0d3b66; }
.duration-lbl   { font-size: 7.5pt; color: #666680; }
/* Combo */
.combo-card {
    border: 0.5pt solid #d5d8dc; margin: 2mm 0;
    padding: 3mm; border-radius: 2mm;
}
.combo-avoid { background: #fdf2f8; border-color: #922b21; }
.combo-title { font-weight: bold; color: #0d3b66; font-size: 9.5pt; }
.combo-ind   { color: #444; font-size: 8.5pt; margin: 1mm 0; }
.combo-caution { color: #b7770d; font-size: 8pt; }
.combo-ref   { color: #666680; font-size: 7.5pt; font-style: italic; }
.stars       { color: #ff8c00; }
/* Alerts */
.alert { padding: 2.5mm 4mm; border-radius: 2mm; margin: 1.5mm 0; font-size: 9pt; }
.alert-warn   { background: #fef9e7; border: 0.5pt solid #b7770d; color: #7d6608; }
.alert-danger { background: #fdedec; border: 0.5pt solid #922b21; color: #78281f; }
.alert-info   { background: #eaf4fb; border: 0.5pt solid #2980b9; color: #1a5276; }
.alert-ok     { background: #eafaf1; border: 0.5pt solid #1e8449; color: #145a32; }
/* Footer */
.footer {
    border-top: 0.5pt solid #d5d8dc; margin-top: 5mm;
    padding-top: 2mm; font-size: 7pt; color: #888;
}
.no-break { page-break-inside: avoid; }
.pb-before { page-break-before: always; }
hr.divider { border: none; border-top: 0.5pt solid #d5d8dc; margin: 3mm 0; }
"""
...
def generate_pdf_html_report(
    ...
    """
    Generate bilingual Arabic+English PDF via WeasyPrint.
    Full RTL Arabic support + professional medical layout.
    """
...
<head>
<meta charset="UTF-8">
<title>تقرير Microbiology CDSS — {_esc(patient_name)}</title>
<style>{_CSS}</style>
</head>
<body>""")
...
<div class="header">
  <div>
    <div class="header-title">🔬 {_esc(lab_name)}</div>
    <div class="header-sub">{_esc(lab_city)} &nbsp;|&nbsp; Microbiology CDSS</div>
    <div class="header-sub">نظام دعم القرار الإكلينيكي في الأحياء الدقيقة</div>
  </div>
  <div class="header-right ltr">
    <b>MICROBIOLOGY CLINICAL REPORT</b><br>
    Date: {_esc(date_in or now_str[:10])}<br>
    Generated: {now_str}
  </div>
</div>
...
<div class="content">""")
    ...
    renal_cls = "danger-val" if is_renal else "val"
    renal_txt = f"مختل — CrCl: {cl_cr:.1f} ml/min" if is_renal else "طبيعية"
    hep_txt   = f"مختل — Child-Pugh {child_pugh}" if is_hepatic else "طبيعية"

    # Pregnancy row: only for females of childbearing age
    show_preg = sex == "Female" and age >= 12
    preg_html = ""
    if show_preg:
        preg_txt = "نعم — راجع قسم تحذيرات الحمل أدناه" if is_preg else "لا"
        preg_cls = "warn-val" if is_preg else "val"
        preg_html = f"""
  <tr>
    <td class="lbl">الحمل / Pregnancy</td>
    <td class="val {preg_cls}">{preg_txt}</td>
    <td class="lbl">–</td>
    <td class="val">–</td>
  </tr>"""
...
<div class="section no-break">
  ...
  <table class="info-table">
    <tr>
      <td class="lbl">اسم المريض / Name</td>
      <td class="val"><b>{_esc(patient_name or "—")}</b></td>
      <td class="lbl">التاريخ / Date</td>
      <td class="val ltr">{_esc(date_in or "—")}</td>
    </tr>
    <tr>
      <td class="lbl">العمر / الجنس</td>
      <td class="val">{age} سنة — {_esc(sex)}</td>
      <td class="lbl">الوزن / Weight</td>
      <td class="val">{weight} kg</td>
    </tr>
    <tr>
      <td class="lbl">الوظيفة الكلوية</td>
      <td class="val {renal_cls}">{renal_txt}</td>
      <td class="lbl">وظيفة الكبد</td>
      <td class="val {'warn-val' if is_hepatic else 'val'}">{hep_txt}</td>
    </tr>
    {preg_html}
  </table>
</div>""")

    # ── SECTION 2: CULTURE RESULT ────────────────────────────────────────
    H.append(f"""
<div class="section no-break">
  <div class="section-title">نتيجة المزرعة — Culture Result</div>
  <table class="info-table">
    <tr>
      <td class="lbl">نوع العينة / Specimen</td>
      <td class="val">{_esc(specimen)}</td>
      <td class="lbl">عدد المستعمرات</td>
      <td class="val ltr">{_esc(colony_count or "—")}</td>
    </tr>
    <tr>
      <td class="lbl">الكائن المعزول / Organism</td>
      <td class="val"><b><i>{_esc(organism)}</i></b></td>
      <td class="lbl">خلايا صديد / Pus Cells</td>
      <td class="val ltr">{_esc(pus_cells or "—")} /HPF</td>
    </tr>
    <tr>
      <td class="lbl">كريات الدم الحمراء / RBCs</td>
      <td class="val ltr">{_esc(rbcs or "—")} /HPF</td>
      <td class="lbl">–</td><td class="val">–</td>
    </tr>
  </table>
</div>""")

    # ── SECTION 3: RESISTANCE CLASSIFICATION ─────────────────────────────
    mdr_class  = mdr_result.get("classification", "")
    ph_labels  = [p.get("phenotype", "") for p in phenotypes]
    esbl_prob  = esbl_result.get("probability", "low")
    has_resist = mdr_class or ph_labels or esbl_prob != "low"

    if has_resist:
        badges_html = ""
        if mdr_class:
            badges_html += f'<span class="mdr-badge">{_esc(mdr_class)}</span>'
        for ph in ph_labels:
            badges_html += f'<span class="pheno-badge">{_esc(ph)}</span>'
        if esbl_prob == "high":
            badges_html += '<span class="pheno-badge">ESBL — Probable</span>'
        elif esbl_prob == "moderate":
            badges_html += '<span class="pheno-badge">ESBL — Possible</span>'

        detail = _esc(mdr_result.get("detail", ""))
        action = _esc(mdr_result.get("action", ""))
        H.append(f"""
<div class="section no-break">
  <div class="section-title">تصنيف المقاومة — Resistance Classification</div>
  <div>{badges_html}</div>
  {"<p style='margin-top:2mm;font-size:9pt;'>"+detail+"</p>" if detail else ""}
  {"<div class='alert alert-warn' style='margin-top:2mm'>"+action+"</div>" if action else ""}
</div>""")
  ...
  <tr>
    <td>{_esc(drug)}</td>
    <td class="{_sir_class(sir)} ltr">{sir}</td>
    <td>{_sir_label(sir)}</td>
  </tr>"""
        H.append(f"""
<div class="section">
  <div class="section-title">نتيجة اختبار الحساسية — AST Results</div>
  <table class="ast-table">
    <thead><tr>
      <th>المضاد الحيوي / Antibiotic</th>
      <th>النتيجة</th>
    ...
    H.append('<div class="section"><div class="section-title">توصيات المضادات الحيوية — Antibiotic Recommendations</div>')

    if allowed:
        H.append('<div style="margin-bottom:2mm"><b style="color:#1e8449">✅ الأدوية الموصى بها — Recommended</b></div>')
        H.append('<div class="stripe-access">')
        for d in allowed:
            H.append(_drug_card_html(d, "Access", is_renal, show_commercial_names))
        H.append('</div>')

    if warned:
        H.append('<div style="margin:3mm 0 1mm"><b style="color:#b7770d">⚠️ استخدام بحذر — Use with Caution</b></div>')
        H.append('<div class="stripe-watch">')
        for d in warned:
            H.append(_drug_card_html(d, "Watch", is_renal, show_commercial_names))
        H.append('</div>')

    if banned:
        H.append('<div style="margin:3mm 0 1mm"><b style="color:#922b21">🚫 ممنوع / موانع — Contraindicated</b></div>')
        H.append('<div class="stripe-banned">')
        for d in banned:
            reason = _esc(d.get("reason_short", d.get("category", "")))
            H.append(f'<div class="drug-card"><span class="drug-name" style="color:#922b21">{_esc(d.get("name",""))}</span>'
                     f'{"&nbsp;&nbsp;|&nbsp;&nbsp;"+reason if reason else ""}</div>')
        H.append('</div>')

    if is_preg and preg_warn_items:
        H.append('<div style="margin:3mm 0 1mm"><b style="color:#6e2fa0">🤰 تحذيرات الحمل — Pregnancy Warnings</b></div>')
        for item in preg_warn_items:
            pn = _esc(item.get("preg_note","")[:200])
            H.append(f'<div class="alert alert-warn"><b>{_esc(item.get("name",""))}</b>: {pn}</div>')

    H.append('</div>')  # end recommendations section

    # ── SECTION 6: DRUG INTERACTIONS ─────────────────────────────────────
    if interactions:
        rows_ia = "".join(
            f'<div class="alert alert-warn" style="margin:1mm 0">⚠ {_esc(ia)}</div>'
            for ia in interactions)
        H.append(f'<div class="section"><div class="section-title">تفاعلات دوائية — Drug Interactions</div>{rows_ia}</div>')

    # ── SECTION 7: PATHOGENICITY ─────────────────────────────────────────
    if patho_assessment:
        sc    = patho_assessment.get("score", 0)
        verd  = _esc(patho_assessment.get("verdict", ""))
        interp= _esc(patho_assessment.get("interpretation", ""))
        flags = patho_assessment.get("special_flags", [])
        recs  = [_esc(r) for r in patho_assessment.get("recommendations", [])]
        clr   = _score_color(sc)

        flag_html = ""
        flag_map  = {
            "ABU_NO_TREAT":  ("alert-warn",   "🔵 ABU — لا تعطي مضادات حيوية (IDSA 2019)"),
            "ABU_TREAT":     ("alert-danger",  "🔵 ABU — علاج مطلوب (سياق عالي الخطورة)"),
            "MW_REJECT":     ("alert-danger",  "🧫 Murray-Washington: عينة مرفوضة — أعِد الأخذ"),
            "MW_ADEQUATE":   ("alert-ok",      "🧫 Murray-Washington: عينة مناسبة ✅"),
            "SIRS_HIGH":     ("alert-danger",  "🩸 SIRS ≥3 معايير — احتمال إنتان مرتفع"),
            "PEDIATRIC_UTI": ("alert-info",    "👶 معيار الأطفال مُطبَّق (عمر < 2 سنة)"),
        }
        for fl, (cls, msg) in flag_map.items():
            if fl in flags:
                flag_html += f'<div class="alert {cls}">{msg}</div>'

        recs_html = "".join(f"<li>{r}</li>" for r in recs)

        H.append(f"""
<div class="section no-break">
  <div class="section-title">تقييم الإمراضية — Pathogenicity Assessment</div>
  <div class="score-wrap">
    <div class="score-label">النتيجة / Score: <b style="color:{clr}">{sc}%</b> — {verd}</div>
    <div class="score-bar-outer">
      <div class="score-bar-inner" style="width:{sc}%;background:{clr}"></div>
    </div>
  </div>
  {"<p style='font-size:9pt;color:#444;margin-top:2mm'>"+interp+"</p>" if interp else ""}
  {flag_html}
  {"<ul style='font-size:9pt;margin:2mm 0 0 5mm'>"+recs_html+"</ul>" if recs else ""}
</div>""")

    # ── SECTION 8: TREATMENT DURATION ────────────────────────────────────
    if duration_data:
        d     = duration_data
        label = _esc(d.get("label", ""))
        std   = d.get("standard_days", d.get("standard", "?"))
        rng   = f"{d.get('min_days','?')}–{d.get('max_days','?')}"
        iv_d  = d.get("iv_days", 0)
        po_d  = d.get("po_days", 0)
        notes = _esc(d.get("notes", ""))
        ref   = _esc(d.get("ref", ""))
        fu    = d.get("follow_up_culture", False)

        H.append(f"""
<div class="section no-break">
  <div class="section-title">مدة العلاج — Treatment Duration</div>
  <p style="font-size:9pt;color:#0d3b66;margin-bottom:3mm"><b>{label}</b></p>
  <div class="duration-grid">
    <div class="duration-cell"><div class="duration-val">{std}</div><div class="duration-lbl">أيام قياسية<br>Standard Days</div></div>
    <div class="duration-cell"><div class="duration-val">{rng}</div><div class="duration-lbl">النطاق المقبول<br>Acceptable Range</div></div>
    <div class="duration-cell"><div class="duration-val" style="font-size:13pt">IV:{iv_d}d · PO:{po_d}d</div><div class="duration-lbl">توزيع الجرعة<br>IV / PO Split</div></div>
  </div>
  {"<div class='alert alert-info' style='margin-top:2mm;font-size:8.5pt'>📋 "+notes+"</div>" if notes else ""}
  {"<div class='alert alert-warn' style='margin-top:1mm'>🔄 مزرعة متابعة موصى بها بعد انتهاء العلاج</div>" if fu else ""}
  {"<p style='font-size:7.5pt;color:#888;margin-top:1mm'>📚 "+ref+"</p>" if ref else ""}
</div>""")

    # ── SECTION 9: COMBINATION THERAPY ───────────────────────────────────
    if combo_suggestions:
        H.append('<div class="section"><div class="section-title">العلاج التوافقي — Combination Therapy</div>')
        for cs in combo_suggestions:
            ph   = cs["phenotype"]
            data = cs["data"]
            urg  = data["urgency"]
            urg_cls = "alert-danger" if urg == "CRITICAL" else "alert-warn"
            H.append(f'<div class="alert {urg_cls}" style="margin-bottom:2mm"><b>{_esc(urg)}</b> — {_esc(data["title"])}</div>')
            for opt in data["options"]:
                avoid = "AVOID" in opt.get("evidence","") or "AVOID" in opt["combo"].upper()
                card_cls = "combo-card combo-avoid" if avoid else "combo-card"
                H.append(f"""
<div class="{card_cls}">
  <div class="combo-title {'danger-val' if avoid else ''}" style="{'color:#922b21' if avoid else ''}">
    {'🚫 ' if avoid else ''}{_esc(opt["combo"])}
    {"&nbsp;&nbsp;<span class='stars'>"+_esc(opt['evidence'])+"</span>" if not avoid else ""}
  </div>
  <div class="combo-ind">{_esc(opt['indication'])}</div>
  {"<div class='combo-caution'>⚠ "+_esc(opt['caution'])+"</div>" if opt.get('caution') else ""}
  <div class="combo-ref">📚 {_esc(opt['ref'])}</div>
</div>""")
        H.append('</div>')

    # ── SECTION 10: HEPATIC DOSING ────────────────────────────────────────
    if is_hepatic and hepatic_recs:
        action_recs = [r for r in hepatic_recs if r.get("requires_action")]
        if action_recs:
            H.append(f'<div class="section no-break"><div class="section-title">جرعات قصور الكبد — Hepatic Dosing (Child-Pugh {_esc(child_pugh)})</div>')
            for r in action_recs:
                avoid = "Avoid" in r["level"]
                cls   = "alert-danger" if avoid else "alert-warn"
                H.append(f'<div class="alert {cls}"><b>{_esc(r["name"])}</b>: {_esc(r["recommendation"])} — <i>{_esc(r["note"])}</i></div>')
            H.append('</div>')

    # ── FOOTER ────────────────────────────────────────────────────────────
    H.append(f"""
<div class="footer">
  <b>المراجع:</b> CLSI M100 2026 | EUCAST 2026 | IDSA AMR 2025 | WHO AWaRe 2025 | BNF 2025 | Sanford Guide 2025<br>
  <b>إخلاء المسؤولية:</b> هذا التقرير أداة دعم قرار ولا يُغني عن الحكم السريري للطبيب المعالج.<br>
  <span class="ltr">Developed by Dr. Hussein Ali | {_esc(lab_name)} | Microbiology CDSS</span>
</div>
</div></body></html>""")
...
if uploaded:
    ...
    with col2:
        ...
        if final_drugs:
            ...
            # ── PDF Report — English Only ──────────────────────────────
            st.markdown("---")
            st.markdown("#### 📄 PDF Clinical Report")
            st.caption("تقرير PDF احترافي — عربي + إنجليزي | WeasyPrint | RTL مدعوم بالكامل")
            _popt1, _popt2, _popt3 = st.columns(3)
            _pdf_combo = _popt1.checkbox("Include Combination Therapy",
                value=st.session_state.get("pdf_include_combo",True), key="pdf_cb_combo")
            _pdf_dur   = _popt2.checkbox("Include Treatment Duration",
                value=st.session_state.get("pdf_include_duration",True), key="pdf_cb_dur")
            _pdf_patho = _popt3.checkbox("Include Pathogenicity Score",
            ...
            if WEASYPRINT_AVAILABLE:
                ...
                _dur_pdf = get_treatment_duration(
                    specimen=culture_type, organism=organism_type,
                    syndrome=syndrome_info["syndrome"] if syndrome_info else "",
                    age=age, sex=sex, is_renal=is_renal,
                    phenotypes=phenotypes, severity=_sev_pdf,
                ) if _pdf_dur else None
                _combo_pdf = get_combination_therapy(phenotypes) if _pdf_combo else None
                _hep_pdf   = get_hepatic_recommendations(allowed, _cp_pdf) if is_hepatic else None

                try:
                    _pdf_bytes = generate_pdf_html_report(
                        patient_name        = _pt,
                        age=age, sex=sex, weight=weight,
                        cl_cr=cl_cr, is_renal=is_renal,
                        is_preg=is_preg, is_hepatic=is_hepatic,
                        ...
                        lab_name=_lab,
                        lab_city=_city,
                        patho_assessment=st.session_state.get("patho_result") if _pdf_patho else None,
                        duration_data=_dur_pdf,
                        combo_suggestions=_combo_pdf,
                        show_commercial_names=show_commercial,
                        child_pugh=_cp_pdf,
                        hepatic_recs=_hep_pdf,
                    )
                    if _pdf_bytes:
                        st.download_button(
                            "📄 تنزيل التقرير PDF (عربي + إنجليزي)",
                            data=_pdf_bytes,
