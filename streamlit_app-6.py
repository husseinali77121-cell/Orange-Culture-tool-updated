...
def generate_pdf_html_report(
    ...
    """
    2-page professional bilingual PDF via WeasyPrint.
    Page 1: Patient + Culture + AST
    ...
    # ── AWaRe ranking (ordered) ─────────────────────────────────────────
    AWARE_ORDER = {"Access": 0, "Watch": 1, "Reserve": 2, None: 3, "": 3}
    ranked_allowed = sorted(allowed,
                            key=lambda d: (AWARE_ORDER.get(d.get("aware"), 3),
                                           d.get("priority", 99)))

    # Group banned by drug class
    def _group_banned(banned_list):
        groups = {}
        for d in banned_list:
            cls = d.get("class", "Other")
            groups.setdefault(cls, []).append(d["name"])
        return groups

    H.append(f"""<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head><meta charset="UTF-8">
<title>Microbiology CDSS — {_esc(patient_name)}</title>
<style>{_CSS}
/* Extra for this layout */
.page-break {{ page-break-after: always; }}
.two-col {{ display: table; width: 100%; border-collapse: separate; border-spacing: 3mm; }}
.col-l {{ display: table-cell; width: 48%; vertical-align: top; }}
.col-r {{ display: table-cell; width: 48%; vertical-align: top; }}
.ranked-card {{ border-radius: 3mm; padding: 3mm 4mm; margin: 1.5mm 0; display: flex; justify-content: space-between; align-items: flex-start; page-break-inside: avoid; }}
.pill {{ padding: 0.5mm 3mm; border-radius: 3mm; font-size: 7.5pt; font-weight: bold; white-space: nowrap; }}
.rank-num {{ font-size: 9pt; font-weight: bold; min-width: 6mm; }}
.drug-name-big {{ font-size: 10pt; font-weight: bold; }}
.tier-hdr {{ font-size: 8pt; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5pt; border-bottom: 1pt solid; padding-bottom: 1mm; margin: 3mm 0 1.5mm; }}
.access-card  {{ background: #eafaf1; border: 1pt solid #1e8449; }}
.watch-card   {{ background: #fef9e7; border: 1pt solid #b7770d; }}
.reserve-card {{ background: #fdf2f2; border: 1pt solid #922b21; }}
.access-pill  {{ background: #1e8449; color: #fff; }}
.watch-pill   {{ background: #b7770d; color: #fff; }}
.reserve-pill {{ background: #922b21; color: #fff; }}
.access-txt   {{ color: #1e8449; }}
.watch-txt    {{ color: #b7770d; }}
.reserve-txt  {{ color: #922b21; }}
</style></head><body>""")
    ...
    mdr_badge_html = ""
    if mdr_class:
        mdr_badge_html = f'<span class="pill reserve-pill" style="font-size:8pt">{_esc(mdr_class)}</span> '
    for ph in ph_labels:
        mdr_badge_html += f'<span class="pill" style="background:#6e2fa0;color:#fff;font-size:8pt">{_esc(ph)}</span> '
    if esbl_prob == "high":
        mdr_badge_html += '<span class="pill" style="background:#b7770d;color:#fff">ESBL Probable</span>'

    H.append(f"""
<div class="header">
  <div>
    <div class="header-title">🔬 {_esc(lab_name)}</div>
    <div class="header-sub">{_esc(lab_city)} &nbsp;|&nbsp; Microbiology CDSS &nbsp;|&nbsp; نظام دعم القرار الإكلينيكي</div>
  </div>
  <div class="header-right ltr">
    <b>PAGE 1 — CULTURE REPORT</b><br>
    {_esc(date_in or now_str[:10])} &nbsp;|&nbsp; {now_str[-5:]}
  </div>
</div>
<div class="accent-bar"></div>
<div class="content">""")

    # ── Patient + Culture side-by-side ─────────────────────────────────
    renal_txt = f"Impaired — CrCl: {cl_cr:.1f} ml/min" if is_renal else "Normal"
    hep_txt   = f"Impaired — Child-Pugh {child_pugh}" if is_hepatic else "Normal"
    show_preg = sex == "Female" and age >= 12

    H.append('<div class="two-col"><div class="col-l">')
    H.append('<div class="section-title">Patient Information</div>')
    H.append(f'''<table class="info-table">
    <tr><td class="lbl">Patient</td><td class="val"><b>{_esc(patient_name or "—")}</b></td></tr>
    <tr><td class="lbl">Age / Sex</td><td class="val">{age}y — {_esc(sex)}</td></tr>
    <tr><td class="lbl">Weight</td><td class="val">{weight} kg</td></tr>
    <tr><td class="lbl">Renal</td><td class="val {"warn-val" if is_renal else ""}">{renal_txt}</td></tr>
    <tr><td class="lbl">Hepatic</td><td class="val {"warn-val" if is_hepatic else ""}">{hep_txt}</td></tr>
    {f'<tr><td class="lbl">Pregnancy</td><td class="val warn-val">YES — See alerts</td></tr>' if (show_preg and is_preg) else ""}
</table>''')

    H.append('</div><div class="col-r">')
    H.append('<div class="section-title">Culture Result</div>')
    H.append(f'''<table class="info-table">
    <tr><td class="lbl">Specimen</td><td class="val">{_esc(specimen)}</td></tr>
    <tr><td class="lbl">Organism</td><td class="val"><b><i>{_esc(organism)}</i></b></td></tr>
    <tr><td class="lbl">Colony Count</td><td class="val ltr">{_esc(colony_count or "—")}</td></tr>
    <tr><td class="lbl">Pus Cells</td><td class="val ltr">{_esc(pus_cells or "—")} /HPF</td></tr>
    <tr><td class="lbl">RBCs</td><td class="val ltr">{_esc(rbcs or "—")} /HPF</td></tr>
</table>
{"<div style='margin-top:2mm'>"+mdr_badge_html+"</div>" if mdr_badge_html else ""}''')
    H.append('</div></div>')

    # ── Full AST Table ─────────────────────────────────────────────────
    if sir_map:
        H.append('<div class="section"><div class="section-title">Antibiotic Sensitivity Test — AST</div>')
        H.append('<table class="ast-table"><thead><tr>'
                 '<th>Antibiotic</th><th>Result</th><th>Interpretation</th>'
                 '<th>WHO AWaRe</th></tr></thead><tbody>')
        _aware_map = {d.get("name",""): d.get("aware","") for d in (allowed + warned + banned)}
        for drug, sir in sorted(sir_map.items()):
            aware = _aware_map.get(drug, "")
            aware_pill = ""
            if aware == "Access":  aware_pill = '<span class="pill access-pill">Access</span>'
            elif aware == "Watch": aware_pill = '<span class="pill watch-pill">Watch</span>'
            elif aware == "Reserve": aware_pill = '<span class="pill reserve-pill">Reserve</span>'
            H.append(f'<tr><td>{_esc(drug)}</td>'
                     f'<td class="{_sir_class(sir)} ltr" style="font-size:10pt">{sir}</td>'
                     f'<td>{_sir_label(sir)}</td>'
                     f'<td>{aware_pill}</td></tr>')
        H.append('</tbody></table></div>')

    # ── Page Break ─────────────────────────────────────────────────────
    H.append('</div><div class="page-break"></div>')
...
<div class="header">
  <div>
    <div class="header-title">🔬 {_esc(lab_name)}</div>
    <div class="header-sub">{_esc(organism)} | {_esc(specimen)} | {_esc(patient_name or "—")}</div>
  </div>
  <div class="header-right ltr">
    <b>PAGE 2 — CLINICAL DECISION SUPPORT</b><br>
    {_esc(date_in or now_str[:10])}
  </div>
</div>
<div class="accent-bar"></div>
<div class="content">""")
    ...
    H.append('<div class="section"><div class="section-title">Recommended Therapy — Ranked by Evidence</div>')

    tier_map    = {"Access": ("Preferred (First-line)", "#1e8449", "access"),
                   "Watch":  ("Alternative",            "#b7770d", "watch"),
                   "Reserve":("Reserve — Severe / MDR", "#922b21", "reserve")}
    prev_tier   = ""
    drug_count  = 0

    for d in ranked_allowed[:10]:
        aware    = d.get("aware", "")
        tier_lbl, tier_clr, tier_cls = tier_map.get(aware, ("Other", "#444", ""))
        if tier_lbl != prev_tier:
            H.append(f'<div class="tier-hdr {tier_cls}-txt"'
                     f' style="border-color:{tier_clr}">{tier_lbl}</div>')
            prev_tier = tier_lbl

        name     = _esc(d.get("name",""))
        route    = "PO" if d.get("high_po") else "IV/IM"
        sir_v    = sir_map.get(d.get("name",""), "S")
        renal_n  = _esc(d.get("renal_note","")) if is_renal else ""
        brands   = _esc(get_commercial_name(d.get("name",""))) if show_commercial_names else ""

        H.append(f'''<div class="ranked-card {tier_cls}-card">
  <div style="flex:1">
    <span class="rank-num {tier_cls}-txt">{drug_count+1}.</span>
    <span class="drug-name-big {tier_cls}-txt">&nbsp;{name}</span>
    &nbsp;<span class="ltr" style="font-size:8pt;color:#555">{route}</span>
    &nbsp;<span class="ltr pill" style="background:#fff;border:1px solid {tier_clr};color:{tier_clr}">{sir_v}</span>
    {"<br><small style='color:#888;margin-right:6mm'>⚠ Renal: "+renal_n+"</small>" if renal_n else ""}
    {"<small style='color:#888;margin-right:6mm'>Brands: "+brands+"</small>" if brands else ""}
  </div>
  <div><span class="pill {tier_cls}-pill">{aware}</span></div>
</div>''')
        drug_count += 1

    H.append('</div>')  # end ranked section

    # ── Warnings (concise) ────────────────────────────────────────────
    if warned:
        H.append('<div class="section no-break"><div class="section-title">⚠ Dose Adjustment / Caution</div>')
        for d in warned[:6]:
            ren = _esc(d.get("renal_note","")) if is_renal else ""
            reason = _esc(d.get("reason_short",""))
            H.append(f'<div class="alert alert-warn" style="margin:1mm 0;font-size:8.5pt">'
                     f'<b>{_esc(d.get("name",""))}</b>'
                     f'{" — "+reason if reason else ""}'
                     f'{"&nbsp;|&nbsp;Renal: "+ren if ren else ""}</div>')
        H.append('</div>')

    # ── Contraindicated — grouped by class ────────────────────────────
    if banned:
        H.append('<div class="section no-break"><div class="section-title">🚫 Contraindicated (Culture-Resistant / Avoid)</div>')
        _banned_groups = _group_banned(banned)
        for cls_name, drugs_in_cls in _banned_groups.items():
            H.append(f'<div style="font-size:8.5pt;margin:1mm 0">'
                     f'<span class="danger-val"><b>{_esc(cls_name)}:</b></span> '
                     f'{_esc(", ".join(drugs_in_cls))}</div>')
        H.append('<div class="alert alert-info" style="font-size:7.5pt;margin-top:2mm">'
                 'See AST table (Page 1) for complete resistance profile</div>')
        H.append('</div>')

    # ── Pregnancy alert (only if pregnant) ───────────────────────────
    if is_preg and preg_warn_items:
        names_preg = ", ".join(d.get("name","") for d in preg_warn_items[:5])
        H.append(f'<div class="section no-break">'
                 f'<div class="section-title">🤰 Pregnancy Considerations</div>'
                 f'<div class="alert alert-warn" style="font-size:8.5pt">'
                 f'<b>Use with caution in pregnancy:</b> {_esc(names_preg)}.<br>'
                 f'Tetracyclines generally avoided. TMP-SMX: depends on trimester. '
                 f'Final decision by treating physician.</div>'
                 f'</div>')

    # ── Renal alerts (only if impaired) ──────────────────────────────
    if is_renal and warned:
        renal_drugs = [d for d in warned if d.get("warning_reason") != "intermediate_culture"]
        if renal_drugs:
            H.append('<div class="section no-break">'
                     '<div class="section-title">🫘 Renal Dose Adjustments</div>')
            for d in renal_drugs[:4]:
                H.append(f'<div style="font-size:8.5pt;margin:1mm 0">'
                         f'<b>{_esc(d.get("name",""))}</b>: {_esc(d.get("renal_note","—"))}</div>')
            H.append('</div>')

    # ── Two-column: Pathogenicity + Duration ──────────────────────────
    H.append('<div class="two-col"><div class="col-l">')
    ...
    if patho_assessment:
        ...
        clr   = _score_color(sc)
        H.append(f'<div class="section-title">Pathogenicity Score</div>')
        H.append(f'<div class="score-wrap">'
                 f'<div class="score-bar-outer"><div class="score-bar-inner" '
                 f'style="width:{sc}%;background:{clr}"></div></div>'
                 f'<div style="font-size:9pt;margin-top:1mm">'
                 f'<b style="color:{clr}">{sc}%</b> — {verd}</div></div>')
        flag_map = {
            "ABU_NO_TREAT":  "ABU — Do NOT Treat (IDSA 2019)",
            "ABU_TREAT":     "ABU — Treat (High-risk context)",
            "MW_REJECT":     "Specimen REJECTED — Repeat required",
            "SIRS_HIGH":     "SIRS ≥3 — Sepsis Probable",
            "PEDIATRIC_UTI": "Pediatric threshold applied",
        }
        for fl, msg in flag_map.items():
            if fl in flags:
                H.append(f'<div class="alert alert-warn" style="margin-top:1mm;font-size:8pt">{msg}</div>')
    ...
    if duration_data:
        ...
        H.append(f'<div class="section-title">Treatment Duration</div>')
        H.append(f'<table class="info-table">'
                 f'<tr><td class="lbl">Protocol</td><td class="val">{_esc(d.get("label",""))}</td></tr>'
                 f'<tr><td class="lbl">Standard</td><td class="val"><b>{std} days</b></td></tr>'
                 f'<tr><td class="lbl">Range</td><td class="val">{rng} days</td></tr>'
                 f'<tr><td class="lbl">IV / PO</td><td class="val ltr">IV:{d.get("iv_days",0)}d · PO:{d.get("po_days",0)}d</td></tr>'
                 f'</table>')
        if d.get("notes"):
            H.append(f'<div class="alert alert-info" style="font-size:7.5pt;margin-top:1mm">{_esc(d["notes"][:150])}</div>')
        if d.get("follow_up_culture"):
            H.append('<div class="alert alert-warn" style="font-size:7.5pt">🔄 Follow-up culture recommended</div>')

    H.append('</div></div>')

    # ── Combination Therapy (MDR) ─────────────────────────────────────
    if combo_suggestions:
        H.append('<div class="section no-break"><div class="section-title">Combination Therapy — MDR Organisms</div>')
        for cs in combo_suggestions[:2]:  # max 2 phenotypes to save space
            data = cs["data"]
            H.append(f'<div class="alert alert-danger" style="margin-bottom:1mm">'
                     f'<b>{_esc(data["urgency"])} — {_esc(data["title"])}</b></div>')
            for opt in data["options"][:3]:  # max 3 options
                avoid = "AVOID" in opt.get("evidence","") or "AVOID" in opt["combo"].upper()
                bg    = "#fdf2f8" if avoid else "#fff"
                brd   = "#922b21" if avoid else "#d5d8dc"
                H.append(f'<div style="background:{bg};border:0.5pt solid {brd};'
                         f'border-radius:2mm;padding:2mm 3mm;margin:1mm 0;font-size:8pt">'
                         f'{"🚫 " if avoid else "• "}<b>{_esc(opt["combo"])}</b> '
                         f'<span style="color:#888">({_esc(opt["evidence"])})</span>'
                         f'{"<br><small style=color:#922b21>"+_esc(opt.get("caution",""))+"</small>" if avoid and opt.get("caution") else ""}'
                         f'</div>')
        H.append('</div>')

    # ── Drug Interactions ─────────────────────────────────────────────
    if interactions:
        H.append('<div class="section no-break"><div class="section-title">Drug Interactions</div>')
        for ia in interactions[:5]:
            H.append(f'<div class="alert alert-warn" style="margin:1mm 0;font-size:8pt">⚠ {_esc(ia)}</div>')
        H.append('</div>')

    # ── Hepatic (Child-Pugh) — compact ────────────────────────────────
    if is_hepatic and hepatic_recs:
        action_recs = [r for r in hepatic_recs if r.get("requires_action")][:4]
        if action_recs:
            H.append(f'<div class="section no-break"><div class="section-title">Hepatic Dosing — Child-Pugh {_esc(child_pugh)}</div>')
            for r in action_recs:
                avoid = "Avoid" in r["level"]
                cls   = "danger-val" if avoid else "warn-val"
                H.append(f'<div style="font-size:8pt;margin:1mm 0">'
                         f'<b class="{cls}">{_esc(r["name"])}</b>: {_esc(r["recommendation"])}</div>')
            H.append('</div>')
...
<div class="two-col">
  <div class="col-l">
    <div style="font-size:7pt;color:#666">
      <b>References:</b><br>
      CLSI M100 2026 | EUCAST 2026 | IDSA AMR 2025<br>
      WHO AWaRe 2025 | Sanford Guide 2025 | BNF 2025<br>
      Egypt National Guidelines
    </div>
  </div>
  <div class="col-r">
    <div style="font-size:7pt;color:#666">
      <b>Disclaimer:</b> This report is a clinical decision support tool
      and does not replace clinical judgment.
      Treatment decisions remain the responsibility of the treating physician.
    </div>
  </div>
</div>
<div style="text-align:center;font-size:7pt;color:#888;margin-top:2mm;border-top:0.5pt solid #ddd;padding-top:1mm">
  Developed by Dr. Hussein Ali &nbsp;|&nbsp; {_esc(lab_name)} &nbsp;|&nbsp; Microbiology CDSS
</div>
</div></body></html>''')

    html_content = "".join(H)
    try:
        return _wp.HTML(string=html_content).write_pdf()
    except Exception:
        return None
...
if uploaded:
    ...
    with col2:
        ...
        if allowed:
            ...
            with st.expander("🏆 Ranked Antibiotic Recommendations", expanded=True):
                st.caption("مرتب حسب: نتيجة المزرعة + WHO AWaRe + طريق الإعطاء + ملاءمة العينة")

                # Color map per AWaRe
                _AWARE_COLORS_UI = {
                    "Access":  ("#1e8449", "#eafaf1", "🟢 Access"),
                    "Watch":   ("#b7770d", "#fef9e7", "🟡 Watch"),
                    "Reserve": ("#922b21", "#fdf2f2", "🔴 Reserve"),
                }
                _RANK_LABELS = {1: "Preferred",  2: "Preferred", 3: "Preferred",
                                4: "Alternative", 5: "Alternative",
                                6: "Reserve", 7: "Reserve", 8: "Reserve"}
                ...
                for i, item in enumerate(ranked[:8], 1):
                    ...
                    note       = (item.get("specimen_notes") or {}).get(culture_type, "")
                    note       = note or item.get("note", "")
                    note_short = note[:90] + "..." if len(note) > 90 else note
                    renal_note = item.get("renal_note", "") if is_renal else ""

                    clr_txt, clr_bg, clr_lbl = _AWARE_COLORS_UI.get(
                        aware, ("#444", "#f8f9fa", aware))

                    # Tier divider
                    tier = _RANK_LABELS.get(i, "Reserve")
                    if tier != prev_tier:
                        st.markdown(
                            f"<div style='margin:6px 0 2px;font-weight:bold;"
                            f"color:{clr_txt};border-top:1px solid {clr_txt};"
                            f"padding-top:4px'>{tier}</div>",
                            unsafe_allow_html=True)
                        prev_tier = tier
                    ...
                    st.markdown(f"""<div style="
                        ...
                        <div>
                          <span style="font-weight:700;font-size:1rem;color:{clr_txt}">
                            {i}. {item['name']}</span>
                          <span style="margin-right:8px;font-size:0.85rem;
                            background:#fff;border:1px solid {clr_txt};
                            border-radius:4px;padding:1px 6px;color:{clr_txt}">
                            {sir_badge}</span>
                          <span style="font-size:0.82rem;color:#555">{route}</span>
                          {"<br><small style='color:#888'>⚠ Renal: "+renal_note+"</small>" if renal_note else ""}
                          {"<br><small style='color:#666'>"+note_short+"</small>" if note_short else ""}
                        </div>
                        <div style="text-align:right;white-space:nowrap">
                          <span style="background:{clr_txt};color:#fff;
        ...
        if final_drugs:
            ...
            _rpt_input_hash = hashlib.md5(
                ...
                f"|{show_commercial}|{_lab}|{_city}".encode()
            ).hexdigest()[:16]
            ...
            if WEASYPRINT_AVAILABLE:
                ...
                # PDF is only generated when user explicitly clicks the button
                if st.button("🔄 توليد تقرير PDF", key="gen_pdf_btn",
                             use_container_width=True,
                             help="اضغط لتوليد التقرير — يستغرق بضع ثوانٍ"):
                    ...
                    with st.spinner("جاري توليد التقرير PDF..."):
                        try:
                            ...
                            if _new_pdf:
                                ...
                                st.success("✅ التقرير جاهز — اضغط زر التنزيل أدناه")
                            else:
                                st.error("فشل توليد PDF — تحقق من تثبيت weasyprint")
                        except Exception as _pdf_err:
                            st.error(f"خطأ في توليد PDF: {_pdf_err}")

                # Download button — always shown if PDF was previously generated
