...
"""
  ...
  <div class="hdr-right">
    ...
    if patho_assessment:
        ...
        if fpos:
            H.append('<div style="font-size:7pt;color:#1e8449;margin-top:1mm"><b>{_T["supporting"]}</b></div>')
            for f in fpos[:3]:
                H.append(f'<div style="font-size:6.5pt;color:#1e8449">{_esc(f[:80])}</div>')
        # Against factors
        if fneg:
            H.append('<div style="font-size:7pt;color:#b7770d;margin-top:0.5mm"><b>{_T["against"]}</b></div>')
            for f in fneg[:3]:
                H.append(f'<div style="font-size:6.5pt;color:#b7770d">{_esc(f[:80])}</div>')
        # Recommendations
        if recs:
            H.append('<div style="font-size:7pt;font-weight:bold;margin-top:1mm">{_T["recs"]}</div>')
            for r in recs[:3]:
