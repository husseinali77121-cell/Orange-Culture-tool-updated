...
<div style="text-align:center;font-size:6.5pt;color:#aaa;margin-top:1mm">
  Dr. Hussein Ali &nbsp;|&nbsp; {_esc(lab_name)} &nbsp;|&nbsp; Microbiology CDSS
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # PAGE 2: RECOMMENDED THERAPY -- RANKED (for Pus + all specimens)
    # ════════════════════════════════════════════════════════════════════════════
    H.append('<div class="pb"></div>')  # page break
    H.append(hdr_html("RECOMMENDED THERAPY"))

    H.append(f'<div class="sec-ttl">{_T["recommended"]}</div>')
    if not ranked:
        H.append('<div class="alert al-info" style="font-size:8pt">'
                 'No recommended antibiotics after applying all clinical restrictions. '
                 'Review culture results and patient factors.</div>')
    else:
        _cur_tier = None
        for _item in ranked:
            _aw = _item.get("aware", "")
            if _aw != _cur_tier:
                _cur_tier = _aw
                _tier_clr = AWARE_CLR.get(_aw, "#888")
                _tier_lbl = TIER_LBL.get(_aw, _aw or "Other")
                H.append(f'<div class="tier-sep" style="color:{_tier_clr};border-top-color:{_tier_clr}">'
                         f'{_tier_lbl}</div>')
            _nm   = _esc(_item.get("name", ""))
            _cls  = _esc(_item.get("class", ""))
            _note = _esc(_item.get("note", ""))
            _route = "🟢 Oral preferred" if _item.get("high_po") else "💉 IV/IM only"
            _card_bg = AWARE_CARD.get(_aw, "background:#f8f9fa;border:0.8pt solid #888")
            _pill_style = AWARE_PILL.get(_aw, "background:#888;color:#fff")
            _sir_tag = ""
            if sir_map and _item.get("name") in sir_map:
                _sv = sir_map[_item["name"]]
                _sir_cls = "sir-s" if _sv == "S" else ("sir-i" if _sv == "I" else "sir-r")
                _sir_tag = f'<span class="{_sir_cls}" style="font-weight:bold;margin-left:1mm">[{_sv}]</span>'
            _spec_note = (_item.get("specimen_notes") or {}).get(specimen, "")
            H.append(
                f'<div class="ranked-row" style="{_card_bg}">'
                f'<div>'
                f'<b style="font-size:9pt">{_nm}</b>{_sir_tag} '
                f'<span style="font-size:6.5pt;color:#888">{_cls}</span><br>'
                f'<span style="font-size:7pt;color:#555">{_note[:140]}</span>'
                + (f'<br><span style="font-size:6.5pt;color:#1a5276">📍 {_esc(_spec_note[:120])}</span>' if _spec_note else "")
                + f'</div>'
                f'<div style="text-align:right;white-space:nowrap">'
                f'<span style="padding:0.2mm 2mm;border-radius:2mm;font-size:6.5pt;font-weight:bold;{_pill_style}">{_aw}</span><br>'
                f'<span style="font-size:6.5pt;color:#555">{_route}</span>'
                + (f'<br><span style="font-size:6pt;color:#b7770d">⚠ Renal adj.</span>' if is_renal and _item.get("renal_limit", 0) > 0 else "")
                + f'</div></div>'
            )
            # Renal note detail
            if is_renal and _item.get("renal_limit", 0) > 0:
                _rn = _esc(_item.get("renal_note", ""))
                if _rn:
                    H.append(f'<div style="font-size:6.5pt;color:#b7770d;margin:0.3mm 0 0 5mm;padding-left:3mm">⚠ {_rn[:120]}</div>')

    # ── Warnings summary on page 2 ──────────────────────────────────────
    if warned:
        H.append(f'<div class="sec-ttl" style="margin-top:3mm;color:#b7770d;border-bottom-color:#b7770d">{_T["dose_adj"]}</div>')
        for _wd in warned:
    ...
    # ── Resistance profile on page 2 (ESBL/MDR for non-urine) ────────────
    if mdr_result and mdr_result.get("level"):
        H.append(f'<div class="sec-ttl" style="margin-top:3mm;color:#922b21;border-bottom-color:#922b21">Resistance Classification</div>')
        _mi2 = MDR_INFO.get(mdr_result["level"], {})
        H.append(f'<div class="alert al-danger" style="font-size:8pt">'
                 f'<b>{_mi2.get("icon","")} {_mi2.get("label","")}</b><br>'
                 f'<span style="font-size:7pt">{_esc(_mi2.get("detail",""))}</span><br>'
                 f'<span style="font-size:7pt">Resistant: {mdr_result["resistant_count"]}/{mdr_result["total_tested"]} categories -- {_esc(", ".join(mdr_result.get("resistant_categories",[])))}</span></div>')
    if esbl_result and esbl_result.get("probability") not in ("low", None):
        ...
        if _ep == "carbapenemase":
            H.append(f'<div class="alert al-danger" style="font-size:8pt"><b>🚨 {_esc(_em2)}</b> (confidence {esbl_result.get("confidence",0)}%)<br>'
                     f'<span style="font-size:7pt">{_esc(esbl_result.get("detail",""))[:160]}</span></div>')
        elif _ep in ("high", "ampc"):
            _lbl2 = "AmpC β-Lactamase" if _ep == "ampc" else "ESBL Producer"
            H.append(f'<div class="alert al-danger" style="font-size:8pt"><b>⚠️ {_lbl2}</b> (confidence {esbl_result.get("confidence",0)}%)<br>'
                     f'<span style="font-size:7pt">{_esc(esbl_result.get("detail",""))[:160]}</span></div>')
        elif _ep == "moderate":
            H.append(f'<div class="alert al-warn" style="font-size:8pt"><b>🔶 ESBL Confirmation Recommended</b> (confidence {esbl_result.get("confidence",0)}%)<br>'
                     f'<span style="font-size:7pt">{_esc(esbl_result.get("detail",""))[:160]}</span></div>')

    # Page 2 footer
    H.append(f"""<hr class="dv" style="margin-top:3mm">
<div style="font-size:6.5pt;color:#666">
  <b>References:</b> CLSI 2026 | EUCAST 2026 | IDSA AMR 2025 | WHO AWaRe 2025 | Sanford 2025 | BNF 2025 | Egypt Nat. Guidelines
</div>
<div style="text-align:center;font-size:6.5pt;color:#aaa;margin-top:1mm">
