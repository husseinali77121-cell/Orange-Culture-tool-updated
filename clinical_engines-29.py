...
from ast_reportability import check_reportability, format_issue
from ast_consistency import (
    QC_RULE_OVERRIDES,
    check_consistency,
)
from ast_consistency import format_issue as format_consistency_issue
...
def _spec_word(s: str, *tokens: str) -> bool:
    """Whole-word match for short/ambiguous specimen tokens.

    The classifier below matches most site keywords by substring, which is fine
    for long, unambiguous ones ("urine", "sputum"). But a handful of the tokens
    are short strings that live INSIDE common words:
        'otic'  ⊂ necr·OTIC        → "necrotic tissue"  mis-classified as throat
        'bal'   ⊂ BAL·anitis       → "balanitis swab"   mis-classified as respiratory
        'ear'   ⊂ sm·EAR / cl·EAR
        'et'    ⊂ (everywhere)
        'gi'    ⊂ sur·GI·cal
    Those go through here so they only fire as standalone words. Longer keywords
    stay on plain substring matching (faster, and safe).
    """
    return any(re.search(r"(?<![a-z])" + re.escape(t) + r"(?![a-z])", s)
               for t in tokens)
...
def classify_specimen(name: str) -> str:
    ...
    if any(k in s for k in ("blood", "haemocult", "hemocult", "bacterem",
                            "septicaem", "septicem")):
        return "blood"
    # 3) Urine  ('msu'/'csu' are short → whole-word)
    if (any(k in s for k in ("urine", "urinary", "midstream"))
            or _spec_word(s, "msu", "csu")):
        return "urine"
    # 4) Genital (before wound → a genital swab is never a "wound").
    #    'balanit' added so "balanitis" is genital, not 'bal'→respiratory.
    #    'hvs' is short → whole-word.
    if (any(k in s for k in ("vagin", "cervi", "endocerv", "urethr", "genital",
                             "semen", "seminal", "prostat", "penile", "vulv",
                             "balanit"))
            or _spec_word(s, "hvs")):
        return "genital"
    # 5) Stool / GI (before wound → 'rectal swab' is stool, not wound).
    #    'gi'/'git' are short → whole-word (so 'surgical' never matches).
    if (any(k in s for k in ("stool", "fecal", "faecal", "rectal", "gastro",
                             "colon"))
            or _spec_word(s, "gi", "git")):
        return "stool"
    # 6) Throat / upper-respiratory-tract & ENT (before wound; no M-W here).
    #    'otic'/'aural'/'ear'/'oral' are short → whole-word, so 'necrotic',
    #    'smear' and the like never trip them.
    if (any(k in s for k in ("throat", "pharyng", "tonsil", "nasal", "nasoph",
                             "nostril", "auricular", "buccal"))
            or _spec_word(s, "aural", "otic", "ear", "oral")):
        return "throat"
    ...
    #    'bal'/'et' are short → whole-word ('balanitis', 'ureter'… never match).
    if (any(k in s for k in ("broncho", "bronch", "tracheal", "endotrach",
                             "lavage", "respir"))
            or _spec_word(s, "bal", "et")):
        return "respiratory"
...
def is_intrinsically_avoided(organism_type: str, drug_name: str, drug_info: Dict[str, Any]) -> bool:
    # ── Authoritative EUCAST intrinsic-resistance table first (single source of
    # truth = clinical_data.INTRINSIC_RESISTANCE). A name-level intrinsic hit
    # routes straight to Avoid (e.g. Doxycycline/Cephalexin for P. aeruginosa),
    # independent of the per-organism ORGANISM_PROFILE["avoid"] list. ──────────
...
def build_banned_item(name: str, category: str, reason_short: str, reason_detail: str,
                      reason_short_en: str = "", reason_detail_en: str = "") -> Dict[str, str]:
    """One entry on the AVOID list, in both languages.

    The English report used to be produced by running the Arabic text through a
    word-substitution table (reports._ar2en) and DELETING whatever the table did
    not know. That turned real clinical statements into fragments — "المزرعة
    أثبتت أن Ceftriaxone لا يثبط نمو الجرثومة" came out as "Ceftriaxone no ." —
    and, worse, occasionally inverted the meaning. Clinical text must be authored
    in each language, never machine-mangled out of the other one, so every
    banned item now carries its own English wording.

    The `*_en` arguments default to the Arabic so an un-migrated call site
    degrades to today's behaviour instead of emitting an empty reason; reports.py
    prefers `*_en` when present and only falls back to _ar2en otherwise.
    """
    return {"name": name, "category": category,
            "reason_short": reason_short, "reason_detail": reason_detail,
            "reason_short_en":  reason_short_en  or reason_short,
            "reason_detail_en": reason_detail_en or reason_detail}
...
def analyze_antibiotics(
    ...
    interp_std: str = "EUCAST",
) -> Tuple[List[Dict], List[Dict], List[Dict], List[Dict], List[str]]:
    """Sort the tested drugs into allowed / warned / banned for this patient.

    `interp_std` ("EUCAST" | "CLSI") decides where an "I" result lands, and the
    two standards genuinely disagree — see the culture_result == "I" block near
    the end of the loop. It is a parameter rather than a module constant because
    a lab can only interpret against one standard per report, and the report has
    to say which.
    """
    allowed:            List[Dict] = []
    warned:             List[Dict] = []
    ...
    # Which of these drugs is this organism intrinsically resistant to? Built
    # once, from the same rule set the QA report uses, so the AVOID list and the
    # QA findings can never disagree about what "intrinsic" means.
    _rep_intrinsic: Dict[str, str] = {}
    for _r in check_reportability(organism_type, sir_map or {}):
        if _r["category"] == "intrinsic":
            for _d in _r["drugs"]:
                _rep_intrinsic[_d] = _r["reason_ar"]
    ...
    for drug in final_drugs:
        ...
        if culture_result == "R":
            # Intrinsic and acquired resistance are both "R", and the old code
            # put them in one undifferentiated bucket — so Tetracycline-R (a real
            # finding about THIS isolate: it acquired a tet gene) and
            # Clindamycin-R (true of every E. coli that has ever existed) printed
            # side by side, identically, on the AVOID list. That reads to a
            # physician as "this organism is resistant to a lot of things" when
            # half the list was never a treatment option to lose. The tag lets
            # every renderer separate the two.
            if _rep_intrinsic and drug in _rep_intrinsic:
                banned.append({
                    **build_banned_item(
                        drug, "intrinsic",
                        "مقاومة جوهرية — ليست نتيجة مزرعة.",
                        f"{organism_type} مقاوم لـ {drug} بطبيعته، وليس باكتساب آلية "
                        f"مقاومة. لم يكن خياراً علاجياً في أي وقت، ولا يُفترض وجوده "
                        f"على اللوحة أصلاً.",
                        reason_short_en="Intrinsic resistance — not a culture finding.",
                        reason_detail_en=(
                            f"{organism_type} is naturally resistant to {drug}; this is not "
                            f"an acquired mechanism. It was never a treatment option, and "
                            f"should not have been on the panel in the first place."),
                    ),
                    "sir_note": "intrinsic",
                    "intrinsic_reason": _rep_intrinsic[drug],
                })
            else:
                banned.append(build_banned_item(
                    ...
                    reason_short_en="Resistant (R) on culture.",
                    reason_detail_en=(
                        f"The culture showed that {drug} does not inhibit growth of this "
                        f"organism. MIC above the therapeutic threshold → risk of "
                        f"treatment failure."),
                ))
            continue
        ...
        if is_intrinsically_avoided(organism_type, drug, info):
            banned.append(build_banned_item(
                ...
                reason_short_en=f"Not active against {organism_type} (intrinsic).",
                reason_detail_en=(f"{drug} has natural resistance or no useful activity "
                                  f"against {organism_type}."),
            ))
            continue
        ...
        if _is_mrsa and any(k in info.get("class", "").lower()
            ...
            banned.append(build_banned_item(
                ...
                reason_short_en="Beta-lactam — does not work against MRSA.",
                reason_detail_en=(
                    "MRSA carries mecA (PBP2a) and is therefore resistant to ALL "
                    "beta-lactams (penicillins, conventional cephalosporins and "
                    "carbapenems) EVEN IF the culture reports susceptible. Treat with "
                    "Vancomycin / Linezolid / Daptomycin per site and susceptibility."),
            ))
            continue
        ...
        # EUCAST v16 report-as-tested (see QC006 / ast_consistency): an S
        # cephalosporin is NOT edited to R on mechanism detection — that is the
        # pre-2017 practice, withdrawn. The treatment nuance is specimen-
        # dependent and is the prescriber's call, mirroring the Cefepime and
        # BLI-combo blocks above:
        ...
        if (_is_esbl_like or _is_carbapenemase) and _is_penicillin_or_ceph(info):
            ...
            banned.append(build_banned_item(
                ...
                "لا يُنصح بالبنسلينات/السيفالوسبورينات في العدوى الجهازية بـ ESBL/AmpC "
                "(أو مع Carbapenemase) حتى لو أظهرت المزرعة حساسية — تأثير اللقاح "
                "(inoculum effect) في العينات غير البولية. الخيار العلاجي = "
                f"{'Colistin / Ceftazidime-Avibactam' if _is_carbapenemase else 'Carbapenem (Meropenem/Ertapenem)'}.",
                reason_short_en=f"Not recommended — {_mech_name}-producing organism.",
                reason_detail_en=(
                    f"This organism produces {_mech_name}. In-vitro susceptibility is "
                    f"reported as tested, but penicillins/cephalosporins are not "
                    f"recommended for systemic ESBL/AmpC (or carbapenemase) infection "
                    f"even if the culture reports susceptible (inoculum effect in "
                    f"non-urine specimens). Treatment of choice = "
                    f"{'Colistin / Ceftazidime-Avibactam' if _is_carbapenemase else 'Carbapenem (Meropenem/Ertapenem)'}."),
            ))
            continue
        ...
        if _is_carbapenemase and _is_carbapenem(info):
            banned.append(build_banned_item(
                ...
                reason_short_en="Not effective — carbapenemase-producing organism.",
                reason_detail_en=(
                    "This organism produces a carbapenemase (KPC/MBL/OXA) and is resistant "
                    "to carbapenems. Use Colistin or Ceftazidime-Avibactam (+/- Aztreonam "
                    "for MBL) according to susceptibility."),
            ))
            continue
        ...
        if age < 18 and not info.get("child_safe", True):
            if "fluoroquinolone" in cls:
                banned.append(build_banned_item(
                    ...
                    reason_short_en="Not suitable under 18 years.",
                ))
                continue
            # Tetracyclines: the age cut-off is 8, not 18. The old code checked
            # `"tetracycline" in cls and age < 8` and then FELL THROUGH to the
            # generic under-18 ban when that was false — so the age<8 test only
            # changed the wording and Doxycycline was banned for a 12- or
            # 17-year-old anyway. From 8 years the tooth-staining risk that
            # justifies the restriction no longer applies and tetracyclines are
            # first-line for several infections, so they must continue into the
            # normal evaluation instead of being banned.
            if "tetracycline" in cls:
                if age < 8:
                    banned.append(build_banned_item(
                        drug, "child", "غير مناسب < 8 سنوات.", CHILD_BAN_REASONS["tetracycline"],
                        reason_short_en="Not suitable under 8 years.",
                    ))
                    continue
                # age >= 8 → allowed; fall through to the normal checks below.
            else:
                banned.append(build_banned_item(
                    drug, "child", "غير مفضل للأطفال.",
                    "يحتاج تقييم متخصص أو لا يُنصح به روتينياً لهذه الفئة العمرية.",
                    reason_short_en="Not preferred in children.",
                    reason_detail_en=("Needs specialist assessment or is not routinely "
            ...
            if erythro_r:
                ...
                if d_test_val == "NEGATIVE":
                    ...
                    banned.append(build_banned_item(
                        ...
                        reason_short_en=f"Inducible clindamycin resistance - {label}",
                        reason_detail_en=(
                            "Erythromycin=R + Clindamycin=S -> possible inducible MLSB "
                            "resistance. Do not use Clindamycin unless a negative D-test "
                            "confirms it. CLSI M100 2026 / EUCAST 2026."),
                    ))
                    continue
        ...
        if ("penicillin" in d_low and "oxacillin" not in d_low
            ...
            if is_staph and culture_result != "S":
                banned.append(build_banned_item(
                    ...
                    reason_short_en="Beta-lactamase (penicillinase) producer",
                    reason_detail_en=(
                        "Over 90% of S. aureus produce penicillinase, so Penicillin is not "
                        "effective. Use Cefazolin or Oxacillin (MSSA), or Vancomycin (MRSA)."),
                ))
                continue
        ...
        if (("trimethoprim" in d_low or "sulfamethoxazole" in d_low or "smx" in d_low)
            ...
            banned.append(build_banned_item(
                ...
                reason_short_en=("Clinically unreliable against Enterococcus "
                                 "(despite in-vitro susceptibility)."),
                reason_detail_en=(
                    "Enterococci take up exogenous folate/thymidine in vivo and bypass "
                    "folate inhibition, so TMP-SMX can test susceptible in the laboratory "
                    "yet fail clinically. It is not relied upon for enterococcal infection "
                    "(CLSI/EUCAST). Use Ampicillin/Amoxicillin, or Vancomycin/Linezolid "
        ...
        # ── Fosfomycin vs Klebsiella: chromosomal fosA ────────────────────────
        # K. pneumoniae carries fosA chromosomally — a glutathione transferase
        # that inactivates fosfomycin. The gene is intrinsic to the species, so
        # a disk or E-test can read "S" while therapy fails: fosA expression is
        # inoculum- and condition-dependent, and in-vitro susceptibility does
        # not predict it. Fosfomycin's headline use is exactly the case this
        # matters for (oral single-dose for uncomplicated cystitis, often
        # reached for BECAUSE the isolate is ESBL) so an unqualified "S" here is
        # the dangerous kind of correct. Not a hard ban — a documented caution
        # that reaches the prescriber, which is what was missing entirely.
        if "fosfomycin" in d_low and _org_in(organism_type, ("Klebsiella",)):
            warned.append({
                "name": drug, **info,
                "warning_reason": "intrinsic_fosa",
                "sir_note": "fosa_klebsiella",
                "sir_note_ar": ("Klebsiella تحمل جين fosA كروموسومياً — قد تفشل "
                                "الاستجابة رغم حساسية المزرعة. لا تعتمد عليه "
                                "كعلاج وحيد؛ فضّل بديلاً مؤكداً."),
                "sir_note_en": ("Klebsiella carries chromosomal fosA — therapy may "
                                "fail despite an S result in vitro. Do not rely on "
                                "fosfomycin monotherapy; prefer a confirmed "
                                "alternative."),
            })
            continue
        ...
        if is_renal and "nitrofurantoin" in d_low and cl_cr < _nf_limit:
            banned.append(build_banned_item(
                ...
                reason_short_en=f"Contraindicated - CrCl {cl_cr:.1f} < {_nf_limit} mL/min",
                reason_detail_en=(
                    f"CrCl = {cl_cr:.1f} mL/min, below the required threshold "
                    f"({_nf_limit} mL/min). Risk of therapeutic failure plus toxic "
                    f"accumulation (EMA/BNF 2025)."),
            ))
            continue
        ...
        if is_preg:
            ...
            if "tetracycline" in cls:
                banned.append(build_banned_item(
                    ...
                    reason_short_en="Contraindicated in pregnancy - tetracyclines.",
                    reason_detail_en=(
                        "Tetracyclines (Doxycycline / Tetracycline / Minocycline / "
                        "Tigecycline):\n"
                        "Deposit in fetal bone and teeth, causing permanent tooth "
                        "discolouration and inhibition of bone growth.\n"
                        "Contraindicated throughout pregnancy, particularly after week 15.\n"
                        "ACOG 2023 / BNF 2025: absolute contraindication.\n"
                        "Alternatives: Azithromycin (atypicals) | Amoxicillin-Clavulanate "
                        "| Cephalosporin."),
                ))
                continue
            ...
            # Data-driven: whatever abx_guidelines marks Banned in pregnancy.
            # (Linezolid used to be hard-banned by NAME here. It is now
            # preg_status="Warn" — Category C, animal-only teratogenicity, NOT an
            # absolute contraindication — so it falls through to block #10 as a
            # physician-decision item, usable when no safer option exists for
            # MRSA/VRE. This matches the abx_guidelines reclassification.)
            if info.get("preg_status") == "Banned":
        ...
        if culture_result == "I":
            # The two standards mean opposite things by "I", and this single
            # branch is where that used to be decided wrongly for everyone.
            #
            #   CLSI    — "I" = Intermediate. Efficacy uncertain; a buffer zone
            #             against technical variation. Prefer a susceptible drug
            #             if one exists. -> warned.
            #   EUCAST  — "I" = "Susceptible, increased exposure" (redefined in
            #             2019). The drug WILL work provided the dosing regimen
            #             delivers the higher exposure: high dose, prolonged or
            #             continuous infusion, or a site where the drug
            #             concentrates (urine). It is a first-line option, not a
            #             fallback. -> allowed, tagged.
            #
            # Sending every "I" to `warned` under a EUCAST footer meant the
            # drugs EUCAST expects to be reported I — piperacillin-tazobactam
            # and cefepime against many Enterobacterales, meropenem and the
            # fluoroquinolones against P. aeruginosa — were pushed out of
            # `allowed` entirely: absent from the ranking, from the option
            # count, from the oral-switch and de-escalation engines, and from
            # the report's recommendation. The prescriber was steered onto a
            # broader agent to avoid a drug that was never unsuitable.
            #
            # The tag is not decoration. An "I" drug given at a STANDARD dose is
            # a treatment failure, so `sir_note` travels with the drug and every
            # renderer must show it.
            if str(interp_std).upper() == "EUCAST":
                allowed.append({
                    "name": drug, **info,
                    "sir_category": "I",
                    "sir_note": "increased_exposure",
                    "sir_note_ar": ("حسّاس بزيادة التعرّض — يتطلب نظام الجرعة الأعلى "
                                    "أو التسريب المُطوّل، وليس الجرعة القياسية."),
                    "sir_note_en": ("Susceptible, increased exposure — requires the "
                                    "high-dose / prolonged-infusion regimen, NOT the "
                                    "standard dose."),
                })
            else:
                warned.append({
                    "name": drug, **info,
                    "sir_category": "I",
                    "warning_reason": "intermediate_culture",
                    "sir_note": "use_with_caution",
                    "sir_note_ar": ("Intermediate (CLSI) — استخدم بحذر. معدّل "
                                    "الاستجابة قد يكون أقل من الحسّاس. قد يكون "
                                    "فعّالاً في المواقع التي يتركّز فيها الدواء "
                                    "(مثل البول) أو بجرعة أعلى من المعتادة. "
                                    "يُفضَّل بديل حسّاس إن وُجد."),
                    "sir_note_en": ("Intermediate (CLSI) — use with caution. Response "
                                    "rates may be lower than for susceptible isolates. "
                                    "May be effective at body sites where the drug is "
                                    "concentrated (e.g. urine) or when a higher than "
                                    "normal dose can be used. Prefer a susceptible "
                                    "alternative where one exists."),
                })
            continue

        allowed.append({"name": drug, **info, "sir_category": culture_result or "S"})
    ...
    if not is_urine:
        ...
        for _nm in sorted(_moved):
            banned.append(build_banned_item(
                ...
                reason_short_en="Urinary agent only - not appropriate for this specimen.",
            ))
...
def classify_mdr(organism: str, sir_map: Dict[str, str]) -> Dict[str, Any]:
    ...
    • Organism-specific category panel (Enterobacterales / Pseudomonas /
      Acinetobacter / gram-positive) via get_mdr_panel()
    • A category counts as NON-SUSCEPTIBLE (toward MDR/XDR) when the isolate is
      non-susceptible (R or I) to ≥1 tested agent in it; it is "susceptible"
      only when EVERY tested agent in it is S. This is the international
      Magiorakos definition. (Requiring ALL agents in a class to be non-S before
      counting the category is stricter than Magiorakos and systematically
      UNDER-counts MDR/XDR — that was the previous behaviour.)
    ...
    • Also returns `conservative_resistant_categories` (the stricter "all agents
      in the class lost" view) as METADATA only — never used to set the level.
    """
    if not sir_map:
        return {
            "level": None, "resistant_categories": [], "susceptible_categories": [],
            "total_tested": 0, "total_categories_evaluable": 0, "resistant_count": 0,
            "single_drug_categories": [], "conservative_resistant_categories": [],
            "reliable": False, "warnings": [], "gram": "",
        }
    ...
    conservative_resistant_cats = []  # "all agents in class lost" — metadata only
    ...
    for cat, drugs in MDR_CATEGORIES.items():
        ...
        # as NON-SUSCEPTIBLE — contributing toward MDR/XDR — when the isolate is
        # non-susceptible (R or I) to AT LEAST ONE tested agent in it. It is
        # "susceptible" only when EVERY tested agent in it is S. (Requiring ALL
        # agents to be non-S before the category counts is stricter than
        # Magiorakos and systematically UNDER-counts MDR/XDR — the previous bug.)
        if any(clean_map.get(d) in ("R", "I") for d in tested):
            resistant_cats.append(cat)
        else:
            susceptible_cats.append(cat)
        # Supplementary conservative view: a class is fully lost only when NO
        # tested agent in it is S. Metadata for the therapeutic read-out and for
        # the PDR test below — it must NEVER drive the MDR/XDR level.
    ...
    # counting, "every category is non-susceptible" (r_count == total) is NOT the
    # same as PDR, because a category can be non-susceptible while still holding
    # one S agent. PDR requires the conservative count (no S in any category) to
    # cover every evaluable category.
...
def predict_esbl(organism: str, sir_map: Dict[str, str]) -> Dict[str, Any]:
    ...
    # Intrinsic-R drugs carry ZERO ESBL/AmpC information. P. aeruginosa is
    # intrinsically R to Ceftriaxone/Cefotaxime (ESBL "primary" markers), so
    # without stripping they falsely triggered "ESBL — high". Strip first, infer
    ...
    if erta_R and (sir_map.get("Meropenem") in ("S", "I")) and not mero_R:
        ...
        # carbapenem most affected by non-carbapenemase mechanisms. So this is a
        # confirm-first signal, not a high-confidence call: keep confidence
        # moderate and name the alternative mechanism in the read-out.
        return {
            "probability": "carbapenemase",
            "confidence": 62,
    ...
    # Requires an ENTEROBACTERALE (is_producer): keeps AmpC/ESBL scoped to
    # Enterobacterales; a non-Enterobacterale (P. aeruginosa/Hafnia) falls
    # through to the producer gate. Cefoxitin is intrinsically resistant here
    # and is stripped before markers, so 3rd-gen cephalosporin R alone is the
    # derepression signal.
    ...
    # (e.g. P. aeruginosa) is NEVER an ESBL producer — this gate is what kills
    # the false "ESBL ALERT" on Pseudomonas even if the intrinsic table is
    # incomplete. Any residual acquired 3rd-gen ceph R is judged on the drug's
    # own AST result downstream (R drug is banned as R regardless).
    if not is_producer:
...
def assess_pathogenicity(
    ...
    if cat == "urine":
        ...
        if pus_val is not None:
            ...
            # Additive, not an elif-chain. A real UTI commonly shows pyuria AND
            # positive nitrites together — the strongest combination there is —
            # but the old chain stopped at the first match, so the second finding
            # scored nothing and could not be entered anyway (the UI was a
            # single-select). Substring tests, so the UI is free to reword the
            # labels without silently detaching them from the engine.
            _ua = (urinalysis_result or "").lower()
            if "طبيعي" in (urinalysis_result or "") or "normal" in _ua:
                score -= 25
                factors_neg.append("❌ Normal urinalysis — strongly suggests contamination")
            else:
                if "pyuria" in _ua or "wbc" in _ua:
                    score += 15
                    factors_pos.append("✅ Pyuria noted on urinalysis")
                if "nitrit" in _ua:
                    score += 10
                    factors_pos.append("➕ Nitrites positive — bacterial activity")
                # Hematuria was offered in the UI and read by nothing: picking it
                # scored exactly zero. It is weak evidence on its own (stones,
                # tumours and menstruation all cause it) but it does support
                # cystitis alongside other findings, so it now carries a small
                # weight instead of being silently discarded.
                if "hematuria" in _ua or "haematuria" in _ua or "دم" in (urinalysis_result or ""):
                    score += 5
                    factors_pos.append("➕ Hematuria — يدعم التهاب المثانة (غير نوعي)")
...
def _parse_pus(text: str):
    """Max WBC/HPF stated in `text`, or None when no count is given.

    TNTC ("too numerous to count") is the most florid pyuria a microscopist can
    report — and it contains no digits, so the old version returned None for it
    and the caller fell through to the weaker urinalysis branch. A field that
    says "the WBCs are uncountable" scored as "no pus cell count entered".
    We map it to a sentinel above every threshold.
    """
    if not text:
        return None
    t = str(text).strip().lower()
    if re.search(r"tntc|too\s+numerous|innumerable|uncountable|numerous", t):
        return 999
    if re.search(r"\bnil\b|\bnone\b|\babsent\b|لا\s*يوجد", t):
        return 0
...
def run_ast_qc(organism: str, sir_map: Dict[str, str]) -> List[Dict[str, Any]]:
    """Check the reported S/I/R panel and return every problem found.

    Two independent passes:

      1. REPORTABILITY — should this agent be on this organism's panel at all?
         Intrinsic resistance, agents with no breakpoints for this organism, and
         agents that test susceptible but fail in vivo. This runs FIRST because
         a result that should never have been produced is a bigger problem than
         one that merely contradicts another result, and because the intrinsic
         data was already in the codebase (INTRINSIC_RESISTANCE) being used
         SILENTLY by _remove_intrinsic_resistance() to clean the MDR count —
         known to the software, never told to the user.

      2. INTERNAL CONSISTENCY — do the results contradict each other?
         (AST_QC_RULES: cephalosporin discrepancies, carbapenem logic, and so on.)
    """
    if not sir_map:
        return []
    issues = []
    ...
    # ── Pass 1: reportability (see ast_reportability.py) ──────────────────────
    for _rep in check_reportability(organism, sir_map):
        _fmt = format_issue(_rep, lang="ar")
        issues.append({
            "id":        _rep["id"],
            "severity":  _rep["severity"],
            "message":   _fmt["message"],
            "fix":       _fmt["fix"],
            "category":  _rep["category"],
            "drugs":     _rep["drugs"],
            "reference": _rep["reference"],
        })

    # ── Pass 2: internal consistency — do the results contradict each other?
    # Ahead of the phenotype rules on purpose: an ESBL/AmpC pattern read off a
    # panel that contradicts itself is an interpretation of noise. If
    # cefotaxime and ceftriaxone disagree, "possible ESBL" is the wrong headline
    # — "repeat the test" is.
    for _dis in check_consistency(organism, sir_map):
        _fmt = format_consistency_issue(_dis, lang="ar")
        issues.append({
            "id":        _dis["id"],
            "severity":  _dis["severity"],
            "message":   _fmt["message"],
            "fix":       _fmt["fix"],
            "category":  _dis["category"],
            "drugs":     _dis["drugs"],
            "reference": _dis["reference"],
        })

    # ── Pass 3: phenotype rules (AST_QC_RULES, clinical_data.py) ─────────────
    for rule in AST_QC_RULES:
        ...
        try:
            if rule["condition"](sir_map):
                # ORDER MATTERS. The override is picked FIRST and the placeholders
                # are filled AFTERWARDS. Doing it the other way round (the old
                # code) meant the override's static text replaced the message that
                # had just been correctly populated with the real drug names, so
                # QC006 always printed the two drugs hardcoded in the override —
                # even when neither was on the panel. Any override text must
                # therefore carry the same {drugs}/{r_drug} placeholders as the
                # rule it overrides (see QC_RULE_OVERRIDES in ast_consistency.py).
                _ovr = QC_RULE_OVERRIDES.get(rule["id"])
                if _ovr:
                    logger.info("QC rule %s served from override layer "
                                "(fix at source in clinical_data.py)", rule["id"])
                _msg = (_ovr or {}).get("message", rule["message"])
                _fix = (_ovr or {}).get("fix",     rule["fix"])
                # استبدال {r_drug} (بـ str.replace وليس format) بالدواء المقاوم الفعلي
