...
"""
  ...
  <div class="hdr-right">
    ...
    # ════════════════════════════════════════════════════════════════
    H.append(hdr_html("CLINICAL DECISION SUPPORT"))
  ...
  <div>{pill(aw, AWARE_PILL.get(aw,""))}</div>
    ...
    # 2-column equal: Pathogenicity + Treatment Duration
    H.append('<div class="grid2" style="margin-top:2mm">')

    # ── Pathogenicity (expanded = same height as duration) ────────────────
    ...
    if patho_assessment:
        ...
        if fpos:
            H.append('<div style="font-size:7pt;color:#1e8449;margin-top:1mm"><b>Supporting:</b></div>')
            for f in fpos[:3]:
                H.append(f'<div style="font-size:6.5pt;color:#1e8449">{_esc(f[:80])}</div>')
        # Against factors
        if fneg:
            H.append('<div style="font-size:7pt;color:#b7770d;margin-top:0.5mm"><b>Against:</b></div>')
            for f in fneg[:3]:
                H.append(f'<div style="font-size:6.5pt;color:#b7770d">{_esc(f[:80])}</div>')
        # Recommendations
        if recs:
            H.append('<div style="font-size:7pt;font-weight:bold;margin-top:1mm">Recommendations:</div>')
            for r in recs[:3]:
...
def generate_decision_tree_image(
    ...
    # ── Font system: Arabic-capable if available, else Latin fallback ───────
    _arabic_font_loaded = [False]   # mutable flag — tracks if Arabic font loaded

    def gf(size: int, bold: bool = False) -> Any:
        _b   = "Bold" if bold else "Regular"
        _ARABIC_FONT_PATHS = [
            # Amiri — best for medical reports (Arabic + Latin)
            f"/usr/share/fonts/opentype/fonts-hosny-amiri/Amiri-{_b}.ttf",
            f"/usr/share/fonts/truetype/amiri/Amiri-{_b}.ttf",
            f"/usr/share/fonts/amiri/Amiri-{_b}.ttf",
            # Noto Naskh Arabic
        ...
        for p in _ARABIC_FONT_PATHS:
            try:
                ...
                _arabic_font_loaded[0] = True
                return f
            except Exception:
                continue
        # Fall back to Latin-only fonts
        for p in _LATIN_FALLBACKS:
            try:
    ...
    def _arabic(text: str) -> str:
        """
        Prepare Arabic text for Pillow.
        • If Arabic font loaded (Amiri/Noto): reshape + bidi → proper joining
        • Otherwise: return raw text (Liberation/DejaVu render basic Arabic as-is)
        This matches the previous behavior when Arabic fonts aren't available.
        """
        if not text:
            return text
        _has_ar = any("\u0600" <= ch <= "\u06ff" for ch in text)
        if not _has_ar:
            return text
        if not _arabic_font_loaded[0]:
            # No Arabic font → return raw text (renders like before, basic Arabic)
            return text
        try:
    ...
    _org_display = _arabic(organism)
    ow = tw(draw, _org_display, F_ORG)
    draw.text(((CB[0]+CB[2]-ow)//2, CB[1]+38*S), _org_display, fill=NAVY, font=F_ORG)
    ...
    _spec_type = specimen
    _spec_label = f"Specimen:     {_spec_type}"
    spec_items = [
        _spec_label,
    ...
    if microbiologist:
        spec_items.append(f"Microbiologist: Dr/ {_arabic(microbiologist)}")
    if age < 18:
        spec_items.append(f"Patient Age:  Pediatric ({age}y)")
    ...
    for ref in refs:
        ...
        ry = text_wrap(draw, fx1+10*S, ry, f"• {ref}",
                       F_SMALL, DARK, fx2-fx1-18*S, gap=3)
    # ── Background watermark — very light for print (ink-saving) ─────────────
    try:
        _wm_font = gf(48, True)
        _wm_txt  = f"{lab_name.upper()}  —  MICROBIOLOGY DEPARTMENT"
        _wm_w    = tw(draw, _wm_txt, _wm_font)
        _wm_x    = (W - int(_wm_w)) // 2
        _wm_y    = H // 2 - fh(_wm_font)
        # Very light gray — visible but not distracting when printed
        _wm_clr  = (225, 228, 232)
        draw.text((_wm_x, _wm_y), _wm_txt, fill=_wm_clr, font=_wm_font)
    except Exception:
        pass
...
st.session_state.lab_city = st.secrets.get("lab_city", "Giza") if hasattr(st, "secrets") else st.session_state.get("lab_city", "Giza")
