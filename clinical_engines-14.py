    ...
    for rule in AST_QC_RULES:
        ...
        try:
            if rule["condition"](sir_map):
                ...
                _fix = rule["fix"]
                # Corrections to rules whose text is wrong at source — see
                # QC_RULE_OVERRIDES in ast_consistency.py. Delete the override
                # once clinical_data.py is fixed.
                ...
                if _ovr:
                    ...
                    _msg = _ovr.get("message", _msg)
                    _fix = _ovr.get("fix", _fix)
                issues.append({
                    "id":       rule["id"],
