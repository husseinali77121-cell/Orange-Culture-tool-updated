...
def generate_decision_tree_image(
    ...
    def gf(size: int, bold: bool = False):
        """
        Font loader — Liberation Sans first (clean sans-serif, same as old images).
        DejaVu as fallback. Both support basic Arabic Unicode (disconnected letters).
        This matches the original font style that the user approved.
        """
        import os as _os
        _b = "Bold" if bold else "Regular"
        paths = [
            # Liberation Sans — PRIMARY (same as original working images)
        ...
        for p in paths:
            try:
                ...
                continue
        # Auto-discover any Liberation or DejaVu font
        for _fdir in ["/usr/share/fonts/truetype", "/usr/share/fonts"]:
            try:
                for _root, _, _files in _os.walk(_fdir):
                    for _f in _files:
                        if _f.lower().endswith(".ttf") and any(
                                k in _f.lower() for k in
                                ("liberationsans", "dejavusans", "freesans", "ubuntu")):
                            try:
