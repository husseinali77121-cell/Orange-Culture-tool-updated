...
"""
  ...
  <div class="hdr-right">
    ...
    if patho_assessment:
        ...
        if fpos:
            H.append('<div style="font-size:8.5pt;color:#1e8449;margin-top:1mmf"><b>{_T["supporting"]}</b></div>')
            for f in fpos[:3]:
                H.append(f'<div style="font-size:8.5pt;color:#1e8449">{_esc(f[:80])}</div>')
        # Against factors
        if fneg:
            H.append('<div style="font-size:8.5pt;color:#b7770d;margin-top:0.5mmf"><b>{_T["against"]}</b></div>')
            for f in fneg[:3]:
