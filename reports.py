...
def generate_pdf_html_report(
    ...
    # QC findings, so a self-contradicting panel cannot reach a prescriber
    # looking clean. Optional: no list, no banner, same PDF as before.
    qc_issues: list = None,
) -> Optional[bytes]:
...
"""
  ...
  <div class="hdr-right">
    ...
    # ── PANEL UNDER REVIEW ───────────────────────────────────────────────
    # Nothing previously connected the QA engine to the physician's report, so a
    # panel that contradicted itself — cefotaxime R with ceftriaxone S on one
    # E. coli, where one of those two results is simply wrong — produced a clean,
    # confident, ranked recommendation with no hint on the page that the lab had
    # already flagged it. The QA engine catching it is worth nothing if the only
    # person who acts on the report never learns. Errors only: warnings are for
    # the internal QA archive, not for a prescriber's desk.
    _qc_errors = [q for q in qc_issues if q.get("severity") == "error"]
    if _qc_errors:
        _hdr = ("PANEL UNDER REVIEW — interpret with caution" if _EN else
                "PANEL UNDER REVIEW — interpret with caution")
        _sub = ("Internal quality control has flagged the finding(s) below on this "
                "panel. Affected agents may be re-tested and the result may change. "
                "Correlate clinically before acting on the ranking."
                if _EN else
                "مراجعة الجودة الداخلية رصدت الملاحظات التالية على هذه اللوحة. "
                "قد تُعاد الأقراص المتأثرة وقد تتغيّر النتيجة. راجع الحالة "
                "إكلينيكياً قبل الاعتماد على الترتيب.")
        _rows = []
        for _q in _qc_errors[:4]:
            _dr = ", ".join(_esc(d) for d in (_q.get("drugs") or [])) or "—"
            _rows.append(
                f'<div style="margin-top:0.5mm">• <b>{_dr}</b></div>')
        H.append(
            '<div style="border:0.8pt solid #922b21;background:#fdedec;'
            'border-radius:1.5mm;padding:1.4mm 2.4mm;margin-bottom:1.6mm">'
            f'<div style="font-weight:700;color:#922b21;font-size:9pt">⚠ {_hdr}</div>'
            f'<div style="font-size:7.8pt;color:#7b241c;margin-top:0.5mm;'
            f'line-height:1.5">{_sub}</div>'
            f'<div style="font-size:7.8pt;color:#7b241c">{"".join(_rows)}</div>'
            '</div>'
        )
    ...
    if banned:
        ...
        _intrinsic_bd = [b for b in banned if b.get("category") == "intrinsic"]
        ...
        if _intrinsic_bd:
            _intr_note = (
                "Intrinsic resistance — a property of the species, NOT a finding "
                "from this culture. These agents were never a therapeutic option "
                "for this organism and should not be on the panel."
                if _EN else
                "مقاومة جوهرية — صفة في نوع الجرثومة، **وليست نتيجة هذه المزرعة**. "
                "لم تكن خياراً علاجياً في أي وقت، ولا يُفترض وجودها على اللوحة."
            )
            H.append(
                '<div style="border:0.4pt solid #5d6d7e;background:#f4f6f7;'
                'border-radius:1.5mm;padding:1mm 2mm;margin-top:1mm;'
                'font-size:8.5pt;line-height:1.6">'
                f'<div style="font-size:7.5pt;color:#5d6d7e;margin-bottom:0.6mm">'
                f'🧬 {_intr_note}</div>'
                f'{"".join(_chip(b) for b in _intrinsic_bd)}'
                '<div style="font-size:6.8pt;color:#85929e;margin-top:0.6mm">'
                'EUCAST Intrinsic Resistance v3.3 · CLSI M100 Ed36 App. B</div>'
                '</div>'
            )
