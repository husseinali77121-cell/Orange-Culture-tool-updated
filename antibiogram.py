...
def _safe_dataframe(data) -> None:
    """Render a table robustly across Streamlit versions.

    `hide_index` (Streamlit ≥1.23) and `use_container_width` (≥1.16) are not
    available on older builds and raise TypeError there — which would crash the
    whole page. Degrade cleanly instead, and fall back to a static table as a
    last resort so a table can never take the app down.
    """
    try:
