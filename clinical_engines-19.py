    ...
    for drug in final_drugs:
        ...
        if (_is_esbl_like or _is_carbapenemase) and _is_penicillin_or_ceph(info):
            ...
            banned.append(build_banned_item(
                ...
                reason_short_en=f"Not effective — {_mech_name}-producing organism.",
                reason_detail_en=(
                    f"This organism produces {_mech_name}: resistant to all penicillins "
                    f"and cephalosporins (including Ampicillin/Amoxicillin and "
                    f"generations 1-4) EVEN IF the culture reports susceptible "
                    f"(inoculum effect). Treatment of choice = "
                    f"{'Colistin / Ceftazidime-Avibactam' if _is_carbapenemase else 'Carbapenem (Meropenem/Ertapenem)'}."),
            ))
            continue
