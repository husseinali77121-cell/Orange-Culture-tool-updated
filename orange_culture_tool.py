...
# ── Cephradine — add to ABX_GUIDELINES at runtime if not present ──
def _inject_cephradine():
    if "Cephradine" not in ABX_GUIDELINES:
        ...
        # Add aliases
        for alias in ["cephradine", "velosef", "sefril", "eskacef", "cefradine"]:
            ABX_ALIAS_INDEX[alias] = "Cephradine"
...
def load_commercial_names(filepath: str = "commercial_names.txt") -> Dict[str, str]:
    """
    يقرأ ملف commercial_names.txt ويرجع dict:
    {generic_name_lower: "Brand1 / Brand2 / Brand3"}
    """
    result: Dict[str, str] = {}
    try:
...
def get_commercial_name(generic: str) -> str:
    """يرجع الأسماء التجارية للدواء أو string فاضي لو مش موجود"""
...
def init_session_state() -> None:
    defaults = {
        ...
        # ─── Commercial Names in Report ──────────────────────────────────
        "show_commercial_names": False,
        # ─── Pathogenicity Assessment v2 ────────────────────────────────
...
def generate_decision_tree_image(
    ...
    # ── 5. ROW 2: Specimen | Microscopic Exam | First-Line | Pathogenicity ──────
    R2_Y1 = 210*S
    R2_Y2 = 310*S

    # 4 boxes if patho_assessment available, else 3
    if patho_assessment:
        r2w = (W - 2*P - 3*G) // 4
    else:
        r2w = (W - 2*P - 2*G) // 3
    ...
    # Pathogenicity box (4th box in Row 2)
    if patho_assessment:
        pa_sc    = patho_assessment.get("score", 0)
        pa_flags = patho_assessment.get("special_flags", [])
        pa_verd  = patho_assessment.get("verdict", "")
        # strip emoji for image rendering
        import re as _re
        pa_verd_clean = _re.sub(r'[^-]+', '', pa_verd).strip(" -")

        # color based on score
        if pa_sc >= 75:
            pa_bd, pa_bg, pa_tc = RED_BD, RED_BG, RED_TXT
        elif pa_sc >= 50:
            pa_bd, pa_bg, pa_tc = AMBER_BD, AMBER_BG, AMBER_TXT
        elif pa_sc >= 30:
            pa_bd, pa_bg, pa_tc = ALERT_BD, ALERT_BG, ALERT_TXT
        else:
            pa_bd, pa_bg, pa_tc = GREEN_BD, GREEN_BG, GREEN_TXT

        px1 = P + 3*(r2w+G)
        px2 = W - P
        rbox(draw, (px1, R2_Y1, px2, R2_Y2), pa_bg, pa_bd, radius=12, width=3)
        draw.text((px1+12*S, R2_Y1+9*S), "PATHOGENICITY", fill=pa_bd, font=F_SUBTITL)
        ...
        fill_w = int(bar_w * pa_sc / 100)
        if fill_w > 0:
            draw.rounded_rectangle([bar_x1, bar_y, bar_x1+fill_w, bar_y+bar_h],
                                    radius=4, fill=pa_bd)

        # Score text
        sc_txt = f"Score: {pa_sc}%"
        draw.text((px1+12*S, bar_y + bar_h + 4*S), sc_txt, fill=pa_tc, font=F_TEXT)

        # Verdict short
        vy = bar_y + bar_h + 4*S + fh(F_TEXT) + 4*S
        text_wrap(draw, px1+12*S, vy, pa_verd_clean,
                  F_SMALL, pa_tc, px2-px1-22*S, gap=3)

        # Special flags
        fy = vy + fh(F_SMALL) + 18*S
        flag_labels = {
            "ABU_NO_TREAT":  "ABU: Do NOT Treat",
            "ABU_TREAT":     "ABU: TREAT (high-risk)",
            "MW_REJECT":     "Specimen: REJECT",
            "MW_ADEQUATE":   "Specimen: Adequate",
            "SIRS_HIGH":     "SIRS>=3: Sepsis",
            "PEDIATRIC_UTI": "Pediatric Threshold",
        }
        for fl in pa_flags:
            if fl in flag_labels and fy + fh(F_SMALL) + 3*S < R2_Y2 - 6*S:
                draw.text((px1+12*S, fy), f"! {flag_labels[fl]}", fill=pa_tc, font=F_SMALL)
                fy += fh(F_SMALL) + 4*S
    ...
    # ── Pathogenicity Assessment Section ──────────────────────────────
    if patho_assessment:
        ...
        if "MW_REJECT" in flags:
            L.append("FLAG     : Murray-Washington — Specimen REJECTED (repeat required)")
    ...
    with col2:
        ...
        if final_drugs:
            ...
            if show_commercial and not COMMERCIAL_NAMES:
                st.warning("⚠️ ملف `commercial_names.txt` غير موجود في مجلد البرنامج — أضفه لتفعيل هذه الخاصية.")
