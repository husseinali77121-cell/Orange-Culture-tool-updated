...
def _register_pdf_fonts():
    """Register Liberation Sans fonts for PDF."""
    from reportlab.pdfbase import pdfmetrics
    from reportlab.pdfbase.ttfonts import TTFont
    font_base = "/usr/share/fonts/truetype/liberation/LiberationSans"
    try:
        pdfmetrics.registerFont(TTFont("RPT",    f"{font_base}-Regular.ttf"))
        pdfmetrics.registerFont(TTFont("RPT-B",  f"{font_base}-Bold.ttf"))
        pdfmetrics.registerFont(TTFont("RPT-I",  f"{font_base}-Italic.ttf"))
        pdfmetrics.registerFont(TTFont("RPT-BI", f"{font_base}-BoldItalic.ttf"))
        return True
    except Exception:
        return False
...
def generate_pdf_report(
    ...
    header_data = [[
        Paragraph(f"<b>🔬  {lab_display}</b>", sTitle),
    ...
    if mdr_r.get("level"):
        ...
        alert_rows.append((
            f"{'🚨' if lvl=='PDR' else '🔴' if lvl=='XDR' else '⚠️'}  {lvl} — "
    ...
    if prob == "carbapenemase":
        ...
        alert_rows.append(("⚠️  High Probability ESBL Producer",
    ...
    # ══════════════════════════════════════════════════════
    if allowed:
        story.append(section_header(f"✅  RECOMMENDED ANTIBIOTICS  ({len(allowed)} options)", GREEN))
        story.append(Spacer(1, 2*mm))
    ...
    # ══════════════════════════════════════════════════════
    if warned:
        story.append(section_header(f"⚠️  USE WITH CAUTION  ({len(warned)} antibiotics)", AMBER))
        story.append(Spacer(1, 2*mm))
    ...
    if is_preg and age >= 18 and preg_warn_items:
        story.append(section_header("🤰  PREGNANCY — USE WITH CAUTION", PURPLE))
        story.append(Spacer(1, 2*mm))
    ...
    # ══════════════════════════════════════════════════════
    if banned:
        story.append(section_header(f"🚫  CONTRAINDICATED / INEFFECTIVE  ({len(banned)} antibiotics)", RED))
        story.append(Spacer(1, 2*mm))
