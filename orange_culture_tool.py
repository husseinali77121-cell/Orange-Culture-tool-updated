...
def generate_decision_tree_image(
    ...
    # عنوان مصغر قليلاً ليكون متسقاً مع المحتوى
    ...
    if "klebsiella" in org_l:
        ...
        alerts += ["High MDR risk",
                   "Check carbapenem sensitivity"]
    if is_renal:
    ...
    # نستخدم F_SMALL بدل F_TEXT للنص ليبقى داخل المربع
    ...
    for al in alerts[:5]:
        ...
        ay = text_wrap(draw, AB[0]+12*S, ay, f"• {al}",
                       F_SMALL, DARK, alert_max_w, gap=4)
        ay += 2*S
...
def _register_pdf_fonts() -> bool:
    """
    Register Liberation Sans + addMapping so <b>/<i> tags work.
    Falls back to Helvetica (built-in) if fonts not found on server.
    """
    from reportlab.pdfbase import pdfmetrics
    from reportlab.pdfbase.ttfonts import TTFont
    from reportlab.lib.fonts import addMapping

    font_base = "/usr/share/fonts/truetype/liberation/LiberationSans"
    alt_base  = "/usr/share/fonts/truetype/dejavu/DejaVuSans"

    # Try Liberation Sans first, then DejaVu, then give up (Helvetica fallback)
    for base, suffix_map in [
        (font_base, {"R": "-Regular", "B": "-Bold", "I": "-Italic", "BI": "-BoldItalic"}),
        (alt_base,  {"R": "",         "B": "-Bold", "I": "-Oblique","BI": "-BoldOblique"}),
    ]:
        try:
            pdfmetrics.registerFont(TTFont("RPT",    f"{base}{suffix_map['R']}.ttf"))
            pdfmetrics.registerFont(TTFont("RPT-B",  f"{base}{suffix_map['B']}.ttf"))
            pdfmetrics.registerFont(TTFont("RPT-I",  f"{base}{suffix_map['I']}.ttf"))
            pdfmetrics.registerFont(TTFont("RPT-BI", f"{base}{suffix_map['BI']}.ttf"))
            # CRITICAL: register family mapping so <b> and <i> tags resolve correctly
            addMapping("RPT", 0, 0, "RPT")
            addMapping("RPT", 1, 0, "RPT-B")
            addMapping("RPT", 0, 1, "RPT-I")
            addMapping("RPT", 1, 1, "RPT-BI")
            return True
        except Exception:
            continue

    # Final fallback: use Helvetica (always available in ReportLab)
    # Map RPT → Helvetica so styles still work
    try:
        addMapping("RPT", 0, 0, "Helvetica")
        addMapping("RPT", 1, 0, "Helvetica-Bold")
        addMapping("RPT", 0, 1, "Helvetica-Oblique")
        addMapping("RPT", 1, 1, "Helvetica-BoldOblique")
    except Exception:
        pass
    return False
...
def generate_pdf_report(
    ...
    header_data = [[
        Paragraph(f"<b>[LAB]  {cpdf(lab_display)}</b>", sTitle),
    ...
    culture_cell = [
        ...
        Paragraph(f'<b><font size="18" color="#04193F">{organism}</font></b>',
    ...
    if sir_map:
        ...
        if len(drugs_list) > 6:
            ...
            def mini_sir_table(items):
                rows = [[Paragraph("<b>Antibiotic</b>",    sBold),
                         Paragraph("<b>S/I/R</b>",         sBold)]]
    ...
    footer_data = [[
        Paragraph(
            "<b>DISCLAIMER:</b> هذا التقرير أداة مساعدة للقرار الطبي — "
...
if uploaded:
    ...
    with col2:
        ...
        if final_drugs:
            ...
            st.markdown("### 📋 التقرير السريري — PDF")
            st.caption("يتحدث فوراً مع كل تغيير • جاهز للطباعة والإرسال")
            ...
            if REPORTLAB_OK:
                ...
                if pdf_ok:
                    # معاينة مدمجة داخل التطبيق
                    import base64 as _b64r
                    _b64pdf = _b64r.b64encode(pdf_bytes).decode()
                    pdf_preview_html = f"""
<iframe
  src="data:application/pdf;base64,{_b64pdf}"
  width="100%" height="480px"
  style="border:1px solid #ddd; border-radius:8px;"
></iframe>"""
                    st.markdown(pdf_preview_html, unsafe_allow_html=True)
                    st.caption("↑ معاينة — ادفع تنزيل للحفظ أو فتح للطباعة")

                    pdf_fname = (f"Orange_{organism_type.replace(' ','_')}_"
                ...
                st.warning("⚠️ أضف `reportlab` لـ requirements.txt لتفعيل PDF")
                # fallback: generate_report للـ TXT
                auto_report = generate_report(
                    patient_name=_pt, age=age, sex=sex, weight=weight,
                    cl_cr=cl_cr, is_renal=is_renal,
                    is_preg=is_preg, is_hepatic=is_hepatic,
                    allowed=allowed, warned=warned, banned=banned,
                    preg_warn_items=preg_warn_items,
                    organism=organism_type, specimen=culture_type,
                    interactions=interactions_alerts, sir_map=sir_map,
                    colony_count=colony_count, date_in=str(date_in),
                    pus_cells=pus_cells_text, rbcs=rbcs_text,
                    lab_name=_lab, lab_city=_city,
                )
                st.download_button(
                    "📥 تنزيل التقرير (TXT — fallback)",
                    data=auto_report,
