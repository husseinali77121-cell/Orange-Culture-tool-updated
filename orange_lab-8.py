...
HEPATIC_DOSING: Dict[str, Dict] = {
    ...
    "Ertapenem":                     {"A": ("Normal","No adjustment"), "B": ("Normal","No adjustment"), "C": ("Normal","No formal hepatic adjustment"), "note": "Primarily renal"},
}
...
/* Header — compact */
.hdr { background:#0d3b66; color:#fff; padding:2.5mm 8mm 2mm; display:flex;
...
.accent   { height:1.5mm; background:#ff8c00; }
.content  { padding: 1mm 0; }
/* Micro info grid */
...
.sec-ttl { font-size:8.5pt; font-weight:bold; color:#0d3b66; text-transform:uppercase;
            border-bottom:1pt solid #0d3b66; padding-bottom:0.3mm; margin:2mm 0 1mm;
...
/* Two-column grid */
.grid2 { display:table; width:100%; border-spacing:1.5mm; border-collapse:separate; direction:ltr; }
...
.tier-sep { font-size:7.5pt; font-weight:bold; text-transform:uppercase; letter-spacing:0.3pt;
            direction:ltr; text-align:left;
            padding:0.3mm 0; margin-top:1.5mm; border-top:0.8pt solid; }
/* Alerts — tighter */
.alert { padding:1mm 2.5mm; border-radius:1.5mm; margin:0.5mm 0; font-size:8.5pt; direction:ltr; text-align:left; }
...
hr.dv { border:none; border-top:0.4pt solid #d5d8dc; margin:1mm 0; }
"""
  ...
  <div class="hdr-right">
    ...
    # PDF: combine allowed + preg_warn_items (sensitive drugs that need caution)
    _pdf_ranked_all = list(ranked)
    _pdf_preg_warn_names = {pw.get("name","") for pw in preg_warn_items}
    # Add preg_warn drugs that are sensitive (not already in ranked)
    _ranked_names = {d.get("name","") for d in _pdf_ranked_all}
    for _pw in preg_warn_items:
        _pw_sir = sir_map.get(_pw.get("name",""), "S")
        if _pw.get("name","") not in _ranked_names and _pw_sir not in ("R",""):
            _pdf_ranked_all.append(_pw)

    if _pdf_ranked_all:
        H.append('<div class="sec-ttl">Recommended Therapy — Ranked</div>')
        prev_tier = ""
        for i, _rd in enumerate(
            sorted(_pdf_ranked_all,
                   key=lambda x: {"Access":0,"Watch":1,"Reserve":2}.get(x.get("aware",""),3)),
            1
        ):
            _raw  = _rd.get("aware","")
            ...
            _is_preg_caution = _rd.get("name","") in _pdf_preg_warn_names
            if _tlbl != prev_tier:
                H.append(f'<div class="tier-sep" style="color:{_clr};border-color:{_clr}">{_tlbl}</div>')
                prev_tier = _tlbl
            # Pregnancy caution badge
            _preg_badge = (
                '&ensp;<span style="background:#7d3c98;color:#fff;font-size:7.5pt;'
                'padding:0.2mm 2mm;border-radius:1mm">🤰 Caution</span>'
                if _is_preg_caution else ""
            )
            H.append(
                f'<div class="ranked-row" style="{_ccss};border-radius:1.5mm;padding:1.5mm 3mm;margin:0.5mm 0">'
                f'<div style="flex:1">'
        ...
        H.append('<div class="alert al-info" style="font-size:8.5pt">'
                 'No active options available — all sensitive agents require clinical caution. '
                 'See sections below for guidance.</div>')

    # ── AVOID — grouped by class ──────────────────────────────────────────
    if banned:
        from collections import defaultdict
        _ban_grp = defaultdict(list)
        for _bd in banned:
            _ban_grp[_bd.get("class","Other")].append(_bd.get("name",""))
        _rows_txt = " &nbsp;|&nbsp; ".join(
            f"<b>{_esc(k)}:</b> {_esc(', '.join(vs))}"
            for k, vs in _ban_grp.items()
        )
        H.append(
            '<div class="sec-ttl" style="margin-top:1mm;color:#922b21;border-bottom-color:#922b21">'
            '🚫 Avoid / Contraindicated</div>'
            f'<div class="alert al-danger" style="font-size:8.5pt">{_rows_txt}</div>'
        )
        ...
        if _preg_banned and is_preg:
            ...
            H.append(
                f'<div class="alert al-danger" style="font-size:8.5pt;margin-top:1mm">'
                f'⛔ <b>Pregnancy Contraindicated:</b> {_pb_names}</div>'
            )

    # ── DOSE ADJUSTMENT / USE WITH CAUTION — full detailed section ──────
    if warned:
        H.append('<div class="sec-ttl" style="margin-top:1mm;color:#b7770d;border-bottom-color:#b7770d">'
                 '⚠ Dose Adjustment / Use with Caution</div>')
        if is_renal:
        ...
        for _wd in warned:
            ...
            H.append(
                f'<div style="margin:0.5mm 0;padding:1mm 2.5mm;border-radius:2mm;'
                ...
                f'<span style="padding:0.2mm 2mm;border-radius:2mm;font-size:8pt;'
            ...
            if _wreason == "renal_adjustment":
                ...
                _en = _esc(_wd.get("esbl_note","ESBL organism — BLI combo for uncomplicated UTI only"))
                H.append(
    ...
    if interactions:
        H.append('<div class="sec-ttl" style="margin-top:1mm">💊 Drug Interactions</div>'
    ...
    H.append('<div class="grid2" style="margin-top:1mm">')
    ...
    if patho_assessment:
        ...
        if fpos:
            H.append('<div style="font-size:8.5pt;color:#1e8449;margin-top:1mm"><b>Supporting:</b></div>')
            for f in fpos[:3]:
                H.append(f'<div style="font-size:8.5pt;color:#1e8449">{_esc(f[:80])}</div>')
        # Against factors
        if fneg:
            H.append('<div style="font-size:8.5pt;color:#b7770d;margin-top:0.5mm"><b>Against:</b></div>')
            for f in fneg[:3]:
                H.append(f'<div style="font-size:8.5pt;color:#b7770d">{_esc(f[:80])}</div>')
        # Recommendations
        if recs:
            H.append('<div style="font-size:8.5pt;font-weight:bold;margin-top:0.5mm">Recommendations:</div>')
            for r in recs[:3]:
    ...
    if is_preg and preg_warn_items:
        ...
        H.append(
            ...
            '(Physician decision required for all items below)</span></div>'
        )
        for _pw in preg_warn_items:
            ...
            _aware_style = {
                "Access":  "background:#eafaf1;border:0.8pt solid #1e8449",
                "Watch":   "background:#fef9e7;border:0.8pt solid #b7770d",
                "Reserve": "background:#fdf2f2;border:0.8pt solid #922b21",
            }.get(_paw, "background:#f8f5fd;border:0.8pt solid #7d3c98")
            H.append(
                f'<div style="margin:0.5mm 0;padding:1mm 2.5mm;border-radius:2mm;'
            ...
            if _pnote:
                ...
                for _line in _pnote.splitlines():
                    ...
                    H.append(
                        ...
                        f'border-radius:1mm">{_esc(_line)}</div>'
                    )
            H.append('</div>')
